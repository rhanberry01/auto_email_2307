...
function set_fullmode() {
	...
	text_row(_("User name"), "user_name_entry_field", 'admin', 20, 30);

	$password = $allow_demo_mode ? "password":"";
	password_row(_("Password:"), 'password', 'srsnova');
	$company = ($_GET['branch_name'] != "" && $_GET['branch_name'] != "tondo") ? $_GET['company'] : '0';
	$branch_name = ($_GET['branch_name'] != "") ? $_GET['branch_name'] : 'tondo';
	text_row(_("Branch Name:"), 'branch_name', $branch_name);
		...
		if (isset($_SESSION['wa_current_user']->company))
			...
			$coy = $def_coy;
		echo text_row(_("Company"), 'company_login_name', $company);
	...
	end_table(1);
	echo "<center><input type='submit' id='btn_login' value='&nbsp;&nbsp;"._("Login -->")."&nbsp;&nbsp;' name='SubmitUser'"
