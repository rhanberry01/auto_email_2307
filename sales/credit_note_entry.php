...
if(isset($_GET['NewCredit'])) {
	...
	$_SESSION['page_title'] = sprintf(_("Modifying Customer Credit Note #%s"), $_GET['ModifyCredit']);
...
if (isset($_GET['AddedID'])) {
	...
	display_notification_centered(sprintf(_("Credit Note # %s has been processed"),$credit_no));

	display_note(get_customer_trans_view_str($trans_type, $credit_no, _("&View this credit note")), 0, 1);

	display_note(print_document_link($credit_no."-".ST_CUSTCREDIT, _("&Print This Credit Note"), true, ST_CUSTPAYMENT),0, 1);
