...
$js .= "
	...
	function openThickBox2(id){
		url = 'customer_del_so.php?OrderNumber=' + id + '&type=1&view=0&KeepThis=true&TB_iframe=true&height=280&width=300';
		tb_show('Void SO', url);
	}
";
...
//	$(document).ready(function(){
	...
	//	alert(ty);
		...
		alert(val+ ' - '+ev+ ' - '+f + ' - ' + f.uname + ' - ' + f.passwd);
			if(val==true)
				if(f.uname!='' && f.passwd!=''){
		...
		function opemprompt_n(ty, fields,extra){
			if(fields == 0){
...
if (isset($_GET['NewDelivery']) && is_numeric($_GET['NewDelivery'])) {
 ...
 unset($_SESSION['cdetails']);
	...
	$_SESSION['page_title'] = sprintf( _("Modifying Sales Order # %s"), $_GET['ModifyOrderNumber']);
...
if (isset($_GET['AddedID'])) {
	...
	echo "
		...
		<!--//a href='javascript:openThickBox2($order_no)'>Void This Sales Order</a//-->
		</center>
	";
...
function can_process() {
	...
	*/
	...
	//display_error('from location '.$branch['default_location'].' - deliver to '.$branch['br_name'].' - del add '.$cust['address'].' - phone '.$branch['phone']);
	//display_error($_POST['p_terms'] . ' - ' . $cust['payment_terms']);
	...
	//if($_SESSION['Items']->trans_type!=ST_SALESORDER){
		if($_POST['p_terms'] != $cust['payment_terms'] && !$_SESSION['cdetails']){
			...
			$stats = false;
			$fields = 1;
			display_error('pterms mismatch ' . $fields);
			$countme++;
		}else{
			$fields = 0;
		}
...
global $fields;
	...
	display_error("<a class='limitLink' onclick='opemprompt_n(4,".$fields.",1)' style='cursor:pointer'>Click here to enable customer to transact</a>");	
	else
	...
	{
	if($_SESSION['fields_change'])
	display_error("<a class='limitLink' onclick='opemprompt_n(4,".$fields.",2)' style='cursor:pointer'>Click here to enable customer to transact</a>");	
	else
	...
	{
	if($_SESSION['fields_change'])
	display_error("<a class='limitLink' onclick='opemprompt_n(4,".$fields.",3)' style='cursor:pointer'>Click here to enable customer to transact</a>");	
	else
	display_error("<a class='limitLink' onclick='openprompt_(2)' style='cursor:pointer'>Click here to enable customer to transact</a>");	
		...
		//$chk_credit = 1;
			...
			display_error("<a class='limitLink' onclick='opemprompt_n(4, ".$fields.")' style='cursor:pointer'>Click here to enable customer to transact</a>");	
			return false;
	}
...
// if (isset($_POST['ProcessOrder']) && $_POST['noti_']==1) {
	//display_error('before copy to cart -> '.$_POST['salesman']);
	copy_to_cart();
	//display_error('before copy from cart -> '.$_POST['salesman']);
	copy_from_cart();
	//display_error('after copy from cart -> '.$_POST['salesman']);
	// if($_POST['noti_']==0){
...
function check_item_data()
{
	...
	if($_POST['stock_id']==''||!isset($_POST['stock_id'])){
		...
		display_error("Please choose a price list first.");
			set_focus('sales_type');
		return false;
	}else if(!check_num('qty', 0) || !check_num('Disc', 0, 100)) {
		...
		is_inventory_item($_POST['stock_id']))
	{
		...
		if (input_num('qty') > $qoh && $order->trans_type !== ST_SALESORDER)
		{
			if(!isset($_SESSION['allownegativecost']) || $_SESSION['allownegativecost'] == 0){
				$_SESSION['negative_inv']=1;
