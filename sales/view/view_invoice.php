...
function skip_dr($trans_no)
{
	$sql = "SELECT skip_dr FROM ".TB_PREF."debtor_trans 
		WHERE type = 10
...
//display_heading(sprintf(_("SALES INVOICE #%s"),getReferencebyType($trans_id, 'INV')));
display_heading(sprintf(_("SALES INVOICE"),''));
...
start_table("$table_style width=100%");
start_row();
label_cells(_("Invoice #"), $myrow["reference"], "class='tableheader2'");
...
$dr_id = get_parent_trans(ST_SALESINVOICE,$trans_id);

// if(skip_dr($trans_id) == 0)
	// label_cells(_("Deliveries"), get_customer_trans_view_str(ST_CUSTDELIVERY, get_parent_trans(ST_SALESINVOICE,$trans_id), getReferencebyType($dr_id[0], 'DR')), "class='tableheader2'");
// else
	// label_cells(_("Deliveries"), "", "class='tableheader2'");
...
if (db_num_rows($result) > 0)
{
	...
	while ($myrow2 = db_fetch($result))
	{
		...
		$value = $myrow2["unit_price"] * $myrow2["quantity"] * (1-$myrow2["discount_percent"]) * (1-$myrow2["discount_percent2"]) * (1-$myrow2["discount_percent3"]) * (1-$myrow2["discount_percent4"]) * (1-$myrow2["discount_percent5"]) * (1-$myrow2["discount_percent6"]);
		$sub_total += $value;
...
$display_total = price_format(
					$myrow["ov_freight"]+$myrow["ov_gst"]+$myrow["ov_freight_tax"]+ 
					(	$myrow["ov_amount"]
...
label_row(_("Total Sales"), $display_sub_tot, "colspan=7 align=right", "nowrap align=right width=15%");

$_tax_items = get_trans_tax_details(ST_SALESINVOICE, $trans_id);
...
label_row(_("Amount Total"), $display_total, "colspan=7 align=right", "nowrap align=right");
end_table(1);
