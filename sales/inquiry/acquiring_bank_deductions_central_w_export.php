...
function cashier_summary_per_day_excel()
{
				...
				global $path_to_root,$Ajax,$db_connections;
...
$bank_header=$selected_bank." Credit";
    ...
    $rep = new FrontReport(_('Recon'), "recon", "LETTER");
	...
	$rep->y ++;
	$rep->sheet->writeString($rep->y, 0, 'CREDIT/ DEBIT CARD RECONCILIATION', $format_bold);
	$rep->y ++;
...
$th = array(' ','Branch',_("Transaction Date"), 'Invoice#', $bank_header,'Merchant Fee (%)','Merchant Fee Amt.','CWTax (%)','CWTax Amount', 'Net Amount');
...
//MYSQL QUERY AND DATA
	...
	$a_b=$acquiring_bank;
		...
		$x++;
		$rep->sheet->writeString($rep->y, $x, $row['br_name'],$format_left);
		$x++;
		
	
	if ($charge_back==1) {
		$rep->sheet->writeString($rep->y, $x, $row['TRANS_DATE'], $format_left);
		$x++;
	}
	else {
		$rep->sheet->writeString($rep->y, $x, $row['TRANS_DATE'], $format_left);
	...
	//label_cell($row['APPROVALNO']);
		...
		$rep->sheet->writeString($rep->y, $x, $row['APPROVALNO'], $format_left);
		$x++;

	if (($selected_bank=='BPI' or $selected_bank=='bpi') and ($card_tender_type=='013')) {
	//amount_cell($row['BPI_CREDIT']);

		$rep->sheet->writeString($rep->y, $x, $row['BPI_CREDIT'], $format_left);
		$x++;
	...
	$rep->sheet->writeString($rep->y, $x, $row['BPI_DEBIT'], $format_left);
	$x++;
	...
	$rep->sheet->writeString($rep->y, $x, $row['PNB_CREDIT'], $format_left);
	$x++;
	...
	$rep->sheet->writeString($rep->y, $x, $row['PNB_DEBIT'], $format_left);
	$x++;
	...
	$rep->sheet->writeString($rep->y, $x, $row['METROBANK_CREDIT'], $format_left);
	$x++;
	...
	$rep->sheet->writeString($rep->y, $x, $row['METROBANK_DEBIT'], $format_left);
	$x++;
	...
	$rep->sheet->writeString($rep->y, $x, $fee, $format_left);
	$x++;
	$rep->sheet->writeString($rep->y, $x, $mdr, $format_left);
	$x++;
	$rep->sheet->writeString($rep->y, $x, $wt, $format_left);
	$x++;
	$rep->sheet->writeString($rep->y, $x, $cwt, $format_left);
	$x++;
	$rep->sheet->writeString($rep->y, $x, $net, $format_left);
	$x++;
	$rep->y++;
	if ($charge_back!=1) {
