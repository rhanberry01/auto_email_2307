	 ...
	 if ($approve_id != -1)
{
	...
	$void_entry = get_voided_entry($inserttype, $d_id);
	db_fetch_row($void_entry);
	if ($void_entry>0)
	{
	
	$sqldelrefs = "Delete FROM ".TB_PREF."voided
					WHERE type = '$inserttype' and id='$d_id'";
		db_query($sqldelrefs);
...
$sql = "SELECT * FROM ".TB_PREF."dep_exp_fixed_assets where inactive='0' and disposed='0'";
...
while($myrow = db_fetch($result))
{
	...
	submit_cells($submit, 'Depreciate', "align=center", true, true,'ok.gif');
		...
		label_cell(pager_link(_('Dispose'), "/sales/fixed_asset_disposal.php?asset_id=" .$myrow['dep_id']));
	}
