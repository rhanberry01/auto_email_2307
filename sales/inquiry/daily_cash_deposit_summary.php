...
page("Daily Cash Deposit Summary ", false, false, "", $js);
...
$dateto=date2sql($_POST['TransToDate']);
    ...
    $rep = new FrontReport(_('Daily Cash Deposit Summary Report'), "Daily_Cash_Deposit_Summary", "LETTER");
	...
	$rep->y ++;
	$rep->sheet->writeString($rep->y, 0, 'DAILY TOTAL CASH DEPOSIT', $format_bold);
	$rep->y ++;
...
$th = array(' ', _("Date"),  _("Total Cash Remit"),_("Undeposited Balance"),'Amount Deposited');
...
$mysql_q="SELECT date_remit,cash_remit,sum(cash_deposit) as amount_deposited,
cash_remit-sum(cash_deposit) as undeposited_bal FROM ".TB_PREF."cash_deposit 
WHERE date_remit>='".$datefrom."' and date_remit<='".$dateto."'
GROUP BY date_remit";
...
$res2=db_query($mysql_q);
	...
	{ 
		...
		$x++;
		$rep->sheet->writeNumber($rep->y, $x, $row['cash_remit'], $format_accounting);
		$x++;
		$rep->sheet->writeNumber($rep->y, $x, $row['undeposited_bal'], $format_accounting);
		$x++;
		$rep->sheet->writeNumber($rep->y, $x, $row['amount_deposited'], $format_accounting);
		$rep->y++;
	
	$t_total_cash+=$row['cash_remit'];
	$t_undeposited_bal+=$row['undeposited_bal'];
	$t_amount_deposited+=$row['amount_deposited'];
	}
	...
	$x++;
	$rep->sheet->writeString($rep->y, $x, $t_total_cash, $format_left);
	$x++;
	$rep->sheet->writeNumber($rep->y, $x, $t_undeposited_bal, $format_accounting);
	$x++;
	$rep->sheet->writeNumber($rep->y, $x, $t_amount_deposited, $format_accounting);
...
div_start('totals');		
display_heading("Daily Cash Deposit Summary From ".$_POST['date_']." To ".$_POST['TransToDate']."");
br();
...
label_cell($c,'align=right');
	...
	amount_cell($row['cash_remit']);
	amount_cell($row['undeposited_bal']);
	amount_cell($row['amount_deposited']);
	...
	label_cell("<font color=#880000><b>".number_format2(abs($t_total_cash),2)."<b></font>",'align=right');
	label_cell("<font color=#880000><b>".number_format2(abs($t_undeposited_bal),2)."<b></font>",'align=right');
	label_cell("<font color=#880000><b>".number_format2(abs($t_amount_deposited),2)."<b></font>",'align=right');
