...
$path_to_root = "../..";
include_once($path_to_root . "/includes/session.inc");
include_once($path_to_root . "/sales/includes/db/sales_remittance_new_version.inc");
...
$th = array(' ', _("Cashier Name"), _("Reading"), 'Cash', 'SRSGC', 'GC', 'Terms', 'EVoucher', 'Rice Promo','DD-Kita','Suki Card', 'Check', 'BPI Debit','BPI Credit','Metro Debit','Metro Credit','ATD','ST','Rec.','CWTax','Others','Diff','Short','Over','');
//---------------------------------------------------------------------------------
...
$diff1 = $row['total']- $mycash - $mysrsgc -$mygc - $myterms - $myevoucher-$myricepromo- $myddkita- $mysuki - $mycheck - $mybpidebit- $mybpicredit - $mymetrodebit- $mymetrocredit - $myatd - $myst - $myrec -$mycwtax -$myothers;
...
$res2=ms_db_query($sql);
															...
															$num_of_cashier = mssql_num_rows($res2);
																...
																$diff2 = $row2['total']- $mycash2 - $mysrsgc2 -$mygc2 - $myterms2 - $myevoucher2-$myricepromo2- $myddkita2- $mysuki2 - $mycheck2 - $mybpidebit2 - $mybpicredit2 - $mymetrodebit2- $mymetrocredit2 -$myatd2 - $myst2 - $myrec2 - $mycwtax2 -$myothers2;
																	//-$myothers
																	$x=0;
																	$rep->y++;
																	$rep->sheet->writeNumber($rep->y, $x, $c, $format_center);
																...
																if($diff2 <= 0)
																	...
																	$total_diff2=$simpos_sales2-($total_cash2+$total_srsgc2+$total_gc2+$total_terms2+$total_evoucher2+$total_ricepromo2+$total_ddkita2+$total_t_sc2+$total_t_check2+$total_t_bpi_dc2+$total_t_bpi_cc2+$total_t_metro_dc2+$total_t_metro_cc2+$total_atd2+$total_st2+$total_rec2+$total_cw_tax2+$total_o2);
...
//display_error($sql."333333333333333");
															...
															$num_of_cashier = mssql_num_rows($res3);
																...
																$diff3 = $row3['total']- $mycash3 - $mysrsgc3 -$mygc3 - $myterms3 - $myevoucher3-$myricepromo3- $myddkita3- $mysuki3 - $mycheck3 - $mybpidebit3 - $mybpicredit3 - $mymetrodebit3- $mymetrocredit3 -$myatd3 - $myst3 - $myrec3 - $mycwtax3 -$myothers3;
																	//-$myothers
																	$x=0;
																	$rep->y++;
																	$rep->sheet->writeNumber($rep->y, $x, $c, $format_center);
																...
																if($diff3 <= 0)
																	...
																	$total_diff3=$simpos_sales3-($total_cash3+$total_srsgc3+$total_gc3+$total_terms3+$total_evoucher3+$total_ricepromo3+$total_ddkita3+$total_t_sc3+$total_t_check3+$total_t_bpi_dc3+$total_t_bpi_cc3+$total_t_metro_dc3+$total_t_metro_cc3+$total_atd3+$total_st3+$total_rec3+$total_cw_tax3+$total_o3);
...
$th = array(' ', _("Cashier Name"), _("Reading"), 'Cash', 'SRSGC', 'GC', 'Terms', 'EVoucher', 'Rice <br> Promo','DD-Kita','Suki <br> Card', 'Check', 'BPI <br> Debit','BPI <br> Credit','Metro <br> Debit','Metro <br> Credit','ATD','ST','Rec.','CWTax','Others','Diff','Short','Over','');
table_header($th);
//---------------------------------------------------------------------------------
...
add_gl_trans(ST_SALESTOTAL, $tsd_id, $_POST['t_date'], $sales_account, 0, 0, $memo, -$_POST['t_simpos'], null, 0);
		...
		// if (!check_sales(ST_SALESTOTAL, $tsd_id))
			// continue;
		
		// $gross = $_POST['t_simpos'];
		// $date_ = $_POST['t_date'];
		// $suki = $_POST['t_t_sc'];
		...
		// delete_wrong_sales_gl($tsd_id);
		// add_gl_trans(ST_SALESTOTAL, $tsd_id, $date_, 4000040, 0, 0, $memo_= '',-$suki);
		// add_gl_trans(ST_SALESTOTAL, $tsd_id, $date_, 4000020, 0, 0, $memo_= '',-$nv_sales);
		
		// if ($zr_sales != 0)
			// add_gl_trans(ST_SALESTOTAL, $tsd_id, $date_, 4000050, 0, 0, $memo_= '',-$zr_sales);
		
		// add_gl_trans(ST_SALESTOTAL, $tsd_id, $date_, 4000, 0, 0, $memo_= '',-$sales_vat);
		// add_gl_trans(ST_SALESTOTAL, $tsd_id, $date_, 2310, 0, 0, $memo_= '',-$output_vat);
