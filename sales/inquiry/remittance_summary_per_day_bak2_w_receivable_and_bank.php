...
function cashier_summary_per_day_excel()
{
	...
	$th = array(' ', _("Cashier Name"), _("Reading"), 'Cash', 'SRSGC',  'GC', 'Terms', 'EVoucher', 'Suki Card', 'Check', 'Debit Card', 'Credit Card','ATD','ST','Rec.','Others',
		'Diff','Short','Over','');
...
$total_debit_reading = get_total_debit_reading($_POST['date_']);
	...
	$debit=$debit_read[$row['cashier_id']];
		...
		$diff = $reading - $row['t_cash'] - $row['t_srsgc'] - $row['t_gc'] -  $row['t_terms'] - $row['t_evoucher'] - $row['t_sc'] - $row['t_check'] - $row['t_dc'] - $row['t_cc'] - $row['t_atd'] - $row['t_st']- $row['t_rec']- $row['t_o'];
		...
		$rep->sheet->writeNumber($rep->y, $x, $row['t_rec'], $format_accounting);
		$x++;
	...
	$rep->y++;
															...
															$res2=ms_db_query($sql);
																...
																$myrec=$myremitrec[$row2['uid']];
																$myothers=$myremitothers[$row2['uid']];
																$myid=$myremitid[$row2['uid']];
																
															
																	$diff2 = $row2['total']- $mycash - $mysrsgc -$mygc - $myterms - $myevoucher- $mysuki - $mycheck - $mydebit - $mycredit - $myatd - $myst - $myrec -$myothers;
																	//-$myothers
																	$x=0;
																	$rep->y++;
																	$rep->sheet->writeNumber($rep->y, $x, $c, $format_center);
...
$_POST['t_receivable'],$_POST['t_short'],$_POST['t_over'],$approved=1,$processed=0);
...
add_gl_trans(ST_SALESTOTAL, $tsd_id, $_POST['t_date'], $gl_bpi_account, 0, 0, $memo, $bpiamount, null, 0);
}
...
add_gl_trans(ST_SALESTOTAL, $tsd_id, $_POST['t_date'], $gl_pnb_account, 0, 0, $memo, $pnbamount, null, 0);
}
...
add_gl_trans(ST_SALESTOTAL, $tsd_id, $_POST['t_date'], $gl_mb_account, 0, 0, $memo, $mbamount, null, 0);
}
