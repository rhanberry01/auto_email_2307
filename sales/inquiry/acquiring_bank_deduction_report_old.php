...
function cashier_summary_per_day_excel()
{
    ...
    $rep = new FrontReport(_('Passbook'), "Passbook", "LETTER");
...
$th = array(' ','BankCard', 'Type', _("Batch#"), _("Trans#"), _("TransDate"),'DateRemit','Invoice#', 'Terminal ID', 'Card#', 'Approval#', 'Amount', 'MerchantFee','WTax','Debit Net','Credit Net','Date Paid');
...
$sql.=" ORDER BY p_ref_id, p_terminal_id, p_invoice_no asc";
...
//display_error($sql);
	...
	$x++;
		...
		$x++;
		$rep->sheet->writeNumber($rep->y, $x, $row['p_invoice_no'], $format_left);
		$x++;
		$rep->sheet->writeNumber($rep->y, $x, $row['p_terminal_id'], $format_left);
		$x++;
		$rep->sheet->writeString($rep->y, $x, $row['p_account_no'], $format_left);
		$x++;
		$rep->sheet->writeString($rep->y, $x, $row['p_approval_no'], $format_accounting);
		$x++;
...
div_start('totals');		
display_heading("Passbook Summary from ".$_POST['date_']." to ".$_POST['TransToDate']."");
br();
...
$th = array(' ','BankCard', 'Type',  _("Batch#"),  _("Trans#"), _("TransDate"),'DateRemit','Invoice#', 'Terminal ID', 'Card#', 'Approval#', 'Amount', 'MerchantFee','WithholdingTax','Debit Net','Credit Net','Date Paid');
table_header($th);
...
while($row = db_fetch($res))
{
	...
	label_cell(get_gl_view_str(ST_CREDITDEBITDEPOSIT, $row["p_ref_id"], $row["p_ref_id"]));
	label_cell($row['p_trans_no']);
	label_cell($row['p_trans_date']);
	label_cell($row['p_remittance_date']);
	label_cell($row['p_invoice_no']);
	label_cell($row['p_terminal_id']);
	label_cell($row['p_account_no']);
	label_cell($row['p_approval_no']);
	amount_cell($row['p_deposited_amount']);
	amount_cell($row['p_mfeeamount']);
	amount_cell($row['p_wtaxamount']);
	
	if ($row['p_tender_type']=='13')
	{
	label_cell('');
	amount_cell($row['p_net_total']);
	$credit_net_total+=$row['p_net_total'];
	}
	...
	label_cell("<font color=#880000><b>".number_format2(abs($t_deposited_amount),2)."<b></font>",'align=right');
	label_cell("<font color=#880000><b>".number_format2(abs($t_mfeeamount),2)."<b></font>",'align=right');
	label_cell("<font color=#880000><b>".number_format2(abs($t_wtaxamount),2)."<b></font>",'align=right');
	label_cell("<font color=#880000><b>".number_format2(abs($debit_net_total),2)."<b></font>",'align=right');
	label_cell("<font color=#880000><b>".number_format2(abs($credit_net_total),2)."<b></font>",'align=right');
	label_cell('');
	end_row();
