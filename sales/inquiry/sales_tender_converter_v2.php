...
$th = array(_('UserID'),_('Transaction Date'),_('Cashier Name'),_('TransactionNo'), _('From Tender Type'),_('Amount'),_('TerminalNo'));
//verified_control_column($th);
table_header($th);
$k = 0; //row colour counter
$previous_tender = "";
$previous_terminal = "";
$total_cashier_trans = 0; 
$total_cashier_terminal = 0; 
while($myrow = mssql_fetch_array($res))
{

	if($previous_terminal!=$myrow['TerminalNo']){
		if($total_cashier_terminal != 0){
		    start_row();
			label_cell('');	
			label_cell('');
			label_cell('');
			label_cell('<font color=#880000><b>'.'TOTAL FOR TERMINAL : '.$previous_terminal. '</b></font>','colspan=2' );
			label_cell("<font color=#880000><b>".number_format2(abs($total_cashier_terminal),2)."<b></font>",'align=right');
			label_cell('');
			end_row();
		}
		$total_cashier_terminal = 0;
		$previous_terminal=$myrow['TerminalNo'];
	}

	if($previous_tender!=$myrow['UserID'])
	{

		if($total_cashier_trans != 0){
			start_row();
			label_cell('');	
			label_cell('');
			label_cell('');
			label_cell('<font color=#880000><b>'.$previous_cashier.' TOTAL TRANSACTION:'.'</b></font>','colspan=2');
			label_cell("<font color=#880000><b>".number_format2(abs($total_cashier_trans),2)."<b></font>",'align=right');
			label_cell('');
			end_row();
		}	

		alt_table_row_color($k);
		label_cell($myrow['name'].":" ,'colspan=7 class=tableheader2');
		end_row();
		alt_table_row_color($k);
		$total_cashier_trans = 0;
		$previous_tender=$myrow['UserID'];
		$previous_cashier=$myrow['name'];
...
end_row();
$total_cashier_terminal +=$myrow["Amount"];
$total_cashier_trans +=$myrow["Amount"];
