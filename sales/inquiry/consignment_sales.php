...
$supp_trans_no=$row['supp_trans_no'];
}
...
$cv_id=auto_create_cv($supp_trans_no);
...
$supp_ref='INVOICE#: '.$invoice_num;

if($supp_trans_no!='' and $cv_id!=''){
		$sql1 = "UPDATE ".TB_PREF."supp_trans SET 
		supp_reference='$supp_ref'
		WHERE type=20
		and trans_no='$supp_trans_no'
		and cv_id='$cv_id'
		";

		db_query($sql1,'failed to update consignment apv reference');
...
$th = array(' ','Ref#','VendorCode', 'Description','Com.(%)', _("Quantity"), 'CostOfSales',_("Sales"),'','','E-mail Address','Purchaser','Items','APV','Invoice#','CV#','CV Date');
table_header($th);
...
while($row = db_fetch($res))
{
	...
	label_cell(get_trans_view_str(20,$row['supp_trans_no'],$row['supp_trans_no']));
