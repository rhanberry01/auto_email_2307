...
$dateto=date2sql($_POST['TransToDate']);
	...
	$rep->sheet->setColumn(1,2,13); //set column width
	$rep->sheet->setColumn(3,4,30); //set column width
	$rep->sheet->setColumn(5,7,13); //set column width
	$rep->sheet->setColumn(7,8,25); //set column width
	$rep->sheet->setColumn(9,15,13); //set column width
...
$th = array("",_('Date Acquired'),_('APV#'),_('Asset Description'), _('Supplier'),_('Inv.#'),'Asset#',' Serial#','Asset Type', _('Life'),_('Months Left'),_('Acquisition Cost'),'Monthly Depreciation',_('Accum. Depreciation'),_('Net Book Value'),_('Total Loss'),_('Status') );
...
$mysql_q="select de.dep_id as de_id, de.date_acquired as de_date_acquired, de.apv_num as de_apv_num, de.invoice_num as de_invoice_num, de.asset_name as de_asset_name,
 ...
 de.asset_gl_type as de_asset_gl_type, de.disposed as de_disposed,  ded.d_id as ded_d_id, de.acquisition_cost as de_acquisition_cost,ded.d_monthly_depreciation as ded_d_monthly_depreciation,
case when round(de.acquisition_cost-sum(abs(gl.amount)))='0'
 then abs(sum(gl.amount))-dfa.dis_total_loss  else abs(sum(gl.amount)) end as accum_depreciation,
round(de.acquisition_cost-sum(abs(gl.amount))) as net_book_value, 
dfa.dis_total_loss as total_loss,
dfa.dis_date as disposal_date
...
left join ".TB_PREF."disposed_fixed_asset as dfa 
on gl.type_no=dfa.dis_trans_no
...
while($myrow = mysql_fetch_array($res2))
{
	...
	$monthly_depreciation=$myrow["de_acquisition_cost"]/$months_life;
		...
		if ($myrow["net_book_value"]<='1')
	{
	$rep->sheet->writeNumber($rep->y, $x, round($myrow["accum_depreciation"]), $format_accounting);
	$x++;
	$t_book_value2+=$myrow["net_book_value"];
	$t_accum_depreciation_expense2+=$myrow['accum_depreciation'];
	$t_accum_depreciation_expense=round($t_accum_depreciation_expense2);
	}
	else {
	...
	if ($myrow["net_book_value"]<='0'){
	$rep->sheet->writeNumber($rep->y, $x, round($myrow["total_loss"]), $format_accounting);
	$x++;
	$rep->sheet->writeString($rep->y, $x, 'Disposed',$format_left);
	$x++;
	$gt_loss+=round($myrow["total_loss"]);
	}
	else{
	$rep->sheet->writeNumber($rep->y, $x, 0, $format_accounting);
	$x++;
	$rep->sheet->writeString($rep->y, $x, 'In Use',$format_left);
	$x++;
	}
	...
	$x++;
		$rep->sheet->writeNumber($rep->y, $x, abs($gt_loss), $format_accounting);
	$x++;
	$rep->sheet->writeString($rep->y, $x, '', $format_bold);
	$x++;
...
alt_table_row_color($k);
	...
	if ($myrow["net_book_value"]<='1')
	{
	label_cell("<font color=red><b>".number_format2(round($myrow["accum_depreciation"]),2)."<b></font>",'align=right');
	$t_book_value2+=$myrow["net_book_value"];
	$t_accum_depreciation_expense2+=$myrow['accum_depreciation'];
	$t_accum_depreciation_expense=round($t_accum_depreciation_expense2);
	}
	...
	if ($myrow["net_book_value"]<='0'){
	amount_cell(round($myrow["total_loss"]),'nowrap');	
	label_cell('Disposed','nowrap');
	$gt_loss+=round($myrow["total_loss"]);
	}
	else{
	amount_cell(0,'nowrap');	
	label_cell('In Use','nowrap');
	}
	...
	label_cell("<font color=#880000><b>".number_format2(abs($gt_loss),2)."<b></font>",'align=right');
	label_cell('');
div_end();
end_table(1);
end_form();
