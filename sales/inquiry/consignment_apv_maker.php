...
$sql = "SELECT * FROM ".TB_PREF."cons_sales_header WHERE supp_trans_no=0 and cv_id=0 and start_date>='2018-01-01'";
$res = db_query($sql);	
while($row = db_fetch($res))
{
		$invoice_num='';
		...
		$create_cv_id=$row['cons_sales_id'];
		$ref="CS".$row['cons_sales_id'];
		$amount=$row['t_sales'];
	...
	if($sup_id!=''){
		...
		if ($vat_stat==1) {
			...
			$invoice_no = add_supp_trans(ST_SUPPINVOICE, $sup_id,sql2date($end_date),sql2date($end_date),$refref,$supp_ref='',$amount, 0, 0,"",0,0,sql2date($end_date),0,0,0);
			...
			add_gl_trans_supplier(ST_SUPPINVOICE, $invoice_no, sql2date($end_date), $purchases, 0, 0,$amount-$tax_amount, $sup_id, "", $rate);
			add_gl_trans_supplier(ST_SUPPINVOICE, $invoice_no, sql2date($end_date), $input_tax, 0, 0,$tax_amount, $sup_id, "", $rate);
			add_gl_trans_supplier(ST_SUPPINVOICE, $invoice_no, sql2date($end_date), $accounts_payable, 0, 0,-$amount, $sup_id, "", $rate);
			}

		else {
		$purchases=5400;
		$invoice_no = add_supp_trans(ST_SUPPINVOICE, $sup_id,sql2date($end_date),sql2date($end_date),$refref,$supp_ref='',$amount, 0, 0,"",0,0,sql2date($end_date),1,0,0);
		...
		add_gl_trans_supplier(ST_SUPPINVOICE, $invoice_no, sql2date($end_date), $purchases, 0, 0,$amount, $sup_id, "", $rate);
		add_gl_trans_supplier(ST_SUPPINVOICE, $invoice_no, sql2date($end_date), $accounts_payable, 0, 0,-$amount, $sup_id, "", $rate);
		}

		//$cv_id=auto_create_cv($invoice_no);

		add_comments(ST_SUPPINVOICE, $invoice_no, sql2date($end_date), $memo_="Sales from ".sql2date($start_date)." to ".sql2date($end_date).", Consignment#: ".$ref);
...
display_notification("APV Created");
}
...
start_row();
	...
	year_list_cells('Year:','d_year',2016);
...
br();
submit_center('create_apv','Create APV');
br();
