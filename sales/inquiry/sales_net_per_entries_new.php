...
FROM ".TB_PREF."gl_trans as gl where gl.type='60' 
and gl.account='1010'
...
$sql="SELECT sum(cd_gross_amount) as t_amount FROM cash_deposit.0_cash_dep_header where cd_trans_type='61'
and cd_sales_date>='".date2sql($start_date)."'  and cd_sales_date<='".date2sql($end_date)."' 
and cd_date_deposited> '".date2sql($end_date)."'
and cd_br_code='sri'
and cd_cleared='1'";
//display_error($sql);
$res = db_query($sql);
...
$sql="SELECT sum(cd_gross_amount)  as t_amount FROM cash_deposit.0_cash_dep_header where cd_trans_type='61'
and cd_sales_date>= '".date2sql(add_months($start_date,-1))."'  and cd_sales_date<='".date2sql(end_month($end_date2))."'
and cd_date_deposited> '".date2sql(end_month($end_date2))."'
and cd_br_code='sri'
and cd_cleared='1'";
//display_error($sql);
$res = db_query($sql);
...
$sql="SELECT sum(cd_gross_amount) as t_amount FROM cash_deposit.0_cash_dep_header where cd_trans_type='61'
and cd_date_deposited>='".date2sql($start_date)."'  and cd_date_deposited<='".date2sql($end_date)."' 
and cd_br_code='sri'
and cd_cleared='1'
...
get_all_cash_on_hand($_POST['start_date'],$_POST['end_date'],$type,$bank_account_code,$bank_id);
label_cell(get_gl_account_name(1020021) ,'nowrap');
$amount_cib=get_coh_credit($_POST['start_date'],$_POST['end_date'],$type,$bank_account_code,$bank_id);
amount_cell(abs($amount_cib),false);
// $amount2=get_coh_debit($_POST['start_date'],$_POST['end_date'],$type,$bank_account_code,$bank_id);
...
//AMOUNT============
// display_error($debit);
// display_error($amount2);


if ($debit!='' and $credit!='') {
start_row();
label_cell('<b><font color=#880000>TOTAL:</font></b>','align=right');
label_cell("<font color=#880000><b>".number_format2(abs($credit+$amount1+$amount_cib),2)."<b></font>",'align=right');
label_cell("<font color=#880000><b>".number_format2(abs(abs($debit)+abs($amount2)),2)."<b></font>",'align=right');
end_row();
}
else {
