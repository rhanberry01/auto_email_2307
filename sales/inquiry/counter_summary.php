...
function counter_summary_excel()
{
	...
	while($r_row = mssql_fetch_array($r_res))
	{
		...
		$last_or_sql .= "SELECT DISTINCT
		TerminalNo,
		max(TransactionNo) as last_trans_no,
		COUNT(DISTINCT LogDate) as zcounter
		FROM FinishedSales 
		WHERE LogDate >='".date2sql($_POST['date_from'])."'
		AND LogDate <='".date2sql($_POST['date_to'])."'
		GROUP BY TerminalNo ORDER BY TerminalNo";
		$last_or_res = ms_db_query($last_or_sql);
		$last_res = ms_db_query($last_or_sql);
		...
		$rep->y++;
		$rep->y++;
		$rep->sheet->writeString($rep->y, $x, 'Last OR#:', $format_bold_right);
		$x++;
		while($last_or_row = mssql_fetch_array($last_or_res))
		{
		$c ++;
		$rep->sheet->writeNumber($rep->y, $x, $last_or_row['last_trans_no'], $format_bold_title);
		$x++;
		}
		
		// $last_sql .= "SELECT DISTINCT
		// TerminalNo,
		// max(TransactionNo) as last_trans_no,
		// COUNT(DISTINCT LogDate) as zcounter
		// FROM FinishedSales 
		...
		// AND LogDate <='".date2sql($_POST['date_to'])."'
		// GROUP BY TerminalNo ORDER BY TerminalNo";
		// $last_res = ms_db_query($last_sql);
		
		$x =0;
		$rep->y++;
		$rep->y++;
		$rep->sheet->writeString($rep->y, $x, 'ZCounter:', $format_bold_right);
		$x++;
		while($last_row = mssql_fetch_array($last_res))
		{
		$c ++;
		$rep->sheet->writeNumber($rep->y, $x, $last_row['zcounter'], $format_bold_title);
		$x++;
		}
