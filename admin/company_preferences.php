...
if (isset($_POST['update']) && $_POST['update'] != "")
{
	...
	else if (strlen($_POST['coy_name'])==0)
	{
		$input_error = 1;
		display_error(_("The company name must be entered."));
		set_focus('coy_name');
	}
	else if (strlen($_POST['gst_no'])==0)
	{
		$input_error = 1;
		display_error(_("Tin No. must be entered."));
		set_focus('gst_no');
	}	
	else if (strlen(str_replace("-", "", $_POST['gst_no']))<12 || strlen(str_replace("-", "", $_POST['gst_no']))>12)
	{
		$input_error = 1;
		display_error(_("Tin No. must be 12-character long."));
		set_focus('gst_no');
	}
	else if (strlen($_POST['rdo_code'])==0)
	{
		$input_error = 1;
		display_error(_("RDO code must be entered."));
		set_focus('rdo_code');
	}
	else if (strlen($_POST['rdo_code'])<3)
	{
		$input_error = 1;
		display_error(_("RDO code must be 3-character long."));
		set_focus('rdo_code');
	}
	else if (strlen($_POST['zip_code'])==0)
	{
		$input_error = 1;
		display_error(_("Zip code must be entered."));
		set_focus('zip_code');
	}
	else if (strlen($_POST['zip_code'])<4)
	{
		$input_error = 1;
		display_error(_("Zip code must be 4-character long."));
		set_focus('zip_code');
	}
	else if (strlen($_POST['atc'])==0)
	{
		$input_error = 1;
		display_error(_("ATC must be entered."));
		set_focus('atc');
	}
	else if (strlen($_POST['atc'])>8)
	{
		$input_error = 1;
		display_error(_("ATC must be 8-character long."));
		set_focus('atc');
	}
	else if (strlen($_POST['line_of_business'])==0)
	{
		$input_error = 1;
		display_error(_("Line of business must be entered."));
		set_focus('line_of_business');
	}
	...
	if ($input_error != 1)
	{
		update_company_setup($_POST['coy_name'], $_POST['coy_no'], 
			...
			$_POST['login_tout'], $_POST['rdo_code'], $_POST['zip_code'], 
			$_POST['atc'], $_POST['line_of_business'],input_num('perc_tax'),input_num('ewt_percent'));
...
$_POST['rdo_code'] = $myrow['rdo_code'];
$_POST['zip_code'] = $myrow['zip_code'];
$_POST['atc'] = $myrow['atc'];
$_POST['perc_tax'] = number_format($myrow['perc_tax'],user_percent_dec());
$_POST['ewt_percent'] = number_format($myrow['ewt_percent'],user_percent_dec());
$_POST['line_of_business'] = $myrow['line_of_business'];
if ($_POST['add_pct'] == -1)
	$_POST['add_pct'] = "";
...
text_row_ex(_("Name (to appear on reports):"), 'coy_name', 42, 250);
...
text_row_ex(_("Tax Identification No.:"), 'gst_no', 25);
...
percent_row("EWT Percentage :",'ewt_percent',$_POST['ewt_percent']);

text_row_ex(_("RDO Code:"), 'rdo_code', 10, 3);
text_row_ex(_("Zip Code:"), 'zip_code', 10, 4);
text_row_ex(_("ATC:"), 'atc', 10, 8);
text_row_ex(_("Line of Business:"), 'line_of_business', 42, 50);
...
percent_row("Percentage Tax:",'perc_tax',$_POST['perc_tax']);
