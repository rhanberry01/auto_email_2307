...
{
	begin_transaction();
	$sql = "SELECT id,br_code_out,DATE(transfer_in_date) as transfer_in_date FROM transfers.0_transfer_header WHERE m_id_in = '$m_id' AND br_code_in = '$branch' AND transfer_in_date>='2017-01-01' and transfer_in_date<='2017-12-31'";
	//display_error($sql);
	$res = db_query($sql);
	$row = db_fetch($res);
