...
page(_($help_context = "Event Locker (Closing of Accounts)"), false, false, "", $js);

simple_page_mode(true);
//---------------------------------------------------------------------------------------------

function is_date_in_lockedyears($date)
{
	$date = date2sql($date);
	$sql = "SELECT * FROM ".TB_PREF."event_locker WHERE '$date' >= begin AND '$date' <= end";

	$result = db_query($sql, "could not get all locked years");
...
{
	$bdate = date2sql($date);
	$sql = "SELECT MAX(end) FROM ".TB_PREF."event_locker WHERE begin < '$bdate'";

	$result = db_query($sql, "could not retrieve last locked years");
...
{
	$date = date2sql($date);
	$sql = "SELECT COUNT(*) FROM ".TB_PREF."event_locker WHERE begin < '$date'";
	if (!$closed)
		$sql .= " AND closed=0";

	$result = db_query($sql, "could not check locked years before");
	$row = db_fetch_row($result);
	return ($row[0] > 0);
}
...
function check_data()
{
	if (!is_date($_POST['from_date']) || is_date_in_lockedyears($_POST['from_date']) || is_bad_begin_date($_POST['from_date']))
	{
		display_error( _("Invalid BEGIN date in locked year."));
		set_focus('from_date');
		return false;
	}
	if (!is_date($_POST['to_date']) || is_date_in_lockedyears($_POST['to_date']))
	{
		display_error( _("Invalid END date in locked year."));
		set_focus('to_date');
		return false;
	}
...
function handle_submit()
{
	...
	if ($selected_id != -1)
	{
		if ($_POST['closed'] == 1)
		{
			...
			{
				display_error( _("Cannot CLOSE this year because there are open locked years before"));
				set_focus('closed');
				return false;
			}	
		...
		if ($ok)
		{
   			update_lockedyear($selected_id, $_POST['closed']);
			display_notification(_('Selected locked year has been updated'));
		}	
	}
	else
	{
		if (!check_data())
			return false;
   		add_lockedyear($_POST['from_date'], $_POST['to_date'], $_POST['closed']);
		display_notification(_('New locked year has been added'));
	}
	$Mode = 'RESET';
}
...
{
	$myrow = get_lockedyear($selected_id);
	// PREVENT DELETES IF DEPENDENT RECORDS IN gl_trans
	if (check_years_before(sql2date($myrow['begin']), true))
	{
		display_error(_("Cannot delete this locked year because thera are locked years before."));
		return false;
	}
	if ($myrow['closed'] == 0)
	{
		display_error(_("Cannot delete this locked year because the locked year is not closed."));
		return false;
	}
	return true;
}
...
function display_lockedyears()
{
	global $table_style;

	$company_year = get_company_pref('f_year');

	$result = get_all_locked_years();
	start_form();
	// display_note(_("Warning: Deleting a locked year all transactions 
		// are removed and converted into relevant balances. This process is irreversible!"), 
		// 0, 0, "class='currentfg'");
	start_table($table_style);

	$th = array(_("Locked Date Begin"), _("Locked Date End"), _("Closed"), "", "");
	table_header($th);
	...
	while ($myrow=db_fetch($result))
	{
		...
		// if ($myrow["id"] != $company_year) {
			...
			// submit_js_confirm("Delete".$myrow['id'],
				// sprintf(_("Are you sure you want to delete locked year %s - %s? All transactions are deleted and converted into relevant balances. Do you want to continue ?"), $from, $to));
		// } else
			label_cell('');
		end_row();
	}
	...
	end_table();
	end_form();
	//display_note(_("The marked locked year is the current locked year which cannot be deleted."), 0, 0, "class='currentfg'");
}

//---------------------------------------------------------------------------------------------

function display_lockedyear_edit($selected_id)
{
	...
	if ($selected_id != -1)
	{
		...
		hidden('from_date');
		hidden('to_date');
		label_row(_("Locked Date Begin:"), $_POST['from_date']);
		label_row(_("Locked Date End:"), $_POST['to_date']);
	}
	else
	{
		date_row(_("Locked Date Begin:"), 'from_date', '', null, 0, 0, 1001);
		date_row(_("Locked Date End:"), 'to_date', '', null, 0, 0, 1001);
	}
...
display_lockedyear_edit($selected_id);
