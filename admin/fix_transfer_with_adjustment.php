...
if (isset($_POST['fix_now']))
{
	...
	while($row = db_fetch($res))
	{
		...
		$sql_gl = "SELECT a.id, b.gl_stock_to
		FROM transfers.0_transfer_header a, transfers.0_branches b
		WHERE a.br_code_in = b.code
		AND a.id = $transfer_id";
		$res_gl = db_query($sql_gl);
		$row_gl = db_fetch($res_gl);
	
		$gl_to = $row_gl['gl_stock_to'];
		
		
		//$date_='12/31/2016';
		$date_=sql2date($transfer_out_date);
		$ref   = $Refs->get_next(0);
		$memo_ = "Adjustment for Correcting Entry of Stock Transfer#: ".$transfer_id." -Adjustment";
		...
		$sqli = "UPDATE transfers.0_transfer_header SET journal_trans_no = '$trans_id'  WHERE id = $transfer_id";
		//display_error($sqli);
		db_query($sqli,'failed to update transfer header');
		
		
					//=============UPDATING QTY RETURNED
					$sql_="SELECT 
					th.id,
					th.br_code_out,
					th.br_code_in,
					(select name from transfers.0_branches where code = th.br_code_out) as out_,
					(select name from transfers.0_branches where code = th.br_code_in) as in_,
					barcode,
					description,
					uom,
					actual_qty_out,
					qty_in,
					th.transfer_in_date,
					th.transfer_out_date
					FROM transfers.0_transfer_header as th
					LEFT JOIN transfers.0_transfer_details as td
					on th.id=td.transfer_id
					where th.transfer_out_date>='2016-01-01'  and th.transfer_out_date<='2016-09-31'
					and actual_qty_out!=qty_in
					and m_no_in!=0
					and th.id=$transfer_id";

					$res_ = db_query($sql_,"error");
					//display_error($sql_);
					
					while($row_ = db_fetch($res_))
					{
						$barcode=$row_['barcode'];
						$qty_out=$row_['actual_qty_out'];
						$qty_in=$row_['qty_in'];
						
						$returned_qty=$qty_out-$qty_in;
						
						
						$sqli_2 = "UPDATE transfers.0_transfer_details SET returned_qty = '$returned_qty'  WHERE transfer_id = $transfer_id AND barcode='$barcode'";
						db_query($sqli_2,'failed to post transfer in');
						//display_error($sqli_2);
					}
					//=======================================
