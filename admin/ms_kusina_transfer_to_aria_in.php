...
page('Kusina Transfer IN - MS Movement to ARIA (Net of VAT per Item)', false, false, "", $js);
set_time_limit(0);
//-----------------------------------------------------------------------------------
...
{
	begin_transaction();
	$sql = "SELECT aria_trans_no_in FROM transfers.0_transfer_header WHERE m_id_in = $m_id AND m_code_in = 'SA2KI' AND  date_created>='2017-01-01' and date_created<='2017-12-31'";
	//display_error($sql);
	$res = db_query($sql);
	$row = db_fetch($res);
	...
	$sql = "UPDATE 0_gl_trans SET amount = -$nettotal WHERE type = 68 AND type_no = $trans_no AND account = '570001'";
	//display_error($sql);
	db_query($sql, 'failed to update gl_trans (positive)'. 'for m_id : '.$m_id );
	$sql = "UPDATE 0_gl_trans SET amount = -$nettotal WHERE type = 68 AND type_no = $trans_no AND amount < 0";
	$sql = "UPDATE 0_gl_trans SET amount = $nettotal WHERE type = 68 AND type_no = $trans_no AND account != '570001'";
	// display_error($sql);
...
function correction_for_negative_amount($trans_no, $nettotal)
{
	...
	$sql = "DELETE FROM 0_gl_trans WHERE type = 68 AND type_no = $trans_no";
...
function get_net_of_vat_total($m_id,$branch)
{
	...
	while($row = db_fetch($res))
	{
		...
		$per_item_total = (get_net_of_vat_cost($row['stock_id_2'], $row['cost']) * ($row['qty_out']*$uoms_qty_multiplier));
...
{
	$sql = "SELECT MovementID,MovementNo FROM Movements WHERE MovementCode = 'SA2KI' and TransactionDate>='2017-01-01 00:00:00' AND TransactionDate<='2017-12-31 00:00:00'";
	// $sql .= " AND MovementID = 672289";
	$res = ms_db_query($sql);
