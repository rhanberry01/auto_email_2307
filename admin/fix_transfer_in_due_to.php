...
function adjust_gl_entry_due_to($trans_no, $account)
{
	$sql = "UPDATE 0_gl_trans SET account = '$account' 
				WHERE type = 71 AND type_no = $trans_no 
				AND account=''
				AND account != 570001";
				// AND amount < 0
	// display_error($sql);
...
if (isset($_POST['fix_now']))
{
	...
	$sql = "SELECT a.id, b.gl_stock_from
				...
				AND transfer_in_date>='2018-01-01'
				AND br_code_out = br_code_out
				AND br_code_in = ". db_escape($db_connections[$_SESSION["wa_current_user"]->company]['br_code']);
	// display_error($sql);
	// AND a.id = 1491
	$res = db_query($sql);
	...
	{
		$cc += adjust_gl_entry_due_to($row['id'], $row['gl_stock_from']);
	}
