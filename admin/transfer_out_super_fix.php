...
function adjust_gl_entry_double()
{
	...
	while($row1 = db_fetch($result1))
	{
			...
			$sqlx = "SELECT account,tran_date FROM 0_gl_trans WHERE type = 70 AND type_no = $trans_no AND account!='570002'";
			$resx= db_query($sqlx);
			$rowx= db_fetch($resx);
			...
			$sql = "DELETE FROM 0_gl_trans WHERE type = 70 AND type_no = $trans_no and tran_date>='2018-01-01'";
			db_query($sql,'failed to delete wrong transfer in GL');
			
			add_gl_trans(70, $trans_no, $tran_date, $gl_from, 0, 0, $memo_, 0);
			add_gl_trans(70, $trans_no, $tran_date, '570002', 0, 0, $memo_,  0);
...
function adjust_gl_entry_not_in_transfer_header($branch)
{
	...
	while($row1 = db_fetch($result1))
	{
			...
			$sqlx = "SELECT aria_trans_no_out FROM transfers.0_transfer_header 
			WHERE aria_trans_no_out='$trans_no' AND br_code_out = '$branch' AND transfer_out_date>='2018-01-01' and transfer_out_date<='2018-12-31'";
			$resx= db_query($sqlx);
			$rowx= db_fetch($resx);
			//display_error($sqlx);
			
			
			$aria_trans_no_out = $rowx['aria_trans_no_out'];
			
			if($aria_trans_no_out==''){
				// display_error($trans_no);
				// display_error($aria_trans_no_out);
				
				$sql = "UPDATE 0_gl_trans SET amount = 0 WHERE type = 70 AND type_no = $trans_no";
...
function adjust_gl_entry_not_in_ms($branch)
{
	...
	while($row1 = db_fetch($result1))
	{
			...
			$sqlx = "SELECT m_id_out FROM transfers.0_transfer_header 
			WHERE aria_trans_no_out='$trans_no' AND br_code_out = '$branch' AND transfer_out_date>='2018-01-01' and transfer_out_date<='2018-12-31'";
			$resx= db_query($sqlx);
			$rowx= db_fetch($resx);
			...
			where (CAST (Movements.TransactionDate  as DATE) >=  '2018-01-01' 
			and  CAST (Movements.TransactionDate  as DATE)<='2018-12-31') 
			and  Movements.status = 2
			and Movements.MovementCode='STO' 
			and Movements.MovementID IN ($m_id_out)
...
function adjust_gl_entry($m_id, $m_no, $nettotal,$branch)
{
	...
	$sql = "SELECT id,cast(transfer_out_date as date) as tran_date FROM transfers.0_transfer_header WHERE m_id_out = $m_id AND br_code_out = '$branch' 
	AND transfer_out_date>='2018-01-01' and m_id_out NOT LIKE '%,%'";
	...
	if ($trans_no == '')
	{
		...
		begin_transaction();
			...
			$sqlx = "SELECT type_no FROM 0_gl_trans WHERE type = 70 AND type_no = $trans_no AND account='570002'";
			$resx= db_query($sqlx);
			...
			if($counter==0){
				...
				add_gl_trans(70, $trans_no, $tran_date, $gl_from, 0, 0, $memo_, $nettotal);
				add_gl_trans(70, $trans_no, $tran_date, '570002', 0, 0, $memo_, -$nettotal);
...
if (isset($_POST['fix_now']))
{
	...
	and CAST (m.PostedDate as Date)>='2018-01-01'
	and CAST (m.PostedDate as Date)<='2018-12-31'
	) x
