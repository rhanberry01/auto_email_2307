...
page(_($help_context = "Delete Old CWO then reimport Receiving"), false, false, "", $js);
set_time_limit(0);
...
function import_new_po($po_no) // for point 1 PO
{
	...
	{
		$ret = import_new_po_item($po_id, $item_row);
...
function import_new_po_item($po_id,$line)
{	
	$DiscountCode1 = 
	$DiscountCode2 = 
	$DiscountCode3 = '';
...
function create_apv_from_rr($rr_id, $advances=0, $type=null)
{
	...
	display_notification("$rr_id imported");
...
function delete_grn_and_po($grn_batch_id, $po)
{
	...
	if (count($grn_batch_id) > 0)
	{
		$sql = "DELETE FROM 0_grn_items WHERE grn_batch_id IN (".implode(',',$grn_batch_id).")";
		db_query($sql,'failed to delete grn_items');
		// display_error($sql);
		
		$sql = "DELETE FROM 0_grn_batch WHERE id IN (".implode(',',$grn_batch_id).")";
		db_query($sql,'failed to delete grn_batch');
	}
	if (count($po) > 0)
	{
		$sql = "DELETE FROM 0_purch_order_details WHERE order_no IN (".implode(',',$po).")";
		db_query($sql,'failed to delete po_details');
		
		$sql = "DELETE FROM 0_purch_orders WHERE order_no IN (".implode(',',$po).")";
		db_query($sql,'failed to delete grn_items');
	}
}

if (isset($_POST['delete_cwo']))
{
	...
	$sql = "SELECT c.* 
			FROM 0_cwo_header a, 0_purch_orders b, 0_grn_batch c
			WHERE TRIM(LEADING '0' FROM c_po_no)= TRIM(LEADING '0' FROM b.reference)
...
if (isset($_POST['ImportAll']) OR isset($_GET['nmhgsdrdntgdsad']))
{
	...
	// if ( $_SESSION["wa_current_user"]->company == 13) // for b silang
	// {	
	...
	{
		if ($rr_row['PurchaseOrderID'] == -1)
			continue;
		

		if ($rr_row['PurchaseOrderID'] != 0)	
		...
		// {
			...
			if ($rr_id > 0 AND !is_array($rr_id)) // rr id
			{
				...
				$sql = "SELECT * FROM ".TB_PREF."cwo_header 
						WHERE TRIM(LEADING '0' FROM c_po_no) = TRIM(LEADING '0' FROM '$po_num') AND voided='0'";
				// display_error($sql);
				$result=db_query($sql);
				$row=db_fetch($result);
				...
				if ($num_row>0)
				{
								...
								{
									create_apv_from_rr($rr_id,$cwo_amount);
								}
				}
				//+++++++++++++++++++++++++++++++++++++++++++++++++INSERTED for cwo	
				else 
...
//===================================================
submit_center('delete_cwo', "<b>DELETE CWO TRANSACTIONS</b>");
