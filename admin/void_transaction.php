...
page(_($help_context = "Void a Transaction"), true, false, "", $js);
...
function exist_transaction($type, $type_no)
{
	...
	switch ($type) 
	{
		...
		case ST_DEPRECIATIONEXPENSE : // it's a depreciation
		case ST_SALESTOTAL : // it's a salestotal
		case ST_CASHDEPOSIT : 
		case ST_CREDITDEBITDEPOSIT :
		case ST_OTHERINCOME :
		case ST_BANKDEPOSIT : // it's  other income
...
function voiding_controls()
{
    ...
    else 
    {
		if ($_POST['filterType'] == ST_JOURNAL) // GL transaction are not included in get_trans_view_str
			{
				$_POST['trans_no'] = get_ids($_POST['filterType'], $_POST['trans_no']);
			}
 		...
 		{
			if ($_POST['filterType'] == ST_SUPPINVOICE){
				$sql = "SELECT * FROM `0_gl_trans` WHERE type = 20 and type_no = '".$_POST['trans_no']."' and account IN(5400,5450)";
				$result = db_query($sql, "Cannot retrieve a supplier transaction");

				if(db_num_rows($result) == 1)
				{
					display_warning(_("For Discrepancy ? <a href='void_transaction_dis.php'>Please click.</a>"));
...
function check_valid_entries()
{
	...
	// }
		...
		// $res = get_gl_trans($_POST['filterType'], $trans_no, true);
		// $rrr = db_fetch($res);
		// $year_of_trans_to_void=$rrr['tran_date'];


		// $split_date = split("-", $year_of_trans_to_void);
		// $year_of_trans_to_void = $split_date[0];
		// //display_error($year_of_trans_to_void);
		
		// $split_date2 = split("-", date2sql(Today()));
		// $current_year = $split_date2[0];
		// //display_error($current_year);
		
		
		// if ($year_of_trans_to_void<$current_year)
		// {
			// display_error(_("The transaction you are trying to void is transaction from year  $year_of_trans_to_void, Voiding transactions from previous year are not allowed."));
			// set_focus('date_');
			// return false;
		// }
