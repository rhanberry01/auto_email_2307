...
function handle_update_item()
{
    if($_POST['UpdateItem'] != "" && check_item_data())
    {
		...
		$_POST['std_cost']=$_POST['costofsales'] * $ms_pos_pack;
    	$_SESSION['adj_items']->update_cart_item($id, input_num('qty'),$_POST['units'], $_POST['std_cost'],$_POST['b_code']);
    }
	line_start_focus();
}
...
function handle_new_item()
{
	...
	$ms_pos_pack = get_adj_qty_multiplier($_POST['units']);
	$_POST['std_cost']=$_POST['costofsales'] * $ms_pos_pack;
	//add_to_order($_SESSION['adj_items'], $_POST['stock_id'], input_num('qty'), $_POST['units'], input_num('std_cost'), $_POST['item_description'],$_POST['barcode']);
	add_to_order($_SESSION['adj_items'], $_POST['stock_id'], input_num('qty'), $_POST['units'], $_POST['std_cost'], $_POST['item_description'],$_POST['barcode']);
	line_start_focus();
}
...
while($row=mysql_fetch_array($res)) {
	...
	add_to_order($_SESSION['adj_items'], $row['stock_id'], $row['qty'], $row['i_uom'],  $row['standard_cost']*$uoms_qty_multiplier,$description='',$row['barcode']);
	}
	line_start_focus();
}
...
adjustment_options_controls($_GET['trans_no']);
end_outer_table(1, false);

//submit_center_first('Update', _("Update"), '', null);
//submit_center_first('Process', _("Process"), '', false);
submit_center('Process', 'Save', "align=center", true, true,'ok.gif');
end_form();
end_page();
