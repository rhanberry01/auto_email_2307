...
function can_process()
{
	...
	elseif (!is_date_in_fiscalyear($_POST['AdjDate'])) 
	{
		...
		if($_SESSION['allownegativecost']==1)
			return true;
		if (!$_POST['Increase']) //$failed_item >= 0
		{
    		...
    		// display_error(_("The adjustment cannot be processed because this may contain item(s) that would cause negative inventory balance"));
				// display_error("<a class='limitLink' onclick='openprompt()' style='cursor:pointer'>Click here to enable customer to transact</a>");	
			display_error("<a class='limitLink' onclick='openprompt()' style='cursor:pointer'>Click here to enable customer to transact</a>");	
...
submit_center_last('Process', _("Process Adjustment"), '', false);
