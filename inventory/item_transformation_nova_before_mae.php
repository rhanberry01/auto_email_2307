...
include_once($path_to_root . "/inventory/includes/item_transformation_ui_nova.inc");
include_once($path_to_root . "/inventory/includes/finished_goods_ui_nova.inc");
...
page(_($help_context = "Movements (Item Transformation)"), false, false, "", $js);
...
ms_db_query("BEGIN TRANSACTION");
	$trans_no = deduct_raw_materials($_SESSION['transform_items']->line_items,
		$_POST['FromStockLocation'],$_POST['ToStockLocation'], $_POST['AdjDate'],$_POST['type'],
		$_POST['ref'], $_POST['memo_']);
	
	$trans_no = add_finished_goods($_SESSION['finished_goods']->line_items,
	$_POST['FromStockLocation'],$_POST['ToStockLocation'], $_POST['AdjDate'],$_POST['type'],
...
function check_item_data()
{
	...
	{
		display_error($_POST['b_code']." -- ".$_POST['item_description']." is currently negative or will turn negative quantity on the system, Kindly inform Audit or ISD Department to proceed scanning of this item.");
		br();
...
if (isset($_GET['NewTransformation']) || !isset($_SESSION['transform_items']) || !isset($_SESSION['finished_goods']))
{
	handle_new_order();
	handle_new_order2();
}
