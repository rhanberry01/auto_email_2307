...
include_once($path_to_root . "/inventory/includes/movements_transfer_out_ui.inc");
include_once($path_to_root . "/inventory/includes/db/items_transfer_out_db.inc");
...
page(_($help_context = "Movements (Transfer-Out Request)"), false, false, "", $js);
...
function handle_new_order()
{
	...
	if (!isset($_SESSION['adj_items']))
	{
		$_SESSION['adj_items'] = new items_cart(ST_TRANSFEROUT);
...
function can_process()
{
	...
	{
		display_error(_("Select Request Date or the entered date for the transfer is invalid."));
		set_focus('AdjDate');
		return false;
	} 
	elseif (!is_date_in_fiscalyear($_POST['AdjDate'])) 
	{
		...
		$failed_item = $adj->check_qoh($_POST['StockLocation'], $_POST['AdjDate']);
...
if (isset($_POST['Process']) && can_process()){
	...
	$trans_no = new_stock_transfer_out($_SESSION['adj_items']->line_items,
	$_POST['StockLocation'],$_POST['StockLocation2'], $_POST['AdjDate'],$_POST['status_type'], $_POST['memo_'],$_POST['get_trans_no']);
...
function check_item_data($units=null,$qtys=null,$pid=null)
{
	if ($_POST['qty']==0)
	{
	...
	$db_133 = get_branch_db_56($_POST['StockLocation']);
	$db_133_2 = get_branch_db_56($_POST['StockLocation2']);
	
	if (!get_branch_stock_id_from_56($db_133, $db_133_2, $_POST['stock_id'],$_POST['item_description']))
	{
		display_error('(from Location) No Global ID found for :  '.$_POST['stock_id']);
		return false;
	}

	#rhan 7.12.18
	$srsvendor = "select ProductID from VENDOR_Products WHERE vendorcode IN('SARSB001','SARSC001','MALFP001') and productid  = '".$pid."'";
	$srsvendors = ms_db_query($srsvendor);			  
	$srsvendorrow = mssql_fetch_array($srsvendors);
	$srsvendorres = $srsvendorrow['ProductID'];


	$uoms_qty_multiplier=out_get_qty_multiplier($units);

	$sellingareabeg = "SELECT ProductCode,SellingArea FROM Products WHERE ProductID = '".$pid."'";
	...
	if(!$srsvendorres){
		...
		if($uoms_qty_multiplier*$qtys > $sellingareaqty){

			if($_SESSION["wa_current_user"]->username == 'lhes'){
				return true;
			}else{
				display_error('Current inventory is less than the requested quantity');
				display_error('Current Inv: '.$sellingareaqty);
				display_error('Requested Qty: '.$uoms_qty_multiplier*$qtys);
				return false;
			}
		}
...
{
	if (!check_item_data($_POST['units'],input_num('qty'),$_POST['stock_id']))
		return;
...
display_adjustment_items(_("Items to Transfer"), $_SESSION['adj_items']);
adjustment_options_controls();
end_outer_table(1, false);

//submit_center_first('Update', _("Update"), '', null);
start_table();
submit_center_first('Process', _("Process Transfer"), '', false);
end_table();
end_form();
end_page();
?>