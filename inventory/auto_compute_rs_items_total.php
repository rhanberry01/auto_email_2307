...
page(_($help_context = "Auto Compute RM Items Total"), false, false, "", $js);
...
function get_rs_items_total($rs_id, $branch_id) {	
	$sql = "SELECT FORMAT(SUM(FORMAT(price*qty, 3)),3) AS items_total FROM centralized_returned_merchandise.".TB_PREF."rms_items
			WHERE rs_id=$rs_id 
...
if (isset($_POST['ImportRS']) OR isset($_GET['sitrololo'])) {
	...
	while ($row = mysql_fetch_array($query)) {
		...
		if (is_null($items_total))
			$items_total = 0;
		// echo $row['rs_id']." - ".$row['branch_id']."-".$items_total."</br>";
