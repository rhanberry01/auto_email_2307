...
//------------------------------------------------------------------------------------------
	...
	$type = ST_SAKUSINAOUT;
$th = array('','Trans#','Trans Date','ProductID','Barcode','Description', 'Cost','UOM', 'QTY', 'Extended');
...
$sql.=" SELECT th.date_created as date_created,th.id as id, th.aria_trans_no_out as trans_out,
(td.cost*td.qty_out) as ext,td.* FROM transfers.".TB_PREF."transfer_details as td left join 
transfers.".TB_PREF."transfer_header as th on th.id=td.transfer_id where th.aria_type_out='$type' ";
...
array_push($th,'', 'Trans#','Trans Date','ProductID','Barcode','Description', 'Cost','UOM', 'QTY', 'Extended');
