...
//------------------------------------------------------------------------------------------
	...
	array_push($th, '','Date Posted', 'Transaction#','Category','Vendor','Product Code','Description','UOM', 'QTY', 'Price', 'Extended');
	...
	$sql = "SELECT ProductID,category,ProductCode, vendor,Description, UOM, sum(qty) as qty, sum(extended) as extended from (SELECT m.[MovementID]
	,m.[MovementNo]
	,m.[MovementCode]
	,m.[DateCreated]
	,m.[Status]
...
$sql .= " ) as a group by ProductID,vendor,category,ProductCode,Description,UOM";

$sql .= " ORDER BY extended DESC";	
	...
	{ 
		...
		$x++;
		$rep->sheet->writeString($rep->y, $x, $row['ProductCode'],$format_left);
		$x++;
