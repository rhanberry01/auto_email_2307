...
page(_($help_context = "Adjustment Inquiry (Centralized)"), false, false, "", $js);

$approve_add = find_submit('selected_id');
$posted_add = find_submit('posted_id');

if ($_SESSION['wa_current_user']->user == "admin") {
	$u = get_id_by_user_id(get_id_by_user_id($_SESSION['wa_current_user']->username));
...
function get_user_by_branch($id, $branch_id) {
	...
	$sql = "SELECT * FROM ".$b['aria_db'].".".TB_PREF."users WHERE id = '".$id."'";
...
function get_user_by_branch_($id, $branch_id) {
	$b = get_branch_by_id($branch_id);

	$sql = "SELECT * FROM ".TB_PREF."users WHERE id = '".$id."'";
...
if(isset($_POST['btn_delete'])){
	// $sql = "UPDATE 0_adjustment_header SET a_status = '0' WHERE a_id = '".$_POST['xheader_id']."' ";
	$sql = "DELETE FROM ".$_POST['dbname'].".".TB_PREF."adjustment_header WHERE a_id = '{$_POST['xheader_id']}' ";
	$result = db_query($sql);
	if($result){
		display_notification_centered('successfully deleted');
	}else{
		display_error($sql);
	}
}
...
array_push($th, 'Branch', 'Date Created', 'TransNo','MovementNo','Movement Type', 'Created By', 'Date Posted', 'Approved By','Posted By', 'Status','Extended','','', 'Operations '.'<input type="checkbox" id="post_all">');
...
foreach ($all_branch as $key => $row) {
	...
	$user_approve = get_user_by_branch_($row['a_temp_posted_by'], $row['branch_id']);
	...
	if ($row['a_status'] == 1) {
		echo '<form method="post">
				<input type="hidden" name="xheader_id" value="'.$row['a_id'].'">			
				<input type="hidden" name="dbname" value="'.$row['dbname'].'">
				<button type="submit" name="btn_delete" id="btn'.$row['a_id'].$b++.'" style="display: none;"></button>			
			</form>';
		label_cell('<a href="#" onclick="ConfirmDelete('.$row['a_id'].$c++.')" id="">delete</a>','Delete','');
	}else{
		label_cell('','','');
	}
	//end jade
	...
	if ($row['a_status'] == 1) {
		...
		// 	if (is_operations($_SESSION['wa_current_user']->user)) {
		...
		// else
		if ($row['a_temp_posted'] == 0) {
			if ($approver == '0238' || $approver == 'cezz' || $_SESSION['wa_current_user']->user == 1 || $approver == 'theresa') {
