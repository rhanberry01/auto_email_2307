...
function update_posted_rms_header($rs_id,$approver_comments)
{
	...
	$sql = "UPDATE returned_merchandise.".TB_PREF."rms_header SET 
			...
			WHERE rs_id ='$rs_id'";
	db_query_rs($sql,'failed to update movement (FDFB)');
}
				...
				$UOM,$unitcost,$qty,$pack,$barcode)
{
	...
	ms_db_query($sql,"error inserting movement line");
}
...
function process_ms_movement($rs_id, $MovementCode, $remarks='')
{
	...
	$sql = "INSERT INTO Movements (MovementNo,
			...
			VALUES ('$movement_no','$MovementCode','','','',$ToDescription,$ToAddress,$ContactPerson,
				...
				$_SESSION['wa_current_user']->ms_user_id.",'".Today()." 00:00:00',2,".
...
global $Ajax;
		...
		if($check == 0){
			...
			update_approved_rms_header($approve_add,$movement_no,$approver_comments);

			display_notification("Disposal #".$approve_add." is successfully approved.");
		}else{
				display_error("Cannot processed barcode(s) ".implode(',',$check)." in Trans. # ".$approve_add." Kindly check if it is currently negative or will turn negative quantity on the system.");		
		}
			$Ajax->activate('dm_list');
}
...
$th =array('#', 'SA to BO Date', 'Supplier','Extended','Created by','Processed by','Remarks', 'Status','Comment','Operations','Audit');
...
while($row = db_fetch($res))
{
	...
	if ($row['approved']==0 and ($approver=='juliet' or $approver=='admin')) {
	...
	if ($row['temp_posted']==0 and ($approver=='juliet' or $approver=='admin' or $approver=='cezz' or $approver=='0238')) {
	...
	if ($row['temp_posted']==1 and  $row['approved']==0 and ($approver=='juliet' or $approver=='admin')) {
	$selected='selected_id'.$row['rs_id'];
	submit_cells($selected, _("Approve Disposal"), "colspan=1",_('Approve Disposal'), true, ICON_ADD);
	}
	else if ($row['temp_posted']==0 and  $row['approved']==0 and ($approver=='juliet' or $approver=='admin')) {
	label_cell('For Approval');
	}
	else {
	label_cell('Approved');
