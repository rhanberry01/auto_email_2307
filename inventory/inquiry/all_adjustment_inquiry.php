...
div_start('dm_list');
if (!isset($_POST['search']) or $approve_add != -1)
	display_footer_exit();
...
array_push($th, 'Date Created', 'TransNo','MovementID','Movement Type', 'Created By', 'Date Posted', 'Posted By', 'Status','','','');
...
while($row = db_fetch($res))
{
	...
	if (($row['a_status']==1 and ($approver=='juliet' or $approver=='jelyn' or $approver=='belle' or $approver=='admin')) 
		...
		OR ($row['a_status']==1 AND  ($approver=='0238' or $approver=='cezz') AND ($row['a_movement_code'] == 'IGNSA' or $row['a_movement_code'] == 'IGSA' ))
