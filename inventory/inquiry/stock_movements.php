...
$th = array(_("Type"), _("#"),  _("Date"), _("Detail"),
	_("Quantity In"), _("Quantity Out"), _("Quantity On Hand"),'Report UOM');
//_("Reference"),
table_header($th);

$sql = "SELECT SUM(qty_pcs) FROM ".TB_PREF."stock_moves WHERE stock_id=".db_escape($_POST['stock_id']) . "
...
$xxx = get_item_report_uom_and_multiplier($_POST['stock_id']);
$report_uom = $xxx[0];
$base_multiplier = $xxx[1];

start_row("class='inquirybg'");
label_cell("<b>"._("Quantity on hand before") . " " . $_POST['AfterDate']."</b>", "align=center colspan=4");
label_cell("&nbsp;", "colspan=2");
$dec = get_qty_dec($_POST['stock_id']);
qty_cell($before_qty, false, $dec);
label_cell('<b>'.$report_uom.'</b>');
end_row();
...
while ($myrow = db_fetch($result))
{
	...
	if ($myrow["qty"] > 0)
	{
		...
		$total_in += $myrow["qty_pcs"];
	}
	else
	{
		$quantity_formatted = number_format2(-$myrow["qty"], $dec);
		$total_out += -$myrow["qty_pcs"];
	}
	$after_qty += $myrow["qty_pcs"];

	if ($myrow["reference"] != 'auto' AND $myrow["type"] != 13)
	{
		label_cell($type_name);	
		// label_cell(get_trans_view_str($myrow["type"], $myrow["trans_no"], $myrow["trans_no"],true));
		label_cell(get_trans_view_str($myrow["type"], $myrow["trans_no"], $myrow["reference"]));
	}
	else if ($myrow["reference"] != 'auto' AND $myrow["type"] == 13)
	{
		label_cell($systypes_array[$myrow["type"]]);
		// label_cell(get_trans_view_str($t_type, $t_trans_no, $t_trans_no,true));
		label_cell(get_trans_view_str($myrow["type"], $myrow["trans_no"], $myrow["reference"]));
	}
	else // auto DR
	{
		$sql = 'SELECT trans_link 
					...
					WHERE trans_no=' .$myrow["trans_no"]. ' AND type='.$myrow["type"].' AND trans_link!=0';

		$res = db_query($sql,'failed to get invoice of DR');
		$row  = db_fetch($res);
		
		$t_type = 10;
		$t_trans_no =  $row[0];
		
		label_cell($systypes_array[$t_type]);
		// label_cell(get_trans_view_str($t_type, $t_trans_no, $t_trans_no,true));
		label_cell(get_trans_view_str($t_type, $t_trans_no, get_reference($t_type, $t_trans_no)));
	}
	...
	label_cell((($myrow["qty"] >= 0) ? $quantity_formatted.' '.$myrow['i_uom'] : ""), "nowrap align=right");
	label_cell((($myrow["qty"] < 0) ? $quantity_formatted.' '.$myrow['i_uom'] : ""), "nowrap align=right");
	qty_cell($after_qty/$base_multiplier, false, $dec);
	label_cell('<b>'.$report_uom.'</b>');
	end_row();
	$j++;
	If ($j == 12)
...
start_row("class='inquirybg'");
label_cell("<b>"._("Quantity on hand after") . " " . $_POST['BeforeDate']."</b>", "align=center colspan=4");
qty_cell($total_in/$base_multiplier, false, $dec);
qty_cell($total_out/$base_multiplier, false, $dec);
qty_cell($after_qty/$base_multiplier, false, $dec);
label_cell('<b>'.$report_uom.'</b>');
end_row();
