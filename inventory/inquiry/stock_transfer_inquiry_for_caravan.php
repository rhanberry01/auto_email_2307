...
{
	global $Ajax;
	$sql = "UPDATE transfers_caravan.0_transfer_header SET inactive = 1,cancelled = 1 WHERE id = ". $delete;
...
if($from_br_code !='srscain2'){
	...
	display_error('PLEASE LOGIN TO CAINTA2');
...
$sql = "SELECT * FROM transfers_caravan.0_transfer_header
...
while($row = db_fetch($res))
{
	...
	$prev_str =  "<a target='_blank' href='$path_to_root/inventory/view/view_transfer_2_for_caravan.php?transfer_id=". $row['id'] ."$actual' 
				onclick=\"javascript:openWindow(this.href,this.target); return false;\">".$row['id'] ."</a>";
				
	$pick_list_str =  "<a target='_blank' href='$path_to_root/reporting/transfer_pick_list_for_caravan.php?transfer_id=".  $row['id'] ."$actual' 
				onclick=\"javascript:openWindow(this.href,this.target); return false;\">".'Print Packing List' ."</a>";
	
	$printout_str = '';
	if ($row['transfer_out_date'])	
		$printout_str =  "<a target='_blank' href='$path_to_root/reporting/transfer_printout_for_caravan.php?transfer_id=".  $row['id'] ."$actual' 
