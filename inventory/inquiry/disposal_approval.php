...
function process_ms_movement($rs_id, $MovementCode, $remarks='')
{
	...
	$sql = "INSERT INTO Movements (MovementNo,
			...
			VALUES ('$movement_no','$MovementCode','','','',$ToDescription,$ToAddress,$ContactPerson,
				...
				. ($net_total+0) .",'POSTED',".($total_qty+0).",".$_SESSION['wa_current_user']->user.",'$remarks',
...
$th = array();
		 array_push($th,'#', 'SA to BO Date', 'Supplier','Extended','Created by','Processed by','Remarks', 'Status','Comment','');
...
while($row = db_fetch($res))
{
	...
	if ($row['approved']==0 and $approver=='juliet') {
	...
	submit_cells($selected, _("Approve Disposal"), "colspan=2",_('Approve Disposal'), true, ICON_ADD);
	}
	else {
	label_cell('');
	}
