...
while ($bRow = db_fetch($bQuery)) {
	...
	if ($_POST['trans_no'])
	{
	$sql .= " AND a.a_trans_no LIKE '%". $_POST['trans_no'] . "%'";	
...
array_push($th, 'Branch', 'Date Created', 'TransNo','MovementNo','Movement Type', 'Created By', 'Date Posted', 'Approved By','Posted By', 'Status','Extended','', 'Operations '.'<input type="checkbox" id="post_all">', 'GM '.'<input type="checkbox" id="approve_all">');
...
foreach ($all_branch as $key => $row) {
	...
	if ($row['a_status'] == 1) {
		...
		if ($row['a_temp_posted'] == 0) {
			...
			if ($_SESSION['wa_current_user']->user == 'admin' || $approver == '6666' || $_SESSION['wa_current_user']->user == '606' || $_SESSION['wa_current_user']->username == 'jenc') {
