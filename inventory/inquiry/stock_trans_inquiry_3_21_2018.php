...
array_push($th, '','Date Created', 'Transfer #','MovementOut #','From Branch','Date Dispatch','Cost Out','MovementIn #', 'To Branch', 'Date In', 'Cost In','diff');
	...
	$mtype=get_selected_movement_type($_POST['movement_type']);
		...
		$sql = "SELECT th.*, sum(cost*actual_qty_out) as cost_out,sum(cost*qty_in) as cost_in FROM transfers.0_transfer_header as th
...
//	label_cell($row['cost']);
	...
	$diff_ =  $row['cost_out'] - $row['cost_in'];
	label_cell($diff_);
