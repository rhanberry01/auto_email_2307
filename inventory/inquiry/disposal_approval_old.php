...
function update_approved_rms_header($rs_id,$movement_no)
{
	...
	$sql = "UPDATE returned_merchandise.".TB_PREF."rms_header SET 
			...
			WHERE rs_id ='$rs_id'";
	db_query($sql,'failed to update movement (FDFB)');
}
...
update_approved_rms_header($approve_add,$movement_no);
...
$th = array();
		 array_push($th,'#', 'SA to BO Date', 'Supplier','Extended','Created by','Processed by','Remarks', 'Status','');
