...
if(isset($_POST['btn_delete'])){
	...
	$sql = "DELETE FROM 0_adjustment_header WHERE a_id = '".$_POST['xheader_id']."' ";
	$result = db_query($sql);
	if($result){
		display_notification_centered('Successfully deleted');
	}else{
		display_error('failed query');
	}
}
//end 3/5/2018
...
$bSql = "SELECT * FROM transfers.0_branches LIMIT 3 ";
...
foreach ($all_branch as $key => $row) {
	...
	//jade added 3/5/2018
	if ($row['a_status'] == 1) {
		label_cell('<a href="#" onclick="confirmMessage('.$row['a_id'].')">delete</a>','Delete','');
	}else{
		label_cell('','','');
	}
	//end jade
	...
	if ($row['a_status'] == 1) {
		...
		// 	if (is_operations($_SESSION['wa_current_user']->user) || $_SESSION['wa_current_user']->user == 'admin' || $approver == '6666' || $_SESSION['wa_current_user']->user == '606' || $_SESSION['wa_current_user']->username == 'jenc' || $_SESSION['wa_current_user']->username == 'marianne' || $_SESSION['wa_current_user']->username == 'admin' || $_SESSION['wa_current_user']->username == 'd0314') {
		...
		if ($row['a_temp_posted'] == 0 && is_operations($_SESSION['wa_current_user']->user)) {
			if (is_operations($_SESSION['wa_current_user']->user)) {
				hidden('a_movement_code'.$row["a_trans_no"], $row['a_movement_code']);
				echo '<td align="center">
					<input type="hidden" name="bid_'.$row["a_trans_no"].'" value="'.$row['branch_id'].'" />
...
<!-- jade added 3/5/2018 -->
<form method="post">
	<input type="hidden" value="" id="xheader_id" name="xheader_id">
	<button type="submit" name="btn_delete" id="btn_delete" style="display: none;"></button>
</form>
