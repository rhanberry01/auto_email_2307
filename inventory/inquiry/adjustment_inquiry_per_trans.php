...
page("Ajustment Summary", false, false, "", $js);
...
$dateto=date2sql($_POST['TransToDate']);
    ...
    $rep = new FrontReport(_('Ajustment Summary'), "adjustment_summary", "LETTER");
	...
	$rep->y ++;
	$rep->sheet->writeString($rep->y, 0, 'ADJUSMENT SUMMARY', $format_bold);
	$rep->y ++;
...
//------------------------------------------------------------------------------------------
	...
	array_push($th, '','Date Posted', 'Transaction#','Category','Vendor','Product Code','Description','UOM', 'QTY', 'Price', 'Extended','Remarks');
	...
	$mtype=get_selected_movement_type_report($_POST['movement_type']);
	...
	$adj_row = get_adjustment_header($row['MovementCode'],$row['MovementID']);
	$comment=get_comments_string(17, $adj_row['a_trans_no']);
	
		if($comment!=''){
		$rep->sheet->writeString($rep->y, $x, $comment,$format_left);
		$x++;
		}
		else{
		$rep->sheet->writeString($rep->y, $x, $row['Remarks'],$format_left);
		$x++;
		}
...
//end of excel report------------------------------------------------------------------------------------
		...
		function get_selected_movement_type_report($type)
		{
			$sql_select_all_input="select movement_code from ".TB_PREF."movement_types_report where id='$type'";
			//display_error($sql_select_all_input);
			$res=db_query($sql_select_all_input);
			$row = db_fetch($res);
			$movement_code=$row['movement_code'];
...
start_table();
start_row();
adjustment_types_list_row_(("Movement Type:"), 'movement_type', $row['id']);
end_row();
end_table();
...
if (mssql_num_rows($res) > 0){
	...
	br();
	table_header($th);
	display_heading("Ajustment Summary From ".$_POST['start_date']." To ".$_POST['end_date']);
	br();
}
