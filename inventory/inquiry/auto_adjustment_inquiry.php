...
$dateto=date2sql($_POST['TransToDate']);
    ...
    $rep = new FrontReport(_('Auto Ajustment Summary'), "auto_adjustment_summary", "LETTER");
	...
	$rep->y ++;
	$rep->sheet->writeString($rep->y, 0, 'AUTO ADJUSMENT SUMMARY', $format_bold);
	$rep->y ++;
...
//------------------------------------------------------------------------------------------
	...
	{ 
		...
		$x++;
				// $rep->sheet->writeNumber($rep->y, $x, number_format2(abs($row['unitcost']),3), $format_accounting);
		// $x++;
		// $rep->sheet->writeNumber($rep->y, $x, number_format2(abs($row['extended']),2), $format_accounting);
		// $x++;
...
if (mssql_num_rows($res) > 0){
	...
	br();
	table_header($th);
	display_heading("Auto Ajustment Summary From ".$_POST['start_date']." To ".$_POST['end_date']);
	br();
}
