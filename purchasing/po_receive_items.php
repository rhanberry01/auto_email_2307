...
if (isset($_GET['AddedID']))
{
	...
	//hyperlink_params("$path_to_root/reporting/rep211.php", _("Print this"), "id=$_GET[AddedID]");
	
	//echo "<p>";
	
	//display_note(get_trans_view_str($trans_type, $grn, _("&View this Delivery")));

	hyperlink_params("$path_to_root/purchasing/supplier_invoice.php", _("Entry AP Voucher for this receival"), "New=1");
...
/*	echo "
		<p>
		<center>
		<a href='../sales/customer_del_so.php?OrderNumber=$grn&type=5&view=0&KeepThis=true&TB_iframe=true&height=280&width=300' class='thickbox'>Void This Receiving Note</a>
		</center>
	";*/
...
if ((!isset($_GET['PONumber']) || $_GET['PONumber'] == 0) && !isset($_SESSION['PO']) && !isset($_SESSION['BatchRR']))
{
...
function display_po_receive_items()
{
	...
	if(empty($_SESSION['BatchRR']))
    $th = array(_("Item Code"), _("Description"), _("Ordered"), _("Units"), _("Received"),_("Outstanding"), _("This Delivery"), _("Notes"));
	else
    $th = array(_("Order No."),_("Item Code"), _("Description"), _("Ordered"), _("Units"), _("Received"),_("Outstanding"), _("This Delivery"), _("Notes"));
    table_header($th);
    ...
    {
       	foreach ($_SESSION['PO']->line_items as $key=>$ln_itm)
       	{
    		...
    		$total += $line_total;
				if(!empty($_SESSION['BatchRR'])){				
					label_cell(get_trans_view_str(ST_PURCHORDER,$ln_itm->or_no));
					$cols=9;}else {$cols=8;}
			...
			text_cells('',$ln_itm->line_no.'rnotes', null, 20);
 ...
 //   label_row(_("Total value of items received"), $display_total, "colspan=8 align=right", "nowrap align=right");
	...
	$sql = "SELECT tax_group_id FROM ".TB_PREF."suppliers WHERE supplier_id=".db_escape($_SESSION['PO']->supplier_id);

	$result = db_query($sql, "could not get supplier");

	$row = db_fetch_row($result);

	$taxes = $_SESSION['PO']->get_taxes_for_receive($row[0]);
	
	//$tax_total = display_edit_tax_items($taxes, 8, $vat_inc);

	$display_total = price_format(($total + input_num('freight_cost') + $tax_total));
	
   // label_row(_("Total"), $display_total, "colspan=8 align=right", "nowrap align=right");
	
    end_table();
	echo "<br>";
	start_table("$table_style width=40%");
		textarea_row(_("Comments:"), "Comments", $order->Comments, 50, 5);
	end_table();
...
function process_receive_po()
{
	...
	if(empty($_SESSION['BatchRR']))
	$grn = add_grn($_SESSION['PO'], $_POST['DefaultReceivedDate'],$_POST['ref'], $_POST['Location'], $_POST['Comments']);
	else
	$grn = add_grn2($_SESSION['PO'], $_POST['DefaultReceivedDate'],$_POST['ref'], $_POST['Location'], $_POST['Comments']);
...
if ((isset($_GET['PONumber']) && $_GET['PONumber'] > 0 && !isset($_POST['Update']))||isset($_SESSION['BatchRR']))
{

	create_new_po();

	/*read in all the selected order into the Items cart  */
	if(isset($_SESSION['BatchRR']))
	read_po2($_SESSION['BatchRR'], $_SESSION['PO']);
	else 
	read_po($_GET['PONumber'], $_SESSION['PO']);
	//var_dump($_SESSION['BatchRR']);
...
if (isset($_POST['Update']) || isset($_POST['ProcessGoodsReceived']))
{
	...
	foreach ($_SESSION['PO']->line_items as $line)
	{
	 if( ($line->quantity - $line->qty_received)>0) {
		...
		$_SESSION['PO']->line_items[$line->line_no]->rnotes = $_POST[$line->line_no."rnotes"];
