...
// display_error($sql);
	...
	while($row = db_fetch($res))
	{
		...
		alt_table_row_color($k);
			...
			label_cell($cv_no_);
			if(get_po_attachment($row['po_number'],$row['reference']))
				label_cell("<a target=blank href='".get_po_attachment($row['po_number'],str_pad($row['reference'], 10, "0", STR_PAD_LEFT))."'onclick=\"javascript:openWindow(href,target); return false;\">".
