...
date_cells(_("from:"), 'OrdersAfterDate',null,null,-1);
date_cells(_("to:"), 'OrdersToDate',null,null,-1);
yesno_list_cells('Hide CV', 'hide_cv');

submit_cells('Search', _("Search"),'',_('Select documents'), false);
end_row();
end_table();

div_start('orders_tbl');

if (!isset($_POST['Search']) AND $_POST['order_number']== '')
{
	br();
	display_heading('click Search to view results');
	display_footer_exit();
}
...
// display_error($sql);
	...
	$th = array(_("Receiving #"),_("PO #"),'CV #', 'Attachment', _("Supplier"), 'Invoice #', _("Delivery Date"), 'RR Total' , 'PO Total');
	table_header($th);
	$this_branch = $db_connections[$_SESSION["wa_current_user"]->company]["br_code"];	

	$k = 0;
	$total_po_total = $total_total = $count = 0;
	while($row = db_fetch($res))
	{
		$count ++;
		...
		$real_po_total = get_real_po_total($row['purch_order_no']);
		
		$total_total += $rr_po_total;
		$total_po_total += $real_po_total;
		...
		alt_table_row_color($k);
			...
			label_cell($cv_no_);
				...
				$sql_ = "SELECT scanned_folder FROM transfers.0_branches where code = '".$this_branch."'";
				$res_ = db_query($sql_);
				$row_ = db_fetch($res_);
				$scanned_folder = $row_[0];

			if(get_po_attachment($row['po_number'],$row['reference'],$scanned_folder))

					label_cell("<a target=blank href='".get_po_attachment($row['po_number'],str_pad($row['reference'], 10, "0", STR_PAD_LEFT),$scanned_folder)."'onclick=\"javascript:openWindow(href,target); return false;\">".
								_("View Invoice") . "&nbsp;</a> ", 'align=center');
			else
			label_cell('');
			...
			amount_cell($real_po_total);
		end_row();
	}
	alt_table_row_color($k);
		label_cell('TOTAL: ','colspan=7 align=right');
		amount_cell($total_total,true);
		amount_cell($total_po_total,true);
	end_row();
...
function get_real_po_total($purch_order_no)
{
	$sql = "SELECT SUM(extended) 
			FROM `0_purch_order_details` 
			WHERE `order_no` = $purch_order_no ";
	$res = db_query($sql);
	$row = db_fetch($res);
	return $row[0];
}
