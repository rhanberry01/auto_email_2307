...
if ($_POST['_discount_changed'])
{
	...
	else
	{
		...
		$_SESSION['s_alloc']->discount = input_num('discount');
		$_POST['amount'] = number_format2(input_num('limiter'),2);
...
if ($all_id != -1)
{
	...
	else
	{
		...
		{
			$_SESSION['s_alloc']->allocs[$all_id]->current_allocated = 0;
			$max_alloc = $_SESSION['s_alloc']->allocs[$all_id]->amount - $_SESSION['s_alloc']->allocs[$all_id]->amount_allocated;
			$left_to_allocate = -$_SESSION['s_alloc']->amount + $_SESSION['s_alloc']->ewt+ $_SESSION['s_alloc']->discount - 
					$_SESSION['s_alloc']->get_total_allocations();
			
			if ($max_alloc <= $left_to_allocate)
			{
				$_SESSION['s_alloc']->allocs[$all_id]->current_allocated = $max_alloc;
				$_POST['amount'.$all_id] = number_format2($_SESSION['s_alloc']->allocs[$all_id]->current_allocated,2);
			}
			else
			{
				$_SESSION['s_alloc']->allocs[$all_id]->current_allocated = $left_to_allocate;
				$_POST['amount'.$all_id] = number_format2($_SESSION['s_alloc']->allocs[$all_id]->current_allocated,2);
			}
			...
			$left_to_allocate = -$_SESSION['s_alloc']->amount + $_SESSION['s_alloc']->ewt+ $_SESSION['s_alloc']->discount - $_SESSION['s_alloc']->get_total_allocations();
			
			if ($max_alloc == $_SESSION['s_alloc']->allocs[$all_id]->current_allocated)
				break;
		}
	}
	...
	if (input_num('limiter') == 0)
	{
		...
		if (count($_SESSION['s_checks']) > 0)
		{
			...
			if ($_SESSION['s_alloc']->get_total_allocations()-$_SESSION['s_alloc']->allocs[$none_id]->current_allocated < $_SESSION['s_checks']->amount)
			{
				...
				$_SESSION['s_checks']->ewt = 0;
			}
		}
		else
			$_POST['c_amt'] = $_POST['amount'];

		// $_POST['ewt_percent'.$none_id] =  0;
		$Ajax->activate('amount');
		$Ajax->activate('ewt');
	}
	else
...
if ($key2 != -1)
{
	if (($_POST['_ewt_percent'.$key2.'_changed']) AND $_SESSION['s_alloc']->allocs[$key2]->ewt != input_num('ewt_percent'.$key2))
	{
	...
	if (($_POST['_amount'.$key.'_changed']))
	{
		...
		{	
			...
			if (input_num('amount'.$key) != $_SESSION['s_alloc']->allocs[$key]->current_allocated)
			{
				...
				$_SESSION['s_alloc']->update_ewt_using_item();
				$_POST['ewt'] = number_format2($_SESSION['s_alloc']->ewt,2);
				$_POST['discount'] = number_format2($_SESSION['s_alloc']->discount,2);
				...
				// {
					...
					// $_POST['c_amt'] = number_format2($_SESSION['s_alloc']->get_total_allocations() - $_SESSION['s_alloc']->ewt ,2);
				// }
...
function check_inputs()
{
	...
	if (input_num('limiter') > 0)
		$_POST['amount'] = -$_SESSION['s_alloc']->amount+$_SESSION['s_alloc']->ewt+$_SESSION['s_alloc']->discount;
	else
	...
	if (round2($_SESSION['s_checks']->amount+$_SESSION['s_alloc']->ewt+$_SESSION['s_alloc']->discount,2) != round(input_num('amount'),2) )
	{
		display_error(_("Amount of Payment is not equal to Check Total Amount") . ' -> '.round($_SESSION['s_checks']->amount+
		$_SESSION['s_alloc']->ewt+
		$_SESSION['s_alloc']->discount,2).
		' -> '.round(input_num('amount'),2));
		return false;
	}	
		return true;
}
...
function handle_add_payment()
{
	...
	foreach($_SESSION['s_checks']->checks as $key=>$check_item)	// Now debit creditors account with payment + discount
	{
		...
		{
			add_check2($payment_id, ST_SUPPAYMENT, $check_item->check_bank, '', $check_item->check_number, $check_item->check_date,-($check_item->check_amount), $id);		
		
			issue_check_number2($payment_id, $check_item->check_number, ST_SUPPAYMENT, $id);
		}
	}
...
// $_SESSION['s_checks']->checks[$_POST['LineNo']]->check_amount))));
	...
	}if (input_num('c_amt') <= 0)
	{
		...
		set_focus('c_amt');
		$error = true;
	}if (round(($_SESSION['s_alloc']->get_total_allocations() - $_SESSION['s_alloc']->ewt - ($_SESSION['s_checks']->amount + input_num('c_amt'))),2) < 0 AND round(input_num('limiter'),2) == 0)
	{
	...
	if (!$error)
	{
		...
		if ($chkchk === null)
		{
			...
			if (input_num('limiter') == 0)
				$_POST['c_amt'] = number_format2($_SESSION['s_alloc']->get_total_allocations() - $_SESSION['s_alloc']->ewt - 
					$_SESSION['s_alloc']->discount - $_SESSION['s_checks']->amount,2);
			else
...
function handle_update_item()
{
	...
	if (($_SESSION['s_alloc']->get_total_allocations() - $_SESSION['s_alloc']->ewt - $_SESSION['s_alloc']->discount - ($_SESSION['s_checks']->amount + input_num('c_amt') - 
	...
	if (!$error)
	{
		...
		if ($chkchk === null)
		{
			...
			//$Ajax->activate('amount');
			if (input_num('limiter') == 0)
				$_POST['c_amt'] = number_format2($_SESSION['s_alloc']->get_total_allocations() - $_SESSION['s_alloc']->ewt - $_SESSION['s_alloc']->discount 
					- $_SESSION['s_checks']->amount,2);
			else
				$_POST['c_amt'] = number_format2(input_num('limiter') - $_SESSION['s_checks']->amount,2);
...
start_form();
	...
	{	// first page call
		...
		if (isset($_POST['_limiter_changed']))
		{
			...
			$_POST['amount'] = $_POST['limiter'];
			$_SESSION['s_alloc']->ewt = $_SESSION['s_alloc']->discount = $_POST['discount'] = $_POST['ewt'] = 0;
			$Ajax->activate('amount');
			$Ajax->activate('discount');
			$Ajax->activate('ewt');
		}
	...
	check_accounts_list_row(_("From Account:<font color=red>*</font>"), 'bank_account', null, true, '');
    ...
    ref_row(_("CV No.:<font color=red>*</font>"), 'ref', '', $Refs->get_next(ST_SUPPAYMENT));
