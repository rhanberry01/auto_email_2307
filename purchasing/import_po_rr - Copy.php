...
function import_rr_items($rr,$supp_id)
{
	...
	while($row = mssql_fetch_array($res))
	{
		...
		$po_detail_item_row = get_po_item_id($rr['PurchaseOrderID'], $row['ProductID']);
		...
		$vendor_code = $supp_id; //get_supplier_id_by_po_id($rr['PurchaseOrderID']);
...
function import_rr($rr)
{
	...
	$supp_id = import_rr_header($rr);
	
	$good = import_rr_items($rr,$supp_id);
	...
	add_audit_trail(ST_SUPPRECEIVE, $r_id, mssql2date($rr['DateReceived']),'import RR');
	
	return $good;
}

function import_po($po_id)
{
	begin_transaction();
	$sql = "SELECT *
			FROM PurchaseOrder 
...
if (isset($_POST['ImportAll']) OR isset($_GET['nmhgsdrdntgdsad']))
{
	...
	// get all RR with trandate >= startdate
	...
	{
		$no_import_errors = import_po($rr_row['PurchaseOrderID']);
	
		// if ($no_import_errors === true) // no error
		// {
			$rr_good = '';
			
			if (!check_rr($rr_row['ReceivingID']))
			{
				continue;
			}
			if (!po_imported_already($rr_row['PurchaseOrderID']))
			{
