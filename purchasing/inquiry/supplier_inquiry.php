...
function print_2307($row)
{
	// return $row['type'] == ST_SUPPINVOICE && $row["TotalAmount"] - $row["Allocated"] > 0 ?
		// pager_link(_("Purchase Return"),
			...
			// : '';
	global $path_to_root;
	return "<a target=blank href='$path_to_root/gl/2307_pdf_per_trans.php?
				type=".($row['type'])."&trans_no=".($row['trans_no'])."&supplier_id=".($row['supplier_id'])." 'onclick=\"javascript:openWindow(this.href,this.target); return false;\"><b>" .
				"Print". "&nbsp;</b></a> ";
}
...
//------------------------------------------------------------------------------------------------
    ...
    $sql = "SELECT trans.type, 
		...
		trans.reference,
		cv_no, trans.cv_id,  trans.non_trade , trans.supplier_id
