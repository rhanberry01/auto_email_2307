...
if (isset($_POST['export'])){
		...
		echo"<tr>
		   ...
		   echo"<td colspan='5' align ='left'>".number_format($amount1,2)."</td>";
...
if (isset($_POST['export1'])){
		...
		$sql = "SELECT * from transfers.0_branches_other_income where id not in(11,23) order by id asc";
		$result=db_query($sql);
		echo"<table  width= '90%' align = 'center' border ='1'>
				<tr>";
				echo"<td align = 'center'>SAF #</td>
				<td align = 'center'>Supplier Name</td>
				<td align = 'center'>Purchaser Name</td>
			...
			{
				echo"<td colspan='1'  align = 'center'>".$row['name']."</td>";
				$branch_count++;
			}	
		echo "<td align = 'center'>Total</td></tr>";
			...
			$sql = "SELECT * from transfers.0_branches_other_income where id not in(11,23) order by id asc";
			$result=db_query($sql);
			while ($branches_res = db_fetch($result))
			{
				$sql = "SELECT a.*, b.supp_name, c.type_name FROM ".$branches_res['aria_db'].".".TB_PREF."sdma a, ".$branches_res['aria_db'].".".TB_PREF."suppliers b,
						".$branches_res['aria_db'].".".TB_PREF."sdma_type c
					WHERE a.supplier_id = b.supplier_id 
				...
				while ($row = db_fetch_assoc($res)) {
					...
					$sdmi[] = "('".$branches_res['name']."',
					'".$row['reference']."',
					'".$row['supp_name']."',
					'".strtoupper(get_display_username_by_id_and_branch_($row['prepared_by']))."',
					'".$row['date_created']."',
					'".$row['type_name']."',
					'".$effectivity."',
					".$row['amount'].",
					...
					$insert_sql = "INSERT INTO srs_aria_nova.0_sdma_reports (branch, reference, supplierName, purchaserName,date_created,  type, effectivity, amount, discount) VALUES ".implode(',',$sdmi);
					db_query($insert_sql);
					//echo "<tr>";
					//echo "<td>".$insert_sql."</td>";
					//echo "</tr>";
					$sdmi = array();
					
				}
			}
			$sql1 = "SELECT a.reference,a.supplierName, a.purchaserName, a.date_created, a.type, a.effectivity, 
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'NOVALICHES') as NOVALICHES,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'IMUS') as IMUS,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'NAVOTAS') as NAVOTAS,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'TONDO') as TONDO,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'CAMARIN') as CAMARIN,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'GEMS (QUEZON)') as GEMS,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'EASTMART (MANALO)') as EASTMART,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'MALABON') as MALABON,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'GAGALANGIN') as GAGALANGIN,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'CAINTA') as CAINTA,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'VALENZUELA') as VALENZUELA,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'BAGONG SILANG') as BAGONGSILANG,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'PUNTURIN') as PUNTURIN,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'PATEROS') as PATEROS,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'COMEMBO') as COMEMBO,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'CAINTA 2 (SAN JUAN)') as CAINTA2,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'SAN PEDRO') as SANPEDRO,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'LAS PINAS') as LASPINAS,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'ALAMINOS') as ALAMINOS,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'BAGUMBONG') as BAGUMBONG,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'GRACEVILLE') as GRACEVILLE,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'MOLINO') as MOLINO,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'MANGGAHAN') as MANGGAHAN,
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'MONTALBAN') as MONTALBAN,
((SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'NOVALICHES') + (SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'IMUS')
+ (SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'NAVOTAS') + (SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'TONDO') 
+ (SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'CAMARIN') + (SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'GEMS (QUEZON)')
+ (SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'EASTMART (MANALO)') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'MALABON') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'GAGALANGIN') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'CAINTA') + 
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'VALENZUELA') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'BAGONG SILANG') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'PUNTURIN') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'PATEROS') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'COMEMBO') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'CAINTA 2 (SAN JUAN)') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'SAN PEDRO') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'LAS PINAS') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'ALAMINOS') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'BAGUMBONG') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'GRACEVILLE') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'MOLINO') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'MANGGAHAN') +
(SELECT IFNULL(sum(amount),0) FROM srs_aria_nova.0_sdma_reports WHERE reference = a.reference AND branch = 'MONTALBAN')) as total
 FROM srs_aria_nova.0_sdma_reports as a 
GROUP BY a.reference, a.supplierName, a.date_created, a.type, a.effectivity  ORDER BY reference";
			...
			echo "<td>".$row1['purchaserName']."</td>";
			echo "<td>".$row1['date_created']."</td>";
			echo "<td>".$row1['effectivity']."</td>";
			echo "<td>".$row1['type']."</td>";
			echo "<td>".$row1['NOVALICHES']."</td>";
			echo "<td>".$row1['IMUS']."</td>";
			echo "<td>".$row1['NAVOTAS']."</td>";
			echo "<td>".$row1['TONDO']."</td>";
			echo "<td>".$row1['CAMARIN']."</td>";
			echo "<td>".$row1['GEMS']."</td>";
			echo "<td>".$row1['EASTMART']."</td>";
			echo "<td>".$row1['MALABON']."</td>";
			echo "<td>".$row1['GAGALANGIN']."</td>";
			echo "<td>".$row1['CAINTA']."</td>";
			echo "<td>".$row1['VALENZUELA']."</td>";
			echo "<td>".$row1['BAGONGSILANG']."</td>";
			echo "<td>".$row1['PUNTURIN']."</td>";
			echo "<td>".$row1['PATEROS']."</td>";
			echo "<td>".$row1['COMEMBO']."</td>";
			echo "<td>".$row1['CAINTA2']."</td>";
			echo "<td>".$row1['SANPEDRO']."</td>";
			echo "<td>".$row1['LASPINAS']."</td>";
			echo "<td>".$row1['ALAMINOS']."</td>";
			echo "<td>".$row1['BAGUMBONG']."</td>";
			echo "<td>".$row1['GRACEVILLE']."</td>";
		//	echo "<td>".$row1['RETAIL']."</td>";
			echo "<td>".$row1['MOLINO']."</td>";
			echo "<td>".$row1['MANGGAHAN']."</td>";
			echo "<td>".$row1['MONTALBAN']."</td>";
		...
		echo "</table>";
				$truncate = "TRUNCATE  srs_aria_nova.0_sdma_reports";
				db_query($truncate);
...
function resend_email($branch, $id){
	...
	//	die;
		if(file_exists($path_to_root1 . "/doc_signs/Sup(SAF".$saf[1].").png")){
			$rep->NewLine($a);
			$rep->AddImage($path_to_root1 . "/doc_signs/Sup(SAF".$saf[1].").png", 380,180,150, 50);
			if(file_exists($path_to_root1 . "/doc_signs/".$po_id.".png")){
				$rep->AddImage($path_to_root1 . "/doc_signs/signatures/".$po_id.".png", 70,180,150, 50);
			}else{
				$rep->AddImage($path_to_root1 . "/doc_signs/signatures/nosign.png", 70,180,150, 50);
			}
			$rep->Text(100,'Purchaser Signature', $companyCol,'center');
			$rep->Text(400,'Supplier Signature',200,'center');
			$rep->NewLine(6);
...
start_table();
	start_row();
		...
		if($_SESSION['wa_current_user']->user == 888 OR $_SESSION['wa_current_user']->user == 1 OR $_SESSION['wa_current_user']->user == 633  OR $_SESSION['wa_current_user']->user == 730 OR $_SESSION['wa_current_user']->user == 642 OR $_SESSION['wa_current_user']->user == 651 OR $_SESSION['wa_current_user']->user == 886 OR $_SESSION['wa_current_user']->user == 654 OR $_SESSION['wa_current_user']->user == 905 OR $_SESSION['wa_current_user']->user == 689){
		...
		get_branchcode_list_cells_byid('Branch:','from_loc',null,'Select Branch',true);
		get_sdma_type_list_cells_('Type:', 'sdma_type');
		...
		submit_cells('export','Export To Excel','','');	
		submit_cells('export1','Export To Excel Per Branch','','');	
	end_row();
end_table(2);
div_end();
...
if ($_POST['from_loc'])
{
	$branches_sql .= " WHERE code ='".$_POST['from_loc']."'";
}
$branches_query = db_query($branches_sql);
...
foreach ($all_branch as $key => $row) 
{
	...
	if($row['is_done'] == 2){
		label_cell("<b>CANCELED BY ".get_display_username_by_id_and_branch_($row['approval_1'], $row['branch_db'])."</b>","colspan=6 align='center'");
	...
	/* 	if (($_SESSION['wa_current_user']->username == 'cecil' OR $_SESSION['wa_current_user']->can_approve_sdma_1) OR 
			...
			$saf = explode('SRSSAF ',$row['reference']);
		label_cell(pager_link(_("Update Signature"),
				"/purchasing/supplier_sign.php?AddedID=".$saf[1], ICON_UPDATE));
