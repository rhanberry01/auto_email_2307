...
if (isset($_GET['New']))
{
	...
	if ($_GET['NT'] =='no')
		$_SESSION['supp_trans']->addback = true;
}
...
//------------------------------------------------------------------------------------------------
function exists_supp_trans_dis($type, $supp_reference, $special_reference)
{
	if ($type == ST_SUPPRECEIVE)
		return exists_grn($type_no);

	$sql = "SELECT trans_no, cv_id FROM ".TB_PREF."supp_trans WHERE type=".db_escape($type)."
		AND supp_reference=".db_escape($supp_reference)." AND special_reference=".db_escape($special_reference);
...
function update_supp_trans_dis($type, $old_invoice, $supp_reference, $special_reference)
{
	if ($type == ST_SUPPRECEIVE)
		return exists_grn($type_no);

	$sql = "UPDATE ".TB_PREF."supp_trans SET supp_reference = ".db_escape($supp_reference)." WHERE type=".db_escape($type)."
		AND supp_reference=".db_escape($old_invoice)." AND special_reference=".db_escape($special_reference);
...
function check_discrep($trans_no)
{
	$sql = "SELECT * FROM ".TB_PREF."discrepancy
			WHERE trans_no = ".$trans_no."
			AND type = 20
			AND special_reference = ".db_escape($_SESSION['supp_trans']->special_reference);
	$res = db_query($sql);
...
function check_data()
{
	...
	{
			$type_no = exists_supp_trans_dis(ST_SUPPINVOICE, $_SESSION['supp_trans']->supp_reference, $_SESSION['supp_trans']->special_reference);
		if(!check_discrep($type_no[0])){
			if(strpos($_SESSION['supp_trans']->special_reference , 'OP')  !== false){
				$old_invoice = $_SESSION['supp_trans']->supp_reference;
			
				$_SESSION['supp_trans']->supp_reference = $_SESSION['supp_trans']->supp_reference.'-'.$type_no[0];
			
				update_supp_trans_dis(ST_SUPPINVOICE, $old_invoice, $_SESSION['supp_trans']->supp_reference, $_SESSION['supp_trans']->special_reference);
			}
			
			//display_error($_SESSION['supp_trans']->supp_reference);
			//display_error($old_invoice);
			//die;
		}else{
...
//--------------------------------------------------------------------------------------------------
function void_cv_header_details($type_no, $cv_id, $account_payable, $new_type_no)
{
	$sql = "SELECT SUM(amount) as amount FROM ".TB_PREF."cv_details WHERE trans_type = 20 AND voided = 0 AND trans_no = ".db_escape($type_no)." and cv_id = ".db_escape($cv_id);	
	$result = db_query($sql, "Cannot retrieve a cv details");
	$row = db_fetch_row($result);
	
	$sql = "UPDATE ".TB_PREF."cv_header set amount = amount-".$row[0]." WHERE id = ".db_escape($cv_id);
	$result = db_query($sql, "Cannot update cv header");
	
	$sql = "UPDATE ".TB_PREF."cv_details set voided = 1 WHERE trans_type = 20 AND trans_no = ".db_escape($type_no)." and cv_id = ".db_escape($cv_id);
	$result = db_query($sql, "Cannot voided cv details");
	
	$sql = "INSERT INTO 0_cv_details(cv_id, trans_type, trans_no, amount) VALUES($cv_id, 20, $new_type_no, ".round2($account_payable,2).")";
	$result = db_query($sql, "Cannot inserted cv details");	
	
	$sql = "UPDATE ".TB_PREF."cv_header set amount = amount+".$account_payable." WHERE id = ".db_escape($cv_id);
	$result = db_query($sql, "Cannot update cv header");
	
	$sql = "UPDATE ".TB_PREF."supp_trans set cv_id = $cv_id WHERE type = 20 and trans_no = ".db_escape($new_type_no);
	$result = db_query($sql, "Cannot update supplier trans_no");
	//display_error('ok');
	//exit;
    return $row;
}//--------------------------------------------------------------------------------------------------
function update_discrepancy_status($type_no)
{
	if ($type == ST_SUPPRECEIVE)
		return exists_grn($type_no);

	$sql = "UPDATE ".TB_PREF."discrepancy set status = 1 WHERE trans_no=".db_escape($type_no);
	
	$result = db_query($sql, "Cannot update discrepancy status");
}
//----------------------------------------------------------------------------
function handle_commit_invoice()
{
	...
	if (!$_SESSION['supp_trans']->nt)
	{
		...
		if(!$type_no){
			...
			void_supp_allocations(ST_SUPPINVOICE, $type_no[0]);
			void_supp_trans(ST_SUPPINVOICE, $type_no[0]);
			update_discrepancy_status($type_no[0]);
		}
		//display_error($type_no[0]);die;
	...
	if (!$_SESSION['supp_trans']->nt){
		void_cv_header_details($type_no[0],$type_no[1], $_SESSION['supp_trans']->acounts_payable, $invoice_no);
	}
    unset($_SESSION['supp_trans']);
