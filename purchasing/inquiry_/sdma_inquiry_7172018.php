...
function sent_email($branch, $id){
	...
	//	die;
		...
		{
			$sql = "SELECT a.* FROM ".$row['aria_db'].".".TB_PREF."sdma a WHERE reference = 'SRSSAF ".$saf[1]."'";
			$query = db_query($sql);	
			$row1=db_fetch($query);
			//	display_error($sql);
			if(!$row1){
...
start_table();
	start_row();
		...
		if($_SESSION['wa_current_user']->user == 633  OR $_SESSION['wa_current_user']->user == 730 OR $_SESSION['wa_current_user']->user == 642 OR $_SESSION['wa_current_user']->user == 651 OR $_SESSION['wa_current_user']->user == 886){
