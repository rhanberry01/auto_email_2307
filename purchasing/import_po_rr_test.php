...
page(_($help_context = "Import Purchase Order and Receiving"), false, false, "", $js);
set_time_limit(0);
$import_errors = array();
//===================================================
...
function auto_apv_to_cv($invoice_no)
{
	//get apv
	//get DM
	//compare to DM (get all on hold APV)
		//if > DM process
		//else wait for next APV
}
...
if (isset($_POST['ImportAll']) OR isset($_GET['nmhgsdrdntgdsad']))
{
	...
	if (date1_greater_date2($_POST['OrdersAfterDate'], '01/12/2014')) // ex 01-13-2014 = true
	{
		// display_error($_POST['OrdersAfterDate']);
		// display_error('01/13/2014');
		// display_error(date1_greater_date2($_POST['OrdersAfterDate'], '01/12/2014'));
		display_error('Date must be before or equal to 01-12-2014. 01-13-2014 onwards are imported to CV');
		display_footer_exit();
	}
	...
	while($rr_row = mssql_fetch_array($res))
	{
		...
		// {
			...
			{
				display_error('ERROR at PO # '. $rr_id[0].'s RECEIVING ID :'.$rr_id[1]);
				cancel_transaction();
			}
		// }
	}
