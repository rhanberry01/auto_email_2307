...
{
$sql = "SELECT code from transfers.0_branches where id='".$br_id."' ";
//display_error($sql);
$result=db_query($sql);
$row=db_fetch($result);
$br_code=$row['code'];
		...
		foreach ($dm_ids as $cleared_id)
		 {
					...
					if ($_POST['supplier_id_aria'] != '')
					{
						$aria_supp_id=get_supplier_id_by_supp_ref($aria_supp_ref);
						$_POST['supplier_id_aria'] = $aria_supp_id;
						
						//display_error($aria_supp_id);
						//display_error($_POST['supplier_id_aria']);
					}
					...
					else if (!$_POST['sdma_id'] == '')
					{
						...
						$id = $_POST['ref'];
						update_sdma($_POST['ref'],$_POST['reference'],$_POST['supplier_id_aria'],$_POST['sdma_type'],$_POST['dm_date'],
							$_POST['freq'],$periods,input_num('amount'),input_num('disc_percent'),
							$_POST['effective_from'],$_POST['effective_to'],$_POST['just_once'],$_POST['po_no'],$_POST['comment']);
							commit_transaction();
...
if ($_POST['po_no'] == '')
{
	...
	{
	check_row("Apply to ".$row['name'],'selected_id'.$row['id'],null,false, '', "align='center'");
	}
