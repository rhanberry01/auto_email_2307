	...
	if ($count>0){
		while($row = db_fetch($res))
		{
			if ($date_paid>=$row['bd_trans_date']){
							...
							if ($row['bd_trans_no']!='') {
								update_other_income_payment_header2($row['bd_trans_no'],$date_paid,$bank_statement_id,$bank_gl_code);
							}
							else{
	...
	if ($_POST['bank_account']=='10102299'){
		...
		// {
			// $myBranchCode=$row_['code'];
				...
				// $sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_aub where cleared='0' AND debit_amount!=0  and date_deposited>='2016-12-01' and date_deposited<='2016-12-31' order by date_deposited";
