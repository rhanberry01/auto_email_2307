...
set_time_limit(0);
	...
	$sql= "SELECT * FROM 0_cwo_header where c_po_no like 'PO%' and c_supp_trans_no!=''";
	$res=db_query($sql);

while($row = db_fetch($res))
{
	$sql1 = "UPDATE ".TB_PREF."supp_trans SET special_reference='".$row['c_po_no']."'
	WHERE trans_no='".$row['c_supp_trans_no']."' and type='20' ";
	db_query($sql1,"Failed to update supp_trans.");
}
	
display_notification("Fixing CWO Special Reference is Successful!");
}
start_form();
start_table();
start_row();
...
br();
start_row();
submit_center('Add',_("Fix CWO Special Reference"), true, '', 'default');
end_table();
end_form();
end_page();
?>