...
function get_result()
{
	...
	if ($_POST['rs_id'] == '')
	{
		$sql .= " WHERE a.bo_processed_date >= '".date2sql($_POST['rs_date_from'])."'
				  AND a.bo_processed_date <= '".date2sql($_POST['rs_date_to'])."'";
...
if (isset($_POST['excel']))
{
	...
	$supp_str = 'All Suppliers';
	$type_str = 'Returns';
	$stat_str = 'Pending';
	
	if ($_POST['supplier_code'] != '')
		$supp_str = get_ms_supp_name($supp_ref);
    ...
    $rep = new FrontReport(_('Returns and Disposal Report'), "ReturnsAndDisposalReport", "LETTER");
	
	$cols = array(0, 150, 250, 350, 420, 490, 570, 630);
	$aligns = array('left', 'left', 'left', 'left', 'left', 'left', 'left');
	$headers = array('#', 'Date', 'Supplier', 'Disposed/Returned by', 'Processed by', 'Remarks','Amount');
	
	$params =   array( 	0 => '',
    				    1 => array('text' => _('Period'), 'from' => $_POST['rs_date_from'],'to' => $_POST['rs_date_to']),
                    	2 => array('text' => _('Supplier'), 'from' => $supp_str, 'to' => ''),
    				    3 => array('text' => _('Type'), 'from' => $type_str,'to' => ''),
    				    4 => array('text' => _('Status'), 'from' => $stat_str,'to' => '')
						);
	$rep->Font();
	...
	{
		$rep->TextCol(0, 1, $row['movement_type'].' # '.$row['movement_no'], -5);
		$rep->TextCol(1, 2, sql2date($row['bo_processed_date']));
		$rep->TextCol(2, 3, get_ms_supp_name($row['supplier_code']));
		
		if ($row['processed_by'] > 0)
			$rep->TextCol(3, 4, get_username_by_id_rs($row['processed_by']));
		else
			$rep->TextCol(3, 4, '');
		...
		{
			$rep->TextCol(4, 5, '');
			$rep->TextCol(5, 6, $row['comment']);
		}
		else //processed already
		{
			$rep->TextCol(4, 5, get_username_by_id($row['acct_processed_by']));
			$rep->TextCol(5, 6, $row['comment']);
		}
		
		$total += $row['Total'];
		$rep->AmountCol(6, 7, $row['Total'],3);
		$rep->NewLine();
	}
	
	$rep->TextCol(0, 6, 'TOTAL:');
	$rep->AmountCol(6, 7, $total,3);
	$rep->NewLine();
	$rep->End();
}
// REPRINT ------------------------------------------------------
...
submit_cells('excel', 'Export to Excel', "", false, false);
end_table(2);
...
while($row = db_fetch($res))
{
	...
	{
		label_cell(get_username_by_id($row['acct_processed_by']),'align=center');
		label_cell($row['comment']);
		label_cell('');
	}
