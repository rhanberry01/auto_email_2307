...
if (isset($_POST['Fix'])) {
		...
		$transfer_dms_sql= "SELECT DISTINCT supp_name FROM transfers.0_transferred_dms where br_code!='srsn' and nova_trans_no=0";
				...
				//display_error('GO2');
						...
						while($row_ = db_fetch($result_))
						{
							...
							//display_error($myBranchCodex);
								...
								FROM transfers.0_transferred_dms
								...
								while($row_y = db_fetch($result_y))
								{
										...
										$trans_no = add_supp_trans(ST_SUPPDEBITMEMO, $supp_id, $date_x, '', $reference, "$myBranchNamex Consolidated $yr Pending Debit Memo", -$amt,  0, 0, "", 0, 0);
										
										// GL entries
										$debit_account = '2000'; //accounts_payable
										$credit_account = '5700033'; // 5700033	Debit Memo In
										
										add_gl_trans(ST_SUPPDEBITMEMO, $trans_no, $date_x, $debit_account, 0, 0, "$myBranchNamex Consolidated $yr Pending Debit Memo", $amt,null,3,$supp_id);
										add_gl_trans(ST_SUPPDEBITMEMO, $trans_no, $date_x, $credit_account, 0, 0, "$myBranchNamex Consolidated $yr Pending Debit Memo", -$amt, null,3,$supp_id);
						

										//=============TO GET MEMO							
										$sql_get_dm = "SELECT * FROM transfers.0_transferred_dms
										where br_code!='srsn' 
										...
										//print_r($dm_x);
										$memo_="Transferred Debit Memo: ".$myBranchNamex."-".$yr.": (".$dm_x.")";
										//display_error($memo_);
										if($memo_ != '')
										{
										...
										$sql = "UPDATE transfers.0_transferred_dms SET nova_trans_no='$trans_no' 
										where br_code!='srsn' 
										and supp_name=".db_escape($supp_name_x)."
										and br_code='$myBranchCodex'
										and YEAR(date_)='$yr'
										...
										db_query($sql,'failed to update 0_transferred_dms.');
										//display_error($sql);
...
br();
start_row();
submit_center('Fix', 'Create DM for transfererred DM');
end_table();
end_form();
end_page();
?>