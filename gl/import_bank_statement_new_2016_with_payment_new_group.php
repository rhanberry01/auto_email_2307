...
function get_online_payments($date_deposited, $debit_amount)
{
	...
	AND g.tran_date >='2016-01-01' AND g.tran_date <='2016-12-31'
	AND (ISNULL(b.reconciled) OR b.reconciled='' OR  b.reconciled='0000-00-00')
	AND b.amount='-$debit_amount'
	LIMIT 1
	...
	//AND g.tran_date >='2016-01-01' AND g.tran_date <='".$to."'
...
AND g.tran_date >='2016-01-01' AND g.tran_date <='2016-12-31'
AND cd.chk_number='$chk_number' AND cd.chk_amount='$debit_amount'
AND (ISNULL(b.reconciled) OR b.reconciled='' OR  b.reconciled='0000-00-00') 
LIMIT 1
";
...
function get_journal_entries($chk_number,$debit_amount,$date_deposited)
{
	...
	where gl.tran_date>='2016-01-01'
	and gl.tran_date<='2016-12-31'
	AND gl.type=0
...
function update_manual($manual_id,$db_)	
{		
	 $sql = "UPDATE cash_deposit.$db_ 
	 SET cleared='1'
	WHERE id = '$manual_id'";
db_query($sql);
//display_error($sql);
...
function clear_aria_cr_dr_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code)
{
	...
	$sql = "SELECT * FROM cash_deposit.".TB_PREF."cash_dep_header_new WHERE cd_trans_type='$trans_type' and cd_gross_amount='$credit_amount'  and cd_bank_account_code='$bank_gl_code' and cd_cleared='0' and cd_sales_date>='2017-01-01' order by cd_trans_date limit 1";
	$res = db_query($sql);
...
function clear_aria_cash_dep_trans_group($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code,$reference,$manual_id)
{
	global $db_connections;
	$trans_type=ST_CASHBOOK;
	$sql = "SELECT * FROM cash_deposit.".TB_PREF."cash_dep_header_new WHERE cd_trans_type='$trans_type' and cd_id IN ($reference) and cd_bank_account_code='$bank_gl_code' and cd_cleared='0' order by cd_trans_date";
	//display_error($sql);
	$res = db_query($sql);
	...
	if ($count>0){
		...
		{	
				...
				if ($bank_gl_code=='10102299'){
				$db_='bank_statement_aub';
				$db_manual='z_aub_deposit_';
				}
				else if ($bank_gl_code=='1020021'){
				$db_='bank_statement_metro';
				$db_manual='z_metro_deposit_';
				}
				else if ($bank_gl_code=='1010040'){
				$db_='bank_statement_bpi';
				$db_manual='z_bpi_deposit_';
				}
				else if ($bank_gl_code=='1010041'){
				$db_='bank_statement_bdo';
				$db_manual='z_metro_deposit_';
				...
				update_bank_statement($db_,$bank_statement_id,$row['cd_trans_type'],$reference,$myBranchCode);
				
				update_manual($manual_id,$db_manual);
...
function clear_aria_cash_dep_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code)
{
	global $db_connections;
	$trans_type=ST_CASHBOOK;
	$sql = "SELECT * FROM cash_deposit.".TB_PREF."cash_dep_header_new WHERE cd_trans_type='$trans_type' and cd_gross_amount='$credit_amount'  and cd_bank_account_code='$bank_gl_code' and cd_cleared='0' order by cd_trans_date limit 1";
	$res = db_query($sql);
...
function clear_aria_other_income_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code)
{
	...
	$sql = "SELECT * FROM cash_deposit.".TB_PREF."cash_dep_header_new WHERE cd_trans_type='$trans_type' and cd_gross_amount='$credit_amount'  and cd_bank_account_code='$bank_gl_code' and cd_cleared='0' and cd_sales_date>='2016-01-01' and cd_sales_date<='2016-12-31'  order by cd_trans_date limit 1";
	$res = db_query($sql);
...
function update_other_income_payment_header2($transno,$date_paid,$bank_statement_id,$bank_gl_code)	
{			//display_error($connect_to);
	 $sql = "UPDATE ".TB_PREF."other_income_payment_header 
	 SET bd_reconciled = '1',
	...
	WHERE bd_trans_no IN ($transno) AND bd_reconciled = '0'";
db_query($sql);
//display_error($sql);
}


function clear_aria_other_income_trans2($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code,$myBranchCode,$reference,$manual_id)
{
	global $db_connections;
	$trans_type=ST_BANKDEPOSIT;
	...
	if ($count>0){
		while($row = db_fetch($res))
		{
			//if ($date_paid>=$row['bd_trans_date']){
							...
							if ($row['bd_trans_no']!='') {
								update_other_income_payment_header2($reference,$date_paid,$bank_statement_id,$bank_gl_code);
							}
							else{
							display_error($myBranchCode.$row['bd_trans_no']."failed to clear.");
							}
				...
				$db_manual='z_metro_deposit_';
					...
					update_bank_statement($db_,$bank_statement_id,$row['bd_trans_type'],$reference,$myBranchCode);
...
if (isset($_POST['upload'])) 
{
	...
	if ($_POST['bank_account']=='10102299'){
		...
		while($row_ = db_fetch($result_))
		{
			...
			//$sqlx="select * from cash_deposit.z_aub_deposit_ where cleared=0 and br_code='$myBranchCode'";
			$sqlx="select * from cash_deposit.z_aub_deposit_ where cleared=0";
			//display_error($sqlx);
			$resx = db_query($sqlx);
			...
			while($rowx = db_fetch($resx))
			{
				...
				$credit_amount=$rowx['bank_statement_amount'];
				$reference=$rowx['ref'];
				$manual_id=$rowx['id'];
				
				$sql1="select * from cash_deposit.".TB_PREF."bank_statement_aub where credit_amount!=0 and credit_amount='$credit_amount' and cleared='0' and date_deposited>='2016-01-01' and  date_deposited<='2016-12-31' order by date_deposited LIMIT 1";
				//display_error($sql1); 
				...
				$count1=clear_aria_cash_dep_trans_group($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$reference,$manual_id);
							 
				 //$count2=clear_aria_other_income_trans2($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode,$reference,$manual_id);
				}
				// if ($count1<=0){
		...
		// {
			// $myBranchCode=$row_['code'];
				...
				// $sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_aub where cleared='0' AND debit_amount!=0  and date_deposited>='2016-01-01' and date_deposited<='2016-12-31' order by date_deposited";
		...
		while($row_ = db_fetch($result_))
		{
			//$myBranchCode=$row_['code'];
				...
				$row_['code']='srssanp';
				$myBranchCode='srssanp';
				
			switch_connection_to_branch_mysql($row_['code']);			
			
			$sqlx="select * from cash_deposit.z_metro_deposit_ where cleared=0 and type=2 and br_code='srssanp'";
			//display_error($sqlx); die;
			$resx = db_query($sqlx);
			...
			while($rowx = db_fetch($resx))
			{
				...
				$sql1="select * from cash_deposit.".TB_PREF."bank_statement_metro where credit_amount!=0 and credit_amount='$credit_amount' and cleared='0' and date_deposited>='2016-01-01' and  date_deposited<='2016-12-31' order by date_deposited LIMIT 1";
				//display_error($sql1); 
		...
		// $sql = "SELECT * from transfers.0_branches";
		// //display_error($sql);
		// $result=db_query($sql);
		...
		// {
				...
				// WHERE sdc.dc_remittance_date>='2017-01-01' AND sdc.dc_remittance_date<='2017-03-31'
				// AND (sdc.dc_card_desc='METROBANK' OR sdc.dc_card_desc='metrobank')
				// AND sdc.processed='0' AND sdc.paid='0'
		...
		// {
				...
				// $sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_metro where cleared='0' AND debit_amount!=0  and date_deposited>='2016-01-01' and  date_deposited<='2016-12-31' order by date_deposited";
				// $res_payment = db_query($sql_payment);
		...
		// {
			...
			// $sql1="select * from cash_deposit.".TB_PREF."bank_statement_bpi where credit_amount!=0 and cleared='0' and date_deposited>='2016-01-01' and date_deposited<='2016-12-31' order by date_deposited";
			// $res1 = db_query($sql1);
		...
		while($row_ = db_fetch($result_))
		{
			$myBranchCode=$row_['code'];
				...
				$sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_bpi where cleared='0' AND debit_amount!=0  and date_deposited>='2016-01-01' and date_deposited<='2016-12-31' order by date_deposited";
				$res_payment = db_query($sql_payment);
		...
		//$sql_ = "SELECT * from transfers.0_branches where code='srsn'";
		$sql_ = "SELECT * from transfers.0_branches where code='srstu'";
		//display_error($sql);
		$result_=db_query($sql_);
		while($row_ = db_fetch($result_))
		{
			//$myBranchCode=$row_['code'];
				...
				$sql="select * from cash_deposit.".TB_PREF."bank_statement_bdo where credit_amount!=0 and cleared='0' and date_deposited>='2016-01-01' and  date_deposited<='2016-12-31' order by date_deposited";
				$res = db_query($sql);
		...
		// {
				...
				// $sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_bdo where cleared='0' AND debit_amount!=0  and date_deposited>='2016-01-01' and date_deposited<='2016-12-31' order by date_deposited";
				// $res_payment = db_query($sql_payment);
