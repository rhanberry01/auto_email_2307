...
$cv_id = $_GET['cv_id'];
$apv_trans_no = $_GET['apv_trans_no'];



$search = find_submit('search1');
$dl_excel = find_submit('dl_excel1');
...
page(_($help_context = "Petty Cash with VAT"), true, false, "", $js);
...
$apv_trans_no=$_POST['apv_trans_no'];

$sql = "select * from 0_petty_cash_details WHERE pcd_cv_id='$cv_id'";
...
if ($tax_type!= '')
{
	$sql .= " AND pcd_tax_type='".$tax_type."'";
}

	$sql .= " AND pcd_w_cv='1' ORDER BY pcd_date";
...
div_start('pmt_header');
	...
	start_table("$table_style2"); // outer table
		tax_gl_list_cells('Tax Type:', 'tax_type', null, true, false, "All Accounts");
		hidden('cv_id',$cv_id);
		hidden('apv_trans_no',$apv_trans_no);
		...
		br();
		submit_cells('search1', 'Search','','',true);
	end_row();
...
$th = array();
array_push($th,'', 'Date', 'Trans #','RF/OR/SI #', 'Payee','Purpose','Account','Tax Type','Gross','Input Tax','Extended');

if (db_num_rows($res) > 0){
	br();
	$comment=get_comments_string(20, $apv_trans_no);
	display_heading($comment);
	br();
	br();
	start_row();
	submit_center('dl_excel1','Download as excel file');
	end_row();
