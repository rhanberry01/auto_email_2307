...
$csv_header = get_csv_header_pbcom($trans_no);

display_heading(_("PBCOM Manager's Check CSV Batch # ".$trans_no));
br();
...
start_table("$table_style width=90%");
    ...
    start_row();
    label_cells(_("PBCOM CSV Batch."), $csv_header['id'], "class='tableheader2'");
    label_cells(_("PBCOM CSV Date"), sql2date($csv_header['date']), "class='tableheader2'");
	end_row();

	start_row();
    // label_cells(_("Filename"),  $csv_header["csv_file"], "class='tableheader2'");
    label_cells(_("Amount"),'<b>'. number_format2(get_csv_amount_pbcom($csv_header['id']),2). '</b>', "class='tableheader2'");
    end_row();
...
$res = get_csv_details_pbcom($trans_no);
