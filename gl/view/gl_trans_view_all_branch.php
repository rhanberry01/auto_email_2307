...
function display_gl_heading($myrow)
{
	...
	if ($_GET['type_id'] == 52 OR $_GET['type_id'] == 53)
	{
		...
		$sql = "SELECT supp_reference, tran_date FROM ".$_GET['branch'].".".TB_PREF."supp_trans 
...
$sql = "SELECT gl.*, cm.account_name, IF(ISNULL(refs.reference), '', refs.reference) AS reference FROM ".$_GET['branch']."."
	.TB_PREF."gl_trans as gl
	LEFT JOIN ".$_GET['branch'].".".TB_PREF."chart_master as cm ON gl.account = cm.account_code
	LEFT JOIN ".$_GET['branch'].".".TB_PREF."refs as refs ON (gl.type=refs.type AND gl.type_no=refs.id)"
...
if ($heading_shown)
{
	if ($_GET['type_id'] == 52 OR $_GET['type_id'] == 53)
	{
		$prepared_by_sql = "SELECT user, date(stamp) FROM ".$_GET['branch'].".".TB_PREF."audit_trail 
										WHERE type = ".$_GET['type_id']." 
										AND trans_no = ".$_GET['trans_no']."
										AND description = ''";
