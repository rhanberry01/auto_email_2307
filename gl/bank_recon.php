...
function get_online_payments($date_deposited, $debit_amount)
{
	$from_date_to_clear=add_days(sql2date($date_deposited), -10);

	$sql = "SELECT s.supp_name,c.trans_no, c.type, c.supplier_id, c.tran_date, 
	...
	FROM 0_supp_trans as c LEFT JOIN 0_cv_header as a ON c.cv_id=a.id 
	LEFT JOIN 0_bank_trans as b ON a.bank_trans_id=b.id 
	LEFT JOIN 0_cheque_details as cd ON b.id=cd.bank_trans_id 
	LEFT JOIN 0_gl_trans as g ON g.type_no=c.trans_no 
	LEFT JOIN 0_suppliers as s ON g.person_id=s.supplier_id
	WHERE a.amount!=0 AND b.type='22' AND c.type='22' AND c.ov_amount!=0 
	AND g.type='22' and g.amount<0 AND g.account IN ('1020021') 
	AND (g.tran_date >='".date2sql($from_date_to_clear)."' AND g.tran_date <='$date_deposited')
	AND g.tran_date >='2018-01-01'
	AND (ISNULL(b.reconciled) OR b.reconciled='' OR  b.reconciled='0000-00-00')
	AND round(b.amount,2) ='".round(-$debit_amount,2)."' 
	LIMIT 1
	
	"; 
	
	//AND g.tran_date >='2018-01-01' AND g.tran_date <='".$to."'

	//after running all transaction rerun again using max date because there are cases that date in bank statement recorded first before aria.
	//AND g.tran_date >='2018-01-01' AND g.tran_date <='2018-05-31'
...
// 0_cheque_details c 
// WHERE a.tran_date>='2018-01-01' AND a.tran_date<='".$date_deposited."'
// AND a.type = '22'
// AND a.type = b.type 
// AND a.ov_amount!=0
...
function get_bt_cheque_details_bdo($chk_number,$debit_amount,$date_deposited)
{
$sql = "SELECT s.supp_name,c.trans_no, c.type, c.supplier_id, c.tran_date, 
c.reference, c.supp_reference,c.cv_id, a.id as a_id, 
a.cv_date, a.cv_no, b.bank_act,b.trans_date, b.id as b_id, 
...
LEFT JOIN 0_suppliers as s ON g.person_id=s.supplier_id
WHERE a.amount!=0 AND b.type='22' AND c.type='22' AND c.ov_amount!=0 
AND g.type='22' and g.amount<0 AND g.account IN ('1010041') 
AND g.tran_date >='2018-01-01' AND g.tran_date <='2018-05-31'
AND cd.chk_number='$chk_number'
AND (ISNULL(b.reconciled) OR b.reconciled='' OR  b.reconciled='0000-00-00') 
LIMIT 1
";
//AND cd.chk_amount='$debit_amount'
...
function get_bt_cheque_details_aub($chk_number,$debit_amount,$date_deposited)
{
$sql = "SELECT s.supp_name,c.trans_no, c.type, c.supplier_id, c.tran_date, 
c.reference, c.supp_reference,c.cv_id, a.id as a_id, 
a.cv_date, a.cv_no, b.bank_act,b.trans_date, b.id as b_id, 
...
LEFT JOIN 0_suppliers as s ON g.person_id=s.supplier_id
WHERE a.amount!=0 AND b.type='22' AND c.type='22' AND c.ov_amount!=0 
AND g.type='22' and g.amount<0 AND g.account IN ('10102299') 
AND g.tran_date >='2018-01-01' AND g.tran_date <='2018-05-31'
AND cd.chk_number='$chk_number' AND round(cd.chk_amount,2)='".round($debit_amount,2)."'
AND (ISNULL(b.reconciled) OR b.reconciled='' OR  b.reconciled='0000-00-00') 
LIMIT 1
";
...
function get_bt_cheque_details_bpi($chk_number,$debit_amount,$date_deposited)
{
	$sql = "SELECT s.supp_name,c.trans_no, c.type, c.supplier_id, c.tran_date, 
	c.reference, c.supp_reference,c.cv_id, a.id as a_id, 
	a.cv_date, a.cv_no, b.bank_act,b.trans_date, b.id as b_id, 
	...
	LEFT JOIN 0_suppliers as s ON g.person_id=s.supplier_id
	WHERE a.amount!=0 AND b.type='22' AND c.type='22' AND c.ov_amount!=0 
	AND g.type='22' and g.amount<0 AND g.account IN ('1010040') 
	AND g.tran_date >='2018-01-01' AND g.tran_date <='2018-05-31'
	AND cd.chk_amount='$debit_amount'
	AND (ISNULL(b.reconciled) OR b.reconciled='' OR  b.reconciled='0000-00-00') 
	LIMIT 1
...
function get_journal_entries($chk_number,$debit_amount,$date_deposited)
{
	// $sql = "SELECT s.supp_name,c.trans_no, c.type, c.supplier_id, c.tran_date, 
	// c.reference, c.supp_reference,c.cv_id, a.id as a_id, 
	// a.cv_date, a.cv_no, b.bank_act,b.trans_date, b.id as b_id, 
	...
	$sql = "SELECT * FROM 0_gl_trans as gl
	left join 0_bank_trans as bt
	on gl.type_no=bt.trans_no
	where gl.tran_date>='2018-01-01'
	and gl.tran_date<='2018-05-31'
	AND gl.type=0
	AND gl.account='1020021'
	and gl.amount!=0
	and gl.amount>0
	AND bt.type=0
	and bt.amount!=0
	and bt.amount>0
	AND gl.amount='$debit_amount'
	AND (ISNULL(bt.reconciled) OR bt.reconciled='' OR  bt.reconciled='0000-00-00') 
	LIMIT 1";
...
function update_cash_dep_header($cleared_id,$date_paid,$date_cleared,$bank_statement_id,$remarks,$payto,$trans_type)	
{		
	 $sql = "UPDATE cash_deposit.".TB_PREF."cash_dep_header_new 
	 SET cd_cleared = '1',
	 cd_date_deposited='$date_paid',
	 cd_date_cleared='".date2sql(Today())."',
	 cd_bank_id='$bank_statement_id'
...
function clear_aria_cr_dr_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code)
{
	...
	$sql = "SELECT * FROM cash_deposit.".TB_PREF."cash_dep_header_new WHERE cd_trans_type='$trans_type' and cd_gross_amount='$credit_amount'  and cd_bank_account_code='$bank_gl_code' and cd_cleared='0' and cd_sales_date>='2018-01-01' order by cd_trans_date limit 1";
	$res = db_query($sql);
	...
	if ($count>0){
		...
		{
			switch_connection_to_branch_mysql($row['cd_br_code']);
			...
			$myBranchCode=$row['cd_br_code'];
				...
				$bank_account=$row['cd_bank_deposited'];
								...
								$sql2="select * from cash_deposit.".TB_PREF."cash_dep_header_new where cd_trans_type='$trans_type' and cd_id='$cd_id' and cd_cleared='0'";
								//display_error($sql);
			...
			update_cash_dep_header($row['cd_id'],$date_paid,$date_cleared,$bank_statement_id,$remarks,$payto,$trans_type);
			...
			else if ($bank_gl_code=='1010041'){
			$db_='bank_statement_bdo';
			}
...
function clear_aria_cash_dep_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code)
{
	...
	$from_date_to_clear=add_days(sql2date($date_paid), -7);
	
	$trans_type=ST_CASHBOOK;
	$sql = "SELECT * FROM cash_deposit.".TB_PREF."cash_dep_header_new WHERE cd_trans_type='$trans_type' and cd_gross_amount='$credit_amount'  
	and (cd_sales_date>='".date2sql($from_date_to_clear)."' and cd_sales_date<='$date_paid')
	and cd_sales_date>='2018-01-01'
	and cd_bank_account_code='$bank_gl_code' and cd_cleared='0' order by cd_trans_date 
	LIMIT 1";
	$res = db_query($sql);
			...
			//if ($date_paid>=$row['cd_date_deposit']){
				...
				//switch_connection_to_branch_mysql($row['cd_br_code']);
					...
					$bank_account=$row['cd_bank_deposited'];
									...
									// if ($transno!='') {
									// add_gl_trans(ST_CASHBOOK, $transno, sql2date($date_paid), 272727, 0, 0, $remarks,-$amount, null, $person_type_id, $person_id);
										...
										// add_gl_trans(ST_CASHBOOK, $transno, sql2date($date_paid), $cash_in_bank, 0, 0, $remarks,$amount, null, $person_type_id, $person_id);

										// $desc='Cleared';
										// add_audit_trail(ST_CASHBOOK, $transno, $date_paid,$desc);
									// }
...
function clear_aria_other_income_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code)
{
	...
	$sql = "SELECT * FROM cash_deposit.".TB_PREF."cash_dep_header_new WHERE cd_trans_type='$trans_type' and cd_gross_amount='$credit_amount'  and cd_bank_account_code='$bank_gl_code' and cd_cleared='0' and cd_sales_date>='2018-01-01' and cd_sales_date<='2018-05-31'  order by cd_trans_date limit 1";
	$res = db_query($sql);
...
function update_bdo_gl_trans($transno,$date_paid,$bank_statement_id,$bank_gl_code,$type,$counter)	
{			
	//display_error($connect_to);
...
function update_bayad_center_gl_trans($transno,$date_paid,$bank_statement_id,$bank_gl_code,$type,$counter)	
{			
	//display_error($connect_to);
	$sql = "UPDATE ".TB_PREF."gl_trans 
	SET dimension_id = '99',
	dimension2_id='$bank_statement_id'
	WHERE type_no = '$transno'
	and type='$type'
	and amount>0
	and counter='$counter'
	and account='$bank_gl_code'";
	db_query($sql);
	//display_error($sql);
}

function update_acquiring_deduction($transno,$date_paid,$bank_statement_id,$bank_gl_code)	
{			//display_error($connect_to);
	 $sql = "UPDATE ".TB_PREF."acquiring_deductions 
	 SET reconciled = '1',
	 recon_ref='$bank_statement_id',
	 recon_date='$date_paid',
	 recon_bank_account='$bank_gl_code'
	WHERE p_ref_id = '$transno'";
db_query($sql);
//display_error($sql);
}


function update_other_income_payment_header2($bd_id,$date_paid,$bank_statement_id,$bank_gl_code,$transno)	
{			//display_error($connect_to);
	 $sql = "UPDATE ".TB_PREF."other_income_payment_header 
	 SET bd_reconciled = '1',
	 bd_recon_ref='$bank_statement_id',
	 bd_recon_date='$date_paid',
	 bd_recon_bank_account='$bank_gl_code'
	WHERE bd_id = '$bd_id'";
	db_query($sql);
	
	 $sql1 = "UPDATE ".TB_PREF."gl_trans 
	 SET dimension_id = '99',
	WHERE type_no = '$transno'
	AND type=2";
	db_query($sql1);
...
function clear_aria_other_income_trans2($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code,$myBranchCode)
{
	global $db_connections;
	$trans_type=ST_BANKDEPOSIT;
	...
	$sql = "SELECT oi.*,gl.amount as amount FROM `0_gl_trans` as gl 
	LEFT JOIN 0_other_income_payment_header as oi
	on gl.type_no=oi.bd_trans_no
	where gl.type=2
	and gl.account='$bank_gl_code'
	and gl.tran_date>='".date2sql($from_date_to_clear)."' and gl.tran_date<='$date_paid'
	and oi.bd_trans_date>='".date2sql($from_date_to_clear)."' and oi.bd_trans_date<='$date_paid'
	and gl.amount>0
	and bd_reconciled=0
	and gl.amount='$credit_amount'
	LIMIT 1
	";
	$res = db_query($sql);
	...
	if ($count>0){
		while($row = db_fetch($res))
		{
			if ($date_paid>=$row['bd_trans_date']){
						
							//$myBranchCode=$db_connections[$_SESSION["wa_current_user"]->company]["br_code"];
				
							if ($row['bd_id']!='') {
								update_other_income_payment_header2($row['bd_id'],$date_paid,$bank_statement_id,$bank_gl_code,$row['bd_trans_no']);
							}
							else{
							display_error($myBranchCode.$row['bd_id']."failed to clear.");
							}
					...
					update_bank_statement($db_,$bank_statement_id,$row['bd_trans_type'],$row['bd_trans_no'],$myBranchCode);
			}
		}
	}
	return $count;
...
function clear_aria_cr_dr_trans2($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code,$myBranchCode)
{
	global $db_connections;
	$trans_type=ST_CREDITDEBITDEPOSIT;
	
	
						// $minimum_amount=$credit_amount-1;
						// $maximum_amount=$credit_amount+1;	
						// and gl.amount>='$minimum_amount' AND gl.amount<='$maximum_amount'
	
	$sql = "SELECT ad.*,gl.amount as amount FROM `0_gl_trans` as gl 
	LEFT JOIN 0_acquiring_deductions as ad
	on gl.type_no=ad.p_ref_id
	where gl.type=62
	and gl.account='$bank_gl_code'
	and gl.tran_date>='2018-01-01' and gl.tran_date<='2018-06-31'
	and gl.amount>0
	and gl.amount='$credit_amount'
	and ad.reconciled=0
	...
	if ($count>0){
		while($row = db_fetch($res))
		{
			if ($date_paid>=$row['date_paid']){
						
							//$myBranchCode=$db_connections[$_SESSION["wa_current_user"]->company]["br_code"];
				
							if ($row['p_ref_id']!='') {
								update_acquiring_deduction($row['p_ref_id'],$date_paid,$bank_statement_id,$bank_gl_code);
							}
							else{
							display_error($myBranchCode.$row['p_ref_id']."failed to clear.");
							}
					...
					update_bank_statement($db_,$bank_statement_id,$trans_type,$row['p_ref_id'],$myBranchCode);
			}
		}
	}
	return $count;
...
function clear_aria_bdo_deposit_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code,$myBranchCode)
{
	global $db_connections;
	//$trans_type=ST_CREDITDEBITDEPOSIT;
	...
	AND  tran_date <= '2018-05-31'
	AND account ='1010041' 
	and amount>='$minimum_amount' AND amount<='$maximum_amount'
	AND amount>0
	AND dimension_id=0
	...
	if ($count>0){
		while($row = db_fetch($res))
		{
		//	if ($date_paid>=$row['tran_date']){
						
							//$myBranchCode=$db_connections[$_SESSION["wa_current_user"]->company]["br_code"];
				
							if ($row['type_no']!='') {
								update_bdo_gl_trans($row['type_no'],$date_paid,$bank_statement_id,$bank_gl_code,$row['type'],$row['counter']);
							}
							else{
								display_error($myBranchCode.$row['type_no']."failed to clear.");
							}
					...
					update_bank_statement($db_,$bank_statement_id,$row['type'],$row['type_no'],$myBranchCode);
		//	}
		}
	}
	return $count;
...
function clear_aria_bayad_center_deposit_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code,$myBranchCode)
{
	global $db_connections;
	//$trans_type=ST_CREDITDEBITDEPOSIT;
	...
	$sql = "SELECT *
	FROM 0_gl_trans
	WHERE tran_date >='".date2sql($from_date_to_clear)."'
	AND  tran_date <='$date_paid'
	AND account ='$bank_gl_code' 
	and amount='$credit_amount'
	AND amount>0
	AND dimension_id=0
	AND type='74'
	LIMIT 1
	...
	if ($count>0){
		...
		//	if ($date_paid>=$row['tran_date']){
							...
							if ($row['type_no']!='') {
								update_bayad_center_gl_trans($row['type_no'],$date_paid,$bank_statement_id,$bank_gl_code,$row['type'],$row['counter']);
							}
							else{
								display_error($myBranchCode.$row['type_no']."failed to clear.");
							}
...
function handle_new_metro_excel_item()
{
	...
	$excel_col= $data->colcount();
	$excel_row= $data->rowcount();
	display_error($excel_col);
	display_error($excel_row);
	
	$bank_format_marker=$data->val(4,7);
		display_error($bank_format_marker);
		$bank_format_marker=$data->val(4,6);
	display_error($bank_format_marker);
	$bank_format_marker='Branch';
	display_error($bank_format_marker);
	$db_='bank_statement_metro';
...
if (isset($_POST['upload'])) 
{
	...
	// //handle_new_aub_excel_item();
		...
		// //$sql_ = "SELECT * from transfers.0_branches_other_income where code='srsn'";
		// $sql_ = "SELECT * from transfers.0_branches_other_income";
		// //display_error($sql);
		...
		// {
			// $myBranchCode=$row_['code'];
				...
				// switch_connection_to_branch_mysql($row_['code']);			
		
				// $sql="select * from cash_deposit.".TB_PREF."bank_statement_aub where credit_amount!=0 and cleared='0' and date_deposited>='2018-01-01' and  date_deposited<='2018-05-31' order by date_deposited";
				// $res = db_query($sql);
				...
				// {
						...
						// if ($count1<=0){
							// $count2=clear_aria_bayad_center_deposit_trans($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode);
						// }
						
						// if ($count2<=0){
							 // $count3=clear_aria_other_income_trans2($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode);
						// }
		...
		// {
				...
				// $sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_aub where cleared='0' AND debit_amount!=0  
							  // and date_deposited>='2018-01-01' and date_deposited<='2018-05-31' order by date_deposited
							   // ";
				...
				// {
					...
					// $row_bank_trans=get_bt_cheque_details_aub($bank_ref_num,$debit_amount,$date_deposited);
					// $bank_trans_id=$row_bank_trans['b_id'];

				// //display_error($row_bank_trans.'--bank_trans_id - >'.$bank_trans_id);
...
// //===========SALES,OI,CR-DR=========
		...
		while($row_ = db_fetch($result_))
		{
			...
			$sql1="select * from cash_deposit.".TB_PREF."bank_statement_metro where credit_amount!=0 and cleared='0' and date_deposited>='2018-01-01' and date_deposited<='2018-06-31' order by date_deposited";
			$res1 = db_query($sql1);
			...
			while($row1 = db_fetch($res1))
			{
				...
				clear_aria_cr_dr_trans2($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode);
				...
				// }
				// if ($count1<=0){
		//$count2=clear_aria_other_income_trans2($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode);
				// }	
				// //if ($count2<=0){
				// //$count3=clear_aria_bayad_center_deposit_trans($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode);
				// //}
			}
		...
		// $sql = "SELECT * from transfers.0_branches_other_income";
		// //display_error($sql);
		// $result=db_query($sql);
		// while($row = db_fetch($result))
		// {
				// $myBranchCode=$row['code'];
				// switch_connection_to_branch_mysql($row['code']);
				...
				// WHERE sdc.dc_remittance_date>='2018-01-01' AND sdc.dc_remittance_date<='2018-05-31'
				// AND (sdc.dc_card_desc='METROBANK' OR sdc.dc_card_desc='metrobank')
				// AND sdc.processed='0' AND sdc.paid='0'
				...
				// //display_error($count1);
					...
					// {
						...
						// $minimum_amount=$row_sales_dr_cr['net']-1;
						// $maximum_amount=$row_sales_dr_cr['net']+1;	
		...
		// {
				...
				// $sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_metro where cleared='0' AND debit_amount!=0  and date_deposited>='2018-01-01' and  date_deposited<='2018-05-31' order by date_deposited";
				// $res_payment = db_query($sql_payment);
		...
		// //=====================START OF - TO RECON MANUAL APPROVED OR WITH GL ENTRIES ALREADY
		// $sql_ = "SELECT * from transfers.0_branches_other_income";
		// //display_error($sql);
		// $result_=db_query($sql_);
		// while($row_ = db_fetch($result_))
		// {
			...
			// $sql1="select * from cash_deposit.".TB_PREF."bank_statement_bpi where credit_amount!=0 and cleared='0' and date_deposited>='2018-01-01' and date_deposited<='2018-05-31' order by date_deposited";
			// $res1 = db_query($sql1);
			...
			// {
				...
				// // }	
				// // if ($count2<=0){
				// $count3=clear_aria_cr_dr_trans2($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode);
				// //}
			// }
		
		// }
		// //=====================END OF - TO RECON MANUAL APPROVED OR WITH GL ENTRIES ALREADY
		
		
		// // //===========START OF - PAYMENT RECON=============
		// $db_='bank_statement_bpi';
		// $type=22;
		...
		// {
			// $myBranchCode=$row_['code'];
				...
				// $sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_bpi where cleared='0' AND debit_amount!=0  and date_deposited>='2018-01-01' and date_deposited<='2018-05-31' order by date_deposited";
				// $res_payment = db_query($sql_payment);
				...
				// {
					...
					// $row_bank_trans=get_bt_cheque_details_bpi($bank_ref_num,$debit_amount,$date_deposited);
					// $bank_trans_id=$row_bank_trans['b_id'];
				...
				// }
		// }
		// //===================================END OF - PAYMENT RECON
		
		
		// //=====================START OF - AUTO APPROVE/CLEARING AND BANK RECON
			
		// //$sql = "SELECT * from transfers.0_branches_other_income where code!='srscain2'";
		// $sql = "SELECT * from transfers.0_branches_other_income";
		// //display_error($sql);
		// $result=db_query($sql);
		// while($row = db_fetch($result))
				...
				// //display_error($count1);
					...
					// {
						...
						// if($tender_type=='013'){
							...
							// $date_to_clear=add_days(sql2date($row_sales_dr_cr['date']), 5);
		...
		// //=====================END  OF - AUTO APPROVE/CLEARING AND BANK RECON
		
		display_notification(_('Selected transaction has been succesfuly approved.'));
	}
	else if ($_POST['bank_account']=='1010041'){
		
		set_time_limit(0);
		
		// //handle_new_bdo_excel_item();
		//===========SALES,OI=========
		...
		while($row_ = db_fetch($result_))
		{
			$myBranchCode=$row_['code'];
				...
				$sql="select * from cash_deposit.".TB_PREF."bank_statement_bdo where credit_amount!=0 and cleared='0' and date_deposited>='2018-01-01' and  date_deposited<='2018-05-31' order by date_deposited";
				$res = db_query($sql);
				...
				while($row = db_fetch($res))
				{
						...
						$credit_amount=$row['credit_amount'];
						$bank_statement_id=$row['id'];
						$bank_gl_code='1010041';
						
					
						//$count1=clear_aria_cash_dep_trans($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code);
						// if ($count1<=0){
							//$count2=clear_aria_other_income_trans2($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode);
							$count2=clear_aria_bdo_deposit_trans($bank_statement_id,$date_deposited,$credit_amount,$bank_gl_code,$myBranchCode);
						// }
		...
		{ 
				...
				$sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_bdo where cleared='0' AND debit_amount!=0  and date_deposited>='2018-01-01' and date_deposited<='2018-05-31' order by date_deposited";
				$res_payment = db_query($sql_payment);
				...
				while($row_debit = db_fetch($res_payment))
				{
					...
					$row_bank_trans=get_bt_cheque_details_bdo($bank_ref_num,$debit_amount,$date_deposited);
					$bank_trans_id=$row_bank_trans['b_id'];
