...
function create_dm_in($pcd_ids,$br_code,$config_db_key,$br_name) {
		...
		$transfer_dms_sql= "SELECT DISTINCT supp_name FROM transfers.0_transferred_transactions where br_code!='$br_code' and br_in_trans_no=0 and type=53 and br_code='$myBranchCode'";
		...
		{		
			...
			set_global_connection_branch_mysql($config_db_key);
			...
			if ($supp_names == '')
			{
				...
				//display_error('GO1');
				display_error("$supp_name_x Supplier does not exist in $br_name.");
				die();
			}
			else{
				//display_error('GO2');
						...
						while($row_ = db_fetch($result_))
						{
							...
							//display_error($myBranchCodex);
								...
								while($row_y = db_fetch($result_y))
								{
										...
										$sql = "UPDATE transfers.0_transferred_transactions SET br_in_trans_no='$trans_no', br_code_in='$br_code'
										where br_code!='$br_code' 
										and supp_name=".db_escape($supp_name_x)."
										and br_code='$myBranchCodex'
										and br_in_trans_no=0
...
if (isset($_POST['approve_all_checked'])) {
	...
	if (empty($_POST['to_loc'])) {
		display_error('Please Select Branch.');
		return false;
	}
	...
	$br_code=$_POST['to_loc'];
	//display_error($br_code);
	
							$sql_ = "SELECT * from transfers.0_branches_other_income where code='$br_code'";
							//display_error($sql);
							$result_=db_query($sql_);
							$row_ = db_fetch($result_);
							$config_db_key=$row_['config_db_key'];
							$br_name=$row_['name'];
							//display_error($config_db_key);
							
							$transfer_dms_sqlx="SELECT sup.supp_name
							...
							AND cv_id=0
							AND ov_amount!=0";
							$transfer_dms_sqlx .= " AND trans_no IN (".$pcd_ids.")";
							
							$transfer_dms_sqlx .= " GROUP BY sth.supplier_id";
							
							$transfer_dms_resultx=db_query($transfer_dms_sqlx);
							
							//display_error($transfer_dms_sql);
							
							while($transfer_dms_result_rowx=mysql_fetch_array($transfer_dms_resultx)) 
							{

									$supp_name=$transfer_dms_result_rowx['supp_name'];
									
									
									set_global_connection_branch_mysql($config_db_key);
									
									//===========SUPP NAME
									$sql_get_new_vendor= "SELECT supplier_id, supp_ref, supp_name FROM 0_suppliers WHERE supp_name=".db_escape($supp_name)."";
									$result1 = db_query($sql_get_new_vendor, "failed to get bank_accounts id.");
									$row1=db_fetch($result1);
									$supplier_id=$row1['supplier_id'];
									$supp_ref=$row1['supp_ref'];
									$supp_names=$row1['supp_name'];
									// display_error($supplier_id);
									// display_error($supp_ref);
									// display_error($supp_names);
									...
									if ($supp_names == '')
									{
										...
										//check_my_suppliers_name($supp_name_x);
										//display_error('GO1');
										display_error("$supp_name Supplier does not exist in $br_name.");
										die();
									}
	...
	begin_transaction();
					...
					while($transfer_dms_result_row_gl=mysql_fetch_array($transfer_dms_result_gl)) 
					{
							...
							while($transfer_dms_result_row=mysql_fetch_array($transfer_dms_result)) 
							{
								...
								$memo_="$supp_name: Journal, To transfer pending DM to $br_name Branch. Ref#: (".$dm_x.")";
	...
	create_dm_in($pcd_ids,$br_code,$config_db_key,$br_name);
...
echo '<center style="margin: 10px 0;">
		<button class="inputsubmit" type="submit" id="approve_all_checked" name="approve_all_checked" style="display: none;">
			...
			start_table();
			get_branchcode_list_cells('Transfer To:','to_loc',null,'Select Branch');
			end_table();
			br();
