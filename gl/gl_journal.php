...
if (isset($_POST['Process']))
{
	...
	// if (!(date('M') == date('M',strtotime($_POST['date_'])))) 
	// {
		// display_error(_("Adding transactions in the previous months are not allowed"));
		// set_focus('date_');
		// $input_error = 1;
	// } 

	if((date('Y-m-d',strtotime($_POST['date_'])) < '2017-12-31')){
		display_error(_("You must enter year 2018 only."));
		set_focus('code_id');
		$input_error = 1;
	}
...
/*	elseif ($Refs->exists(ST_JOURNAL, $_POST['ref']))  comment  2232018  saving of new ref is based on 0_refs #rhan
	{
...
if (isset($_POST['Process']))
{
	...
	$next_ref=$Refs->get_next(ST_JOURNAL);
	$cart->reference = $next_ref;
