...
if (isset($_POST['Fix'])){
	...
	begin_transaction();	
		...
		$sql_get_old_bank_id= "SELECT id FROM ".TB_PREF."bank_accounts WHERE account_code='10102299'";
		...
		$sql="
		SELECT 'srscom' as br_code, c.type, c.trans_no,  c.supplier_id, 
		...
		{
			//display_error('good');
			$sql2 = "UPDATE ".TB_PREF."bank_trans SET bank_act='$bank_id' WHERE id='".$row['b_id']."'";
			//display_error($sql2);
	...
	display_notification("Updating bank & gl acc is successful!");
...
start_form();
start_row();
submit_center('Fix',_("Fix bank & gl acc"), true, '', false);
end_table();
end_form();
end_page();
?>