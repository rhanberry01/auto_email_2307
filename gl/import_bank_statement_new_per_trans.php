...
function update_bank_journal($db_,$tran_date,$m_id,$trans_type,$dep_id)	
{		
...
if (isset($_POST['upload'])) 
{
	...
	if ($_POST['bank_account']=='10102299'){
		...
		while($journal_row = db_fetch($res_journal))
		{
				...
				WHERE sdc.dc_remittance_date='$tran_date'
				AND (sdc.dc_card_desc='METROBANK' OR sdc.dc_card_desc='metrobank')
				AND sdc.processed='0' AND sdc.paid='0'
				...
				//display_error($count1);
					...
					while($row_sales_dr_cr = db_fetch($res))
					{
						...
						//display_error($date_paid);
											...
											update_bank_journal($db_,$tran_date,$merchant_id_orig,$inserttype,$cd_id);
