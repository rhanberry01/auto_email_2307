...
page(_($help_context = "Fix Bank Accounts AUB"), false, false, "", $js);
...
if (isset($_POST['Fix'])){
	...
	$sql="SELECT id,date_,amount,DATE_FORMAT(date_,'%M') as month_desc FROM cash_deposit.0_fix_sales_aub WHERE br_code='$myBranchCode' and status=0";
	...
	while($row = db_fetch($result))
	{
		...
		$ref   = $Refs->get_next(0);
		$memo_ = "BANK RECON, AUB Sales Deposit for the month of ".$row['month_desc'];
		
		$trans_type = ST_JOURNAL;

		$trans_id = get_next_trans_no($trans_type);
		
			add_gl_trans($trans_type, $trans_id, $date_, 10102299, 0, 0, $memo_, $row['amount'],null,null,'');
		...
		$sql__="UPDATE cash_deposit.0_fix_sales_aub SET status=1 WHERE br_code='$myBranchCode' and id='$id'";
