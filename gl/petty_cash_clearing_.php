...
function insert_petty_cash_details($selected_add,$pc_id,$pcd_date,$pcd_payee,$pcd_purpose,$exp_type,$pcd_ref,$pcd_tin,$prepared_by_id,$prepared_by_name)
{
global $path_to_root;
...
VALUES ('$pc_id','".date2sql($pcd_date)."',".db_escape($pcd_payee).",".db_escape($pcd_purpose).",'$exp_type','$pcd_ref','$pcd_tin','".input_num('pcd_amount2'.$selected_add)."','".input_num('tax_amount2')."','$prepared_by_id',".db_escape($prepared_by_name).",'','','0','0000-00-00','0','1','$selected_add')";
...
function clear_petty_cash_details($selected_id,$pcd_gl_type,$pcd_payee,$pcd_purpose,$pcd_ref,$pcd_tin,$approved_by_id,$approved_by_name)	
{	
...
insert_petty_cash_details($selected_add,$pc_id,$pcd_date,$pcd_payee,$pcd_purpose,$exp_type,$pcd_ref,$pcd_tin,$approver_id,$approver_real_name);
...
clear_petty_cash_details($approve_add,$exp_type,$pcd_payee,$pcd_purpose,$pcd_ref,$pcd_tin,$approver_id,$approver_real_name);
...
if ($_POST['yes_no']==0) {
$th = array("TransNo","SEQ#","Date","Employee Name",'Payee','Purpose of Expense','Type','REF#','TIN#','Input Tax','Amount','','','');
}
else {
$th = array("TransNo","SEQ#","Date","Employee Name",'Payee','Purpose of Expense','Type','REF#','TIN#','Input Tax','Amount');
}
table_header($th);			
										...
										text_cells(null, 'pcd_payee2'.$row['pcd_id'],$row['pcd_payee'],18,20);
										text_cells(null, 'pcd_purpose2'.$row['pcd_id'],'',18,20);

										get_expenditure_type_list_cells('','exp_type2'.$row['pcd_id'],'',true,true);
										
if ($x=find_submit('exp_type2'.$row['pcd_id']))
{
global $Ajax;
//display_error('rerere');
//display_error($x);
//display_error($_POST['pcd_amount2'.$x]);
$selected_type=$_POST['exp_type2'.$x];
//display_error($selected_type);

$amount_to_deduct=$_POST['pcd_amount2'.$x];

$sql="select * from ".TB_PREF."expenditure_type where exp_id='$selected_type'";
$result=db_query($sql);
$row2=db_fetch($result);
$exp_ov=$row2['exp_ov'];

if ($exp_ov>0)
{
$vat=($amount_to_deduct/$exp_ov)-$amount_to_deduct;
}

//display_error($vat);

$_POST['tax_amount2']=$vat;

$Ajax->activate('tax_amount2');
}
										text_cells(null, 'pcd_ref2'.$row['pcd_id'],$row['pcd_ref'],8,20);
										text_cells(null, 'pcd_tin2'.$row['pcd_id'],$row['pcd_tin'],8,20);
										
										$tx=number_format2(abs($_POST['tax_amount2']),2);
										//display_error($tx);
										text_cells(null, 'tax_amount2',$tx,6,20);
										
										text_cells(null, 'pcd_amount2'.$row['pcd_id'],'',6,20);
										...
										left join ".TB_PREF."petty_cash_header as pch
										on pcd.pc_id=pch.pc_id";
										$sql.=" where (pcd.pcd_id='$create_breakdown' or pcd.pcd_head_id='$create_breakdown')";
										...
										left join ".TB_PREF."petty_cash_header as pch
										on pcd.pc_id=pch.pc_id";
										$sql.=" where (pcd.pcd_id='$selected_add' or pcd.pcd_head_id='$selected_add')";
										$sql.=" order by pcd.pcd_id asc";
										//display_error($sql);
										}
										...
										left join ".TB_PREF."petty_cash_header as pch
										on pcd.pc_id=pch.pc_id";
										$sql.=" where (pcd.pcd_id='$hidden_pc_id' or pcd.pcd_head_id='$hidden_pc_id')";
										$sql.=" order by pcd.pcd_id asc";
										//display_error($sql);
										}
										...
										$sql .= " WHERE (pcd.pcd_date>='$date_after' AND pcd.pcd_date<='$date_before')";
										...
										if ($_POST['yes_no']==0) {
													...
													label_cell($row['pc_employee_name']);
															...
															label_cell($row['pcd_payee']);
															label_cell($row['pcd_purpose']);
															label_cell(get_expenditure_name($row['pcd_gl_type']));
															label_cell($row['pcd_ref']);
															label_cell($row['pcd_tin']);
															label_cell($row['pcd_tax']);
															label_cell($row['pcd_amount']);
															...
															text_cells(null, 'pcd_payee'.$row['pcd_id'],$row['pcd_payee'],18,20);
															text_cells(null, 'pcd_purpose'.$row['pcd_id'],$row['pcd_purpose'],18,20);
															get_expenditure_type_list_cells('','exp_type'.$row['pcd_id'],'',true,true);
																											
if (list_updated('exp_type'.$row['pcd_id']))
{
global $Ajax;
//display_error($row['pcd_id']);
$selected_type=$_POST['exp_type'.$row['pcd_id']];
//display_error($selected_type);

$amount_to_deduct=$row['pcd_amount'];
...
$_POST['tax_amount'.$row['pcd_id']]=$vat;
$Ajax->activate('tax_amount'.$row['pcd_id']);
}

															
															text_cells(null, 'pcd_ref'.$row['pcd_id'],$row['pcd_ref'],8,20);
															text_cells(null, 'pcd_tin'.$row['pcd_id'],$row['pcd_tin'],8,20);
															//label_cell(number_format2(abs($row['pcd_tax']),2),'align=right');
															
															$tx=number_format2(abs($_POST['tax_amount'.$row['pcd_id']]),2);
															//display_error($tx);
															
															text_cells(null, 'tax_amount'.$row['pcd_id'],$tx,6,20);
