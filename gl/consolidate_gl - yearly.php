...
function create_month_ending($year, $branch_db_, $dimension_id, $branch_name)
{
	...
	// $memo_ = $_POST['memo'];
	$memo_ = 'import branch ending balance - ' . $year . " ($branch_name)";
	
	$sql = "SELECT account, SUM(round(amount,2)) as amt
				FROM $branch_db_.0_gl_trans
				WHERE tran_date >= '$year-01-01'
	...
	while($row = db_fetch($res))
		add_gl_trans($trans_type, $trans_id, $date_, $row['account'], $dimension_id, 0, $memo_, $row['amt']); 
...
if (isset($_POST['upload'])) 
{
	...
	foreach($db_connections as $key=>$db_con)
	{
		...
		$sql = "SELECT DISTINCT YEAR(tran_date) 
					FROM ".$db_con['dbname'].".0_gl_trans ORDER BY 1";
		// display_error ($sql);
		// continue;
		$res = db_query($sql);
...
start_form();
submit_center('upload','JV Ending Balance to consolidated branch');
end_form();
