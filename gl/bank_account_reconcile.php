...
// save also in hidden field for testing during 'Reconcile'
	return checkbox(null, $name, $value, false, _('Reconcile this transaction'))
		. hidden($hidden, $value, false);
}
...
function change_tpl_flag($reconcile_id)
{
	...
	{
		$sql3 =  "UPDATE ".TB_PREF."cheque_details SET status=".db_escape("Cleared")."
				WHERE bank_trans_id = 
					(SELECT trans_no FROM ".TB_PREF."bank_trans WHERE id =".db_escape($reconcile_id).")
				AND type = 
					(SELECT type FROM ".TB_PREF."bank_trans WHERE id =".db_escape($reconcile_id).")";
		db_query($sql3, "Can't change check status");	
...
if (get_bank_trans_type($_POST['bank_account']) == 1)
{
	$sql = "SELECT MAX(reconciled) as last_date,
			  ...
			  SUM(".TB_PREF."cheque_details.chk_amount) as total
		FROM ".TB_PREF."bank_trans 
		JOIN ".TB_PREF."cheque_details ON ".TB_PREF."cheque_details.bank_trans_id  = ".TB_PREF."bank_trans.trans_no 
		WHERE ".TB_PREF."bank_trans.bank_act = ".db_escape($_POST['bank_account']) . " 
		AND trans_date <= '$date' 
		AND ".TB_PREF."cheque_details.deposited = 1
		AND ".TB_PREF."cheque_details.type = ".TB_PREF."bank_trans.type
		AND ".TB_PREF."bank_trans.type != 12
...
if (get_bank_trans_type($_POST['bank_account']) == 1)
{
	$sql = "SELECT ".TB_PREF."bank_trans.type, trans_no,
					ref, ".TB_PREF."cheque_details.chk_number, ".TB_PREF."cheque_details.chk_date, 
					".TB_PREF."cheque_details.chk_amount, person_id, person_type_id, reconciled, ".TB_PREF."bank_trans.id
			FROM ".TB_PREF."bank_trans 
			JOIN ".TB_PREF."cheque_details ON ".TB_PREF."cheque_details.bank_trans_id  = ".TB_PREF."bank_trans.trans_no 
			WHERE ".TB_PREF."bank_trans.bank_act = ".db_escape($_POST['bank_account']) . "
			AND ".TB_PREF."cheque_details.deposited = 1
...
if (get_bank_trans_type($_POST['bank_account']) == 1)
{
	...
	array(
		...
		"Cleared"=>array('insert'=>true, 'fun'=>'rec_checkbox'),
		"Uncleared"=>array('align'=>'center','insert'=>true, 'fun'=>'che_checkbox')
	   );
}
else
{
...
// display_error(date("Y/m/d", $tomorrow));
