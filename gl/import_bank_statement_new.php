...
function clear_aria_cr_dr_trans($bank_statement_id,$date_paid,$credit_amount,$bank_gl_code)
{
	...
	$sql = "SELECT * FROM cash_deposit.".TB_PREF."cash_dep_header WHERE cd_trans_type='$trans_type' and cd_gross_amount='$credit_amount'  and cd_bank_account_code='$bank_gl_code' and cd_cleared='0' and (cd_sales_date>='2015-11-01' and cd_sales_date<='2015-12-31') order by cd_trans_date limit 1";
	$res = db_query($sql);
	...
	if ($bank_format_marker=='Date')
	{
		for ($i = 2; $i <= $excel_row; $i++) 
		{
			...
			if ($credit_amount>0 AND ($deposit_type==' CK3 - Check Deposit - Local' OR $deposit_type==' CD - Cash Deposit')){
				...
				if ($check_count<=0){
					$sql = "INSERT INTO cash_deposit.".TB_PREF."bank_statement_aub(date_deposited,deposit_type,credit_amount,balance)				
...
if (isset($_POST['upload'])) 
{
	...
	if ($_POST['bank_account']=='10102299'){
		...
		$sql="select * from cash_deposit.".TB_PREF."bank_statement_aub where cleared='0' and (date_deposited>='2015-11-01' and date_deposited<='2015-12-31') order by date_deposited";
		$res = db_query($sql);
		...
		$sql1="select * from cash_deposit.".TB_PREF."bank_statement_metro where cleared='0' and (date_deposited>='2015-11-01' and date_deposited<='2015-12-31') order by date_deposited";
		$res1 = db_query($sql1);
		...
		while($row = db_fetch($result))
		{
				...
				WHERE sdc.dc_remittance_date>='2015-11-01' AND sdc.dc_remittance_date<='2015-12-31'
				AND (sdc.dc_card_desc='METROBANK' OR sdc.dc_card_desc='metrobank')
				AND sdc.processed='0' AND sdc.paid='0'
