...
if (isset($_FILES['xls_file']) AND $_FILES['xls_file']['name'] != '' AND isset($_POST['upload'])) 
{
	...
	$sql = "SELECT DISTINCT type, type_no 
				FROM 0_gl_trans
				WHERE memo_ LIKE 'adjust 2014 and prior dates to zero'
				AND amount != 0
				AND type = 0";
	// display_error($sql);
	$res = db_query($sql);
	...
	{	
		$x = void_transaction('0', $row['type_no'], Today(), 'has computation error');
	...
	while($row = db_fetch($res))
	{	
		$x = void_transaction('0', $row['type_no'], Today(), 'has computation error - beg 2015');
		// display_error($x . $row['type_no']);
	}
	...
	$ref   = $Refs->get_next(0);
	// $memo_ = $_POST['memo'];
	$memo_ = 'adjust 2014 and prior dates to zero';
	...
	$sql = "SELECT account, SUM(amount) as amt
				FROM 0_gl_trans
				WHERE tran_date < '2015-01-01'
				GROUP BY account
				HAVING SUM(amount) != 0";
	$res = db_query($sql);
	
	while($row = db_fetch($res))
		add_gl_trans($trans_type, $trans_id, $date_, $row['account'], 0, 0, $memo_, -$row['amt']); //-----Updated variable name for memo, added underscore
...
if (!isset($upload_file) OR $upload_file != 'Yes')
{
	...
	textarea_row(_("Memo"), 'memo_', 'beg 2015', 50, 3);
	echo "</table><br>";
