...
if (isset($_POST['Process']))
{
	...
	*/	
	...
	//mhae start
	//display_error($_POST['new_old']);

		// $tran_date=sql2date($_POST['date_']);
	...
	}else if($_POST['new_old'] == 1) {
			//display_error($_POST['saf_no']);
		//die;
		// if (trim($_POST['purchaser']) == '')
		// {
			// display_error('Purchaser is required.');
			// return false;
		// }
		// if (trim($_POST['supp_id']) == '')
		// {
			// display_error('Supplier is required.');
			// return false;
		// }
		$sql = "SELECT * FROM srs_aria_nova.0_c_payment_saf_sign
				WHERE id =".$_POST['saf_no'];
				$res = db_query($sql);
				$row = db_fetch($res);
		
	//	$sql = "Update srs_aria_nova.0_counter set counter=counter+1 WHERE code = 'CP'";
	//	db_query($sql);
	
		$sql1 = "Update srs_aria_nova.0_c_payment_saf_sign set status=1 WHERE id =".$_POST['saf_no'];
		db_query($sql1);
	//	display_error('ok');
	//	exit;
	//	$ref = str_pad($row['counter'], 5, '000000', STR_PAD_LEFT);
		...
		$saf = $row['reference'];
		$sql = "INSERT INTO srs_aria_nova.0_c_payment_saf(reference,purchaserID) VALUES('".$saf."','".$row['purchaserID']."')";
		$query = db_query($sql);
		//display_error($sql);
		$sql = "INSERT INTO srs_aria_nova.0_c_payment_saf_supp(reference,supplierCode) VALUES('".$saf."','".$row['supplierName']."')";
		$query = db_query($sql);
		//display_error($sql);
		//die;
	}else{
		if (trim($_POST['saf_no']) == '')
		{
			display_error('SRSSAFCP is required.--');
			return false;
		}
		$sql = "SELECT reference FROM srs_aria_nova.0_c_payment_saf WHERE id = ".$_POST['saf_no'];
		$res = db_query($sql);
		$row = db_fetch($res);
		$saf = $row['reference'];
	...
	if ($_POST['rec_type']!='') {
		...
		//display_error($res[0]); die();

		if($res){
			if (!$_POST['saf_no'])
			{
				display_error(_("SAF NO. Field is Required"));
				$input_error = 1;
			}
	...
	/*if (!is_date_in_fiscalyear($_POST['date_']))
	{
		display_error(_("The entered date is not in fiscal year.".$_POST['date_']));
		set_focus('date_');
		$input_error = 1;
	}*/
...
}													 if (isset($_POST['Process']))
													{
														...
														if ($_POST['from_loc']!=''){
																...
																//display_error($c_id_str);
																	...
																	$other_income_memo = $_POST['memo_'];
																if($_POST['saf_no']){
																	//mhae
																	$other_income_memo = $saf.'-'.$_POST['memo_'];
																	//$other_income_memo = 'SAF '.$_POST['saf_no'].'-'.$_POST['memo_'];
																}else{
																	$other_income_memo = $_POST['memo_'];
																}	
															//	display_error($other_income_memo);
															//	die;
																...
																$_POST['ref'], $other_income_memo, true, $_SESSION['checks'],$_POST['or_no'],$saf,$_POST['paid_online']);
																}
																else { //without receivable
																$trans = add_other_income_process_payment_ho($c_id_str,$_SESSION['pay_items']->trans_type, $_POST['bank_account'],
																$_SESSION['pay_items'], $_POST['date_'],$_POST['PayType'], $_POST['person_id'], get_post('PersonDetailID'),
																$_POST['ref'], $other_income_memo, true, $_SESSION['checks'],$_POST['or_no'],$saf,$_POST['paid_online']);
																} 
															...
															if ($trans_no!='') {
																//display_error($trans_no);
															meta_forward($_SERVER['PHP_SELF'], "AddedDep=$saf");
															}
...
start_form();
	...
	/*if ($_POST['rec_type']!='') {
		...
		display_error($res[0].'-----'.$_POST['rec_type']);
		//hidden('acsaf', '1');
		$Ajax->activate('head');  
	}*/
...
display_payment_header_nova_upd($_SESSION['pay_items']);		
start_table("$table_style2 width=80%", 10);
echo "<td>";

//-----START OF CASH------
if ($_POST['payment_type']=='0' or $_POST['payment_type']=='3') {
