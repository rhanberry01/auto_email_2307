...
//AMOUNT============
	...
	$x++;
	$rep->sheet->writeNumber($rep->y, $x, abs($credit)+$amount1+$bank1+$bank2+$bank3+$bank4, $format_accounting);
	$x++;	
...
$sql="SELECT sum(bd_amount) as t_amount FROM `0_other_income_payment_header`
where bd_trans_date>= '".date2sql(add_months($start_date,-1))."'
and bd_trans_date<= '".date2sql(end_month($end_date2))."'
...
$sql="SELECT gl.account, sum(gl.amount) as t_amount 
FROM ".TB_PREF."gl_trans as gl where gl.type='$type'
";
...
label_cell("<font color=#880000><b>".number_format2(abs($credit)+$amount1+$bank1+$bank2+$bank3+$bank4,2)."<b></font>",'align=right');
