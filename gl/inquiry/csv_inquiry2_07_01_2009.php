...
if (isset($_POST['send_all'])) {
	$count = 0;
	if (count($_POST['csv_batch_id_array']) > 0) {
		if($count++ == 0) {
			foreach ($_POST['csv_batch_id_array'] as $csv_batch_id_all => $value) {
				$data = explode('-', $csv_batch_id_all);
				$csv_batch_id = $data[0];
				$type = $data[1];
				...
				if ($type == 1) {
					...
					while($row = db_fetch($res))
					{
						$message = send_that_cv($row['id'],$row['bank_trans_id']);
					} 
				}

				if ($type == 5) {
					$res = get_csv_details_pbcom($csv_batch_id);
					while($row = db_fetch($res))
					{
						$message = send_that_cv($row['id'],$row['bank_trans_id']);
					} 
				...
				if ($type == 6) {
					$res = get_csv_details_pbcomaca($csv_batch_id);
					while($row = db_fetch($res))
					{
						$message = send_that_cv($row['id'],$row['bank_trans_id']);
					} 
