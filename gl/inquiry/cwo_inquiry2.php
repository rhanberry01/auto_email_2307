...
array_push($th);
	...
	$sql="SELECT a.*, c.supp_name, st.supp_reference, st.special_reference 
		...
		AND a.person_type_id = 3) LEFT JOIN 0_supp_trans as st ON a.type=st.type 
		AND a.type_no=st.trans_no WHERE a.tran_date >= '".date2sql($_POST['end_date'])."'
		AND a.tran_date <= '".date2sql($_POST['start_date'])."' AND a.account = '1440' 
		AND a.person_id='".$_POST['supplier_id']."'
		ORDER BY a.tran_date, a.counter";
...
$from_head = array('','Date', 'Supplier', 'Trans#','PO/PR#','CV#', 'Delivery Date','Amount Paid','Amount Delivered','Debit Memo','GL');
$th = array_merge($from_head);
array_push($th);
	 table_header($th);
...
//==========AMOUNT
	...
	$sql="SELECT a.*, c.supp_name, st.supp_reference, st.special_reference,st.cv_id,st.del_date
		FROM 0_gl_trans a JOIN 0_chart_master b ON a.account = b.account_code 
		LEFT OUTER JOIN 0_suppliers c ON (a.person_id = c.supplier_id 
		AND a.person_type_id = 3) LEFT JOIN 0_supp_trans as st ON a.type=st.type 
		AND a.type_no=st.trans_no WHERE a.tran_date >= '".date2sql($_POST['start_date'])."'
		AND a.tran_date <= '".date2sql($_POST['end_date'])."' AND a.account = '1440' 
		and amount!=0
		AND a.person_id='".$_POST['supplier_id']."'
		ORDER BY c.supp_name,a.tran_date, a.counter";
		//display_error($sql);
...
label_cell(get_trans_view_str(20,$row['type_no'],$row['type_no']));

if($row["special_reference"]=='' OR $row["special_reference"]=='0'){
label_cell(($row['supp_reference']),'nowrap');
}
else{
label_cell(($row['special_reference']),'nowrap');
}
...
if ($row['cv_id']==0 or $row['cv_id']=='' or IS_NULL($row['cv_id']) and $row['type']==20 ){
label_cell("");
}
else{
label_cell(cv_no_link($row['cv_id'],$cv_num));
}
label_cell(sql2date($row['del_date']),'nowrap');
// label_cell(($row['reference']),'nowrap');

if($row['type']==20){
	amount_cell($row['amount'],false);
	$advance_payment+=$row['amount'];
}
else{
	amount_cell(0,false);
}
...
label_cell("<font color=#880000><b>".number_format2(abs($advance_payment),2)."<b></font>",'align=right');
label_cell("<font color=#880000><b>".number_format2($amount_delivered,2)."<b></font>",'align=right');
