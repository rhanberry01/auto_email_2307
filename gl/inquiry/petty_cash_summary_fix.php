...
$sql="SELECT * FROM 0_petty_cash_details where (pcd_date>='$date_from'  and pcd_date<='$date_to') 
AND pcd_id NOT IN (
...
)  and pcd_w_cv='0' and pcd_replenished='1' ";
...
)  AND pcd.pcd_replenished='1' AND pcd_wid_breakdown='0' GROUP BY pcd_gl_type";
//display_error($sql);
$res = db_query($sql);
$no_of_gl=db_num_rows($res);
$gls=array();
...
{
	$sql .= " WHERE (pcd.pcd_id LIKE ".db_escape('%'.$_POST['trans_no'].'%')." )";
}
...
) AND pcd.pcd_replenished='1' AND pcd_wid_breakdown='0' GROUP BY pcd_tax_type";
//display_error($sql);
$res = db_query($sql);
$no_of_gl=db_num_rows($res);
...
) AND gl.type='$type' AND gl.account NOT IN (1410,1410010,1410011,1410012,1410013) AND gl.amount>0  AND pcd.pcd_replenished='1' AND pcd_wid_breakdown='0'";
