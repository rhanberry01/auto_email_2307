...
$invoice_no = add_supp_trans(ST_SUPPINVOICE, $supplier_id,Today(),Today(),$_POST['reference'],'',input_num('t_amount'), 0, 0,"",0,0,Today(),1,0,0);
...
where type='59' 
and amount>0 and tran_date>='$date_from' and tran_date<='$date_to'
and posted='0'";
...
add_gl_trans_supplier(ST_SUPPINVOICE, $invoice_no, Today(), $account, 0, 0,$amount, $supplier_id, "", $rate);
}

add_gl_trans_supplier(ST_SUPPINVOICE, $invoice_no, Today(), $accounts_payable, 0, 0,-input_num('t_amount'), $supplier_id, "", $rate);

$cv_id=auto_create_cv($invoice_no);

add_comments(ST_SUPPINVOICE, $invoice_no, Today(), $memo_="Petty Cash ".$d1." - ".$d2);
