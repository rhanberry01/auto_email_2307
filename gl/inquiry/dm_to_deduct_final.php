...
$from_head = array('<input type="checkbox" id="select_all">',"","Supplier","Total APV (Present)","DM","STATUS");
...
$sql="SELECT supp_name,ROUND(ABS(SUM(ov_amount)),2) as amount 
FROM (SELECT sti.*, gli.amount, spi.supp_ref,spi.supp_name FROM 0_supp_trans as sti
LEFT JOIN 0_gl_trans as gli
ON sti.trans_no=gli.type_no
LEFT JOIN 0_suppliers as spi
ON sti.supplier_id=spi.supplier_id
where sti.type=53 and gli.type=53
and gli.account='5700033'
and sti.ov_amount!=0
) as a
GROUP BY supp_name";
// display_error($sql);
...
while($row = db_fetch($res))
{
	...
	label_cell(number_format2($t_apv=get_pen_apv($row['supp_name'])));
	label_cell(number_format2($amount=$row['amount']));
