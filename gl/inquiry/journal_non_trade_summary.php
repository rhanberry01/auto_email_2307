...
page(_($help_context = "Journal Non-trade Breakdown"), false, false, "", $js);
...
$date_before = date2sql($_POST['end_date']);
    ...
    $rep = new FrontReport(_('Journal Non-trade Breakdown'), "Journal Non-trade Breakdown", "LETTER");
	...
	$rep->y ++;
	$rep->sheet->writeString($rep->y, 0, 'Journal Non-trade (Breakdown)', $format_bold);
	$rep->y ++;
...
//gl header
$sql="select pcd_gl_type from ".TB_PREF."journal_non_trade_details as pcd
...
//tax header
$sql="select pcd_tax_type from ".TB_PREF."journal_non_trade_details as pcd
left join ".TB_PREF."journal_non_trade_header as pch
on pcd.pc_id=pch.pc_id";
...
$k = 0;
$type=ST_JOURNAL_NT;
$sql="select pcd.*,pch.pc_employee_name,gl.account, gl.amount from ".TB_PREF."journal_non_trade_details as pcd
left join ".TB_PREF."journal_non_trade_header as pch
on pcd.pc_id=pch.pc_id
...
end_table();
br();
display_heading("Journal Non-trade Summary from ".$_POST['start_date']." to ".$_POST['end_date']."");	
br();	
br();
//====================================end of heading=========================================
...
{
	$sql = "SELECT max(tran_date) as tran_date FROM ".TB_PREF."gl_trans_temp where type='73' and posted='1'";
	$result = db_query($sql);
	$row=db_fetch($result);
...
function new_journal_non_trade_cv($myBranchCode)
{
global $Refs;	
begin_transaction();
$d1=$_POST['date1'];
...
add_supp_invoice_item(ST_SUPPINVOICE, $invoice_no, $stock_id+0, $description='',$account, $amount, $unit_tax+0, $quantity+0, $grn_item_id+0, $po_detail_item_id+0, $memo_="Journal NT ".$d1." - ".$d2,$err_msg="", $i_uom='',  $multiplier=1);
...
//update cv status from gl_trans_temp and journal_non_trade_details.
$sql="SELECT * FROM 0_journal_non_trade_details where (pcd_date>='$date_from'  and pcd_date<='$date_to') and pcd_w_cv='0' and pcd_replenished='1' ";
...
{
		$pcd_id=$row['pcd_id'];
		$sql_update_pcd = "UPDATE ".TB_PREF."journal_non_trade_details SET 
				pcd_w_cv = 1, pcd_cv_id = '".$cv_id."'
				WHERE pcd_id='".$pcd_id."'";
		//display_error($sql_update_pcd);
		db_query($sql_update_pcd,'failed to update journal_non_trade_details.');
...
add_comments(ST_SUPPINVOICE, $invoice_no, $apv_cv_date, $memo_="Journal NT ".$d1." - ".$d2);
...
// else {
$myBranchCode=$db_connections[$_SESSION["wa_current_user"]->company]["br_code"];	
new_journal_non_trade_cv($myBranchCode);
//}
}
