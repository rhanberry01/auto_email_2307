...
{
	return get_trans_view_str($row["type"], $row["type_no"], $row['reference']);
}
...
function edit_link($row)
{
	...
	if($row["type"] == 0 ){
		...
		if (strpos($row['reference'], '-2018') !== false) {
			$link = $editors[$row["type"]];
		}else{
			$link = "/gl/gl_journal_2017.php?ModifyGL=Yes&trans_no=%d&trans_type=%d";
		}
	}
	return isset($link) && !is_closed_trans($row["type"], $row["type_no"]) ? 
		pager_link(_("Edit"), 
			sprintf($link, $row["type_no"], $row["type"]),
			ICON_EDIT) : '';
}
/*
...
$table =& new_db_pager('journal_tbl', $sql, $cols,null,null,20);
