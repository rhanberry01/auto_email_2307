...
if ($_POST['receipt_type']!= '' and $_POST['receipt_type']!= 0)
{
...
$from_head = array('','Date Paid', 'Trans #','RF/SI #','OR', 'Payee','Description','Type','Date Deposited','VAT','Wtax');
...
$data[$row['bd_trans_no']] ['bd_official_receipt']=$row['bd_official_receipt'];
...
//change bd_det_amount to bd_oi or bd_amount if account is not tally.
...
foreach($data as $emp_id=>$details) {
		...
		$x++;		
		$rep->sheet->writeString($rep->y, $x, $details['bd_official_receipt'],$format_left);
		$x++;
...
label_cell($details['bd_official_receipt'] ,'nowrap');
label_cell($details['payee'] ,'nowrap');
label_cell($details['desc'] ,'nowrap');
label_cell($details['payment_type']);

if ($details['date_deposited']> date2sql($_POST['end_date'])){
label_cell("<font color='red'><b>".sql2date($details['date_deposited'])."</b></font>");
}
else{
