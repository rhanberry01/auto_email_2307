...
$update_id = find_submit('update_selected',false);
...
if ($update_id != "")
{
 $update_id_data = explode("-",$update_id);


 $update_id = $update_id_data[0];
 $upd_branch = $update_id_data[1];
...
display_notification
 ...
 display_notification(_("The Transaction has been Updated."));
...
foreach ($all_branch as $key => $row) 
{
	...
	alt_table_row_color($k);
	 if (($row['bd_trans_date']==Today()) OR ($_SESSION['wa_current_user']->username == 'admin') OR ($_SESSION['wa_current_user']->username == 'jenT')){
		...
		if (!isset($_POST['date_'.$row['bd_id']])){
			$_POST['date_'.$row['bd_id']]=sql2date($row['bd_trans_date']);
		}
			date_cells('', 'date_'.$row['bd_id'], '', null);
	...
	$submit='delete_selected'.$row['bd_id'].'-'.$row['branch_db'];
	 ...
	 $submit='update_selected'.$row['bd_id'].'-'.$row['branch_db'];
	 if (($row['bd_trans_date']==Today()) OR ($_SESSION['wa_current_user']->username == 'admin') OR ($_SESSION['wa_current_user']->username == 'jenT')){
		submit_cells($submit, 'Update', "align=center", true, true,'ok.gif');
	 } else {
	  label_cell('');
	 }
