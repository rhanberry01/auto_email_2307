...
if (get_bank_trans_type($_POST['bank_account']) == 1)
{
	$sql = "SELECT ".TB_PREF."bank_trans.*, chk_date, chk_amount, chk_number, bank
		...
		AND chk_date >= '$date_after'
		AND chk_date <= '$date_to'
...
if (get_bank_trans_type($_POST['bank_account']) == 1)
	$th = array(_("Type"), _("Trxn #"), /*_("Reference"),*/ _("Check #"), _("Date"), _("Debit"), _("Credit"), _("Balance"), _("Person/Item"), "");
else
	$th = array(_("Type"), _("Trxn #"), /*_("Reference"),*/ _("Date"), _("Debit"), _("Credit"), _("Balance"), _("Person/Item"), "");
...
while ($myrow = db_fetch($result))
{
	...
	if (get_bank_trans_type($_POST['bank_account']) == 1)
	{
		$amount = $myrow["chk_amount"];
		$running_total += $myrow["chk_amount"];
		$trandate = sql2date($myrow["chk_date"]);
	}
	else
	{
		$amount = $myrow["amount"];
