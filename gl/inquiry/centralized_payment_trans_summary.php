...
$sql_t="SELECT sum(abs(amount)) as gl_amount FROM `0_gl_trans`
where type='22'
and account='10102299'
AND tran_date >='".date2sql($_POST['start_date'])."' AND  tran_date <='".date2sql($_POST['end_date'])."'";
//display_error($sql_t);
$res_t = db_query($sql_t);
$trow = db_fetch($res_t);
$tb_amount=$trow['gl_amount'];

$sql_t="SELECT sum(chk_amount) as reconciled FROM cash_deposit.0_centralized_payment
WHERE reconciled!='0000-00-00'
AND bank_trans_date >='".date2sql($_POST['start_date'])."' AND  bank_trans_date <='".date2sql($_POST['end_date'])."'
AND br_code='$br_code'";
//display_error($sql_t);
$res_t = db_query($sql_t);
$trow = db_fetch($res_t);
$reconciled=$trow['reconciled'];

$sql_t="SELECT sum(chk_amount) as unreconciled FROM cash_deposit.0_centralized_payment
WHERE reconciled='0000-00-00'
AND bank_trans_date >='".date2sql($_POST['start_date'])."' AND  bank_trans_date <='".date2sql($_POST['end_date'])."'
AND br_code='$br_code'";
//display_error($sql_t);
$res_t = db_query($sql_t);
$trow = db_fetch($res_t);
$unreconciled=$trow['unreconciled'];


$sql_t="SELECT sum(chk_amount) as beginning FROM cash_deposit.0_centralized_payment
WHERE reconciled='0000-00-00'
AND bank_trans_date >='2015-01-01' AND  bank_trans_date <='".date2sql(add_months($_POST['end_date'],-1))."'
AND br_code='$br_code'";
//display_error($sql_t);
$res_t = db_query($sql_t);
$trow = db_fetch($res_t);
$beginning=$trow['beginning'];
...
$th = array(_("Beginning <br> Balance"),_("Cash in Bank <br>  AUB"),_("Reconciled <br> Checks"),_("Outstanding <br> Checks"));
table_header($th);
start_row();
amount_cell($beginning);
amount_cell($tb_amount);
amount_cell($reconciled);
amount_cell($unreconciled);
end_row();
end_table();
div_end();
br();
...
// if (!isset($_POST['start_date']))
	...
	if ($_POST['type']==1 and isset($_POST['search'])) { //AUB checks
				$db_='bank_statement_aub_2015';
...
// AND c.tran_date >='".date2sql($_POST['start_date'])."' AND  c.tran_date <='".date2sql($_POST['end_date'])."'";
	...
	$sql="SELECT  * from cash_deposit.".TB_PREF."centralized_payment";

	if ($_POST['recon_status']==1) {
	$sql.=" WHERE bank_trans_date >='".date2sql($_POST['start_date'])."' AND  bank_trans_date <='".date2sql($_POST['end_date'])."' AND !(ISNULL(reconciled) OR reconciled='' OR  reconciled='0000-00-00')";
	$sql .= "  AND br_code='$myBranchCode'";
	}
	else if ($_POST['recon_status']==2) {
	$sql.=" WHERE bank_trans_date >='".date2sql($_POST['start_date'])."' AND  bank_trans_date <='".date2sql($_POST['end_date'])."' AND (ISNULL(reconciled) OR reconciled='' OR  reconciled='0000-00-00')";
	$sql .= "  AND br_code='$myBranchCode'";
	...
	if ($_POST['type']==1) { //AUB checks
	$sql.=" AND !ISNULL(chk_number) order by chk_amount desc";
	}
	else if ($_POST['type']==2) { //Metrobank Online Payment
	$sql.=" AND ISNULL(chk_number) order by chk_amount desc";
	}
...
while($row = db_fetch($res))
{
	...
	label_cell($c,'align=right');
	label_cell(sql2date($row['supp_trans_date']));
	if ($row['reconciled']){
...
//	label_cell($row['cv_no']);
	label_cell(cv_no_link($row['cv_id'],$row["cv_no"],$connect_to));
	if ($row['chk_number']){
