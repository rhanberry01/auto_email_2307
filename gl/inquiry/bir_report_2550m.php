...
page(_($help_context = "BIR Report 2550M"), false, false, "", $js);
...
function load_vat_return_values($from_date, $to_date){
	...
	$oth_income_array = array("'4020'",
										...
										"'4020063'",
										"'4020070'");
	$capital_goods_array = array("'1500'",
											"'1510'",
											"'1710'",
											"'1530'",
											"'1730'",
	...
	foreach($db_connections as $key=>$db_con)
	{
		...
		$sql = "(SELECT 
						...
						SUM(IF(account = '4000', amount, 0))AS 'gross_sales_amt', 
						SUM(IF(account = '4900', amount, 0))AS 'suki_pts_amt', 
						SUM(IF(account = '4000020', amount, 0))AS 'sales_vat_exempt_amt', 
						SUM(IF(account = '4000025', amount, 0))AS 'vat_zero_rated_amt', 
						SUM(IF(account = '23102318', amount, 0))AS 'output_vat_amt', 
						SUM(IF(account = '4000010', amount, 0))AS 'wholesale_retail_sales_vat_amt', 
						SUM(IF(account IN (".implode(',', $oth_income_array)."), amount, 0))AS 'business_services_amt', 
						SUM(IF(account = '4030', amount, 0))AS 'real_property_lease_amt', 
						SUM(IF(account = '5450', amount, 0))AS 'purchases_vat_amt', 
						SUM(IF(account = '5400', amount, 0))AS 'purchases_vat_exempt_amt', 
						SUM(IF(account = '1410100', amount, 0))AS 'excess_input_vat_amt',
						SUM(IF(account = '141000409', amount, 0))AS 'deferred_input_vat_amt',
						SUM(IF(account IN (".implode(',', $capital_goods_array).") AND amount > 0, amount, 0))AS 'capital_goods_amt',
						SUM(IF(account = '2310010', amount, 0))AS 'vat_payable_amt'
		...
		$sql = "SELECT branch, SUM(gross_sales_amt) as gross_sales_amt, 
								SUM(suki_pts_amt) as suki_pts_amt, 
								SUM(sales_vat_exempt_amt) as sales_vat_exempt_amt, 
								SUM(vat_zero_rated_amt) as vat_zero_rated_amt, 
								SUM(output_vat_amt) as output_vat_amt, 
								SUM(wholesale_retail_sales_vat_amt) as wholesale_retail_sales_vat_amt, 
								SUM(business_services_amt) as business_services_amt, 
								SUM(real_property_lease_amt) as real_property_lease_amt, 
								SUM(purchases_vat_amt) as purchases_vat_amt, 
								SUM(purchases_vat_exempt_amt) as purchases_vat_exempt_amt, 
								SUM(excess_input_vat_amt) as excess_input_vat_amt, 
								SUM(deferred_input_vat_amt) as deferred_input_vat_amt, 
								SUM(capital_goods_amt) as capital_goods_amt, 
								SUM(vat_payable_amt) as vat_payable_amt FROM (";
		...
		$sql .= ") xxx
					GROUP BY branch";
		$res = db_query($sql,'error.');
		// display_error("FOR BIR 2550---".$sql);
		return $res;
}
function load_purchases($is_service,$branch, $from_date, $to_date){
	...
	// $others = array('6110','6160','6280','6300','6112'); //original
	$others = array('6110','6160','6280','6300','6112','6010','6150','6190','8000');
	
	// array_push($others,implode(',', $others_serv));
