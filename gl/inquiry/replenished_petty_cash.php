...
page(_($help_context = "Replenished Petty Cash"), false, false, "", $js);
...
$sql = "SELECT DISTINCT glt.memo_ as gl_trans_temp_cv_id,st.trans_no as apv_trans_no,
co.memo_ as pc_desc,cv_h.cv_date as cv_date, cv_h.bank_trans_id as bank_trans_id,cv_h.bank_trans_id as cv_number, 
cd.chk_number as chk_number, cd.chk_amount as chk_amount,cd.pay_to as chk_pay_to
FROM 0_gl_trans_temp as glt
LEFT JOIN 0_supp_trans as st
ON glt.memo_=st.cv_id 
LEFT JOIN 0_comments as co
ON st.trans_no=co.id
LEFT JOIN 0_cv_header as cv_h
ON st.cv_id=cv_h.id
LEFT JOIN 0_cheque_details as cd
ON cv_h.bank_trans_id=cd.bank_trans_id
WHERE glt.type='59' AND glt.posted='1'
AND st.type='20' AND co.type='20'
AND cv_h.amount!='0'";

if (trim($_POST['trans_no']) == '')
{
	$sql .= " AND cv_h.cv_date >= '".date2sql($_POST['start_date'])."'
			  AND cv_h.cv_date <= '".date2sql($_POST['end_date'])."'";
}
else
{
	$sql .= " AND (cv_h.cv_date LIKE ".db_escape('%'.$_POST['trans_no'].'%')." )";
}
...
$th = array();
array_push($th,'', 'CV Date','CV#', 'APV#','Check#','Payee','Remarks','Amount','');


if (db_num_rows($res) > 0){
	br();
	display_heading("Replenished Petty Cash Summary from ".$_POST['start_date']." to ".$_POST['end_date']."");
	br();
	br();
...
start_form();
	...
	label_cell(cv_no_link($row['gl_trans_temp_cv_id'],$row["cv_number"]));
	label_cell(get_trans_view_str(20,$row['apv_trans_no'],$row['apv_trans_no']));
	//label_cell($row['apv_trans_no'] ,'nowrap');
	label_cell($row['chk_number'] ,'nowrap');
	label_cell($row['chk_pay_to'] ,'nowrap');
	label_cell($row['pc_desc'] ,'nowrap');
	amount_cell($row['chk_amount'],false);
	label_cell(viewer_link('View', "gl/view/petty_cash_with_vat_view.php?cv_id=".$row['gl_trans_temp_cv_id']."&apv_trans_no=".$row['apv_trans_no'], '', '', ''),'align=center');
	end_row();
end_form();

$t_amount+=$row['chk_amount'];
