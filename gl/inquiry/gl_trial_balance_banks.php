...
function gl_inquiry_controls()
{
	...
	{
		$_POST['TransFromDate'] = '01/01/2016';
		$_POST['TransToDate'] = '12/31/2016';
	}
...
function display_trial_balance()
{
	...
	$from_x = array('1410010','5400','5450');
	
	$from_x = array('1010040', '10102299', '1020021');
	$from_x = array('1010');
	
	$accounts = get_gl_accounts_array($from_x);
	...
	while ($account = db_fetch($accounts))
	{
		...
		if (check_value('Balance'))
		{
			...
			if($prev['balance']>=0)
				...
				$pcre+=abs($prev['balance']);

			if($curr['balance']>=0)
				$cdeb+=$curr['balance'];
			else
				$ccre+=abs($curr['balance']);
			
			if($tot['balance']>=0)
				$tdeb+=$tot['balance'];
			else
				$tcre+=abs($tot['balance']);
