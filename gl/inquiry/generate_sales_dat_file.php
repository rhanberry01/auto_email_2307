...
page(_($help_context = "For VAT Relief (Sales)"), false, false, "", $js);
...
function load_other_income($from_date, $to_date){
	...
	foreach($db_connections as $key=>$db_con)
	{
		...
		$sql = "
					(SELECT
						'".date2sql($to_date)."' as nt_date , bd_tin, bd_payee, bd_memo, bd_receipt_type, SUM(bd_oi) as tot_amount, SUM(bd_vat) as tot_tax
					FROM
	...
	$sql = "SELECT '".date2sql($to_date)."' as nt_date , bd_tin, bd_payee, bd_memo, bd_receipt_type, SUM(tot_amount) as tot_amount, SUM(tot_tax) as tot_tax FROM ("; /* Original */
	/* $sql = "SELECT '".date2sql($to_date)."' as nt_date ,bd_payee, bd_memo, bd_amount as tot_amount, bd_wt as tot_tax FROM ("; */
...
if(isset($_POST['submit_btn']))
{
	...
	$con[0] = array('H','S',"007492840","SRS RETAIL SYSTEMS INC.","","","",
	...
	$taxable_sales_amt = $purch_services_amt = $purch_capital_goods_amt = $purchase_non_vat_amt =
	$purch_goods_other_than_capital_goods = $output_tax_amt = $gross_taxable_purch_amt = 0;
	
	$tot_gross_purchase = $tot_output_tax_amt = $tot_purchase_non_vat_amt = 
	...
	$tot_gross_purchase2 = $tot_output_tax_amt2 = $tot_purchase_non_vat_amt2 = 
	...
	$res = load_other_income($_POST['from_date'], $_POST['to_date']);
	while($row = db_fetch($res)){
		$counter++;
		
		$supp_name = $row['bd_payee'];
		// $contact = $row['contact'];
		$address = '';
		
		$tin = str_replace("-", "", trim($row['bd_tin']));
		$tin = substr($tin, 0, 9);
		$tin = addslashes($tin);
		
		// $tin = ''; /* TIN not retrievable in Other Income Query because it's not included in the other income table */
		
		$taxable_sales_amt = round($row['tot_amount'],2);
		// $purchase_non_vat_amt = round($row['p_n_vat'],2); //original
		$purchase_non_vat_amt = 0;
		$purch_goods_other_than_capital_goods = round($row['tot_amount'],2);
		// $output_tax_amt = round($row['i_tax'],2); //original
		$output_tax_amt = round($row['tot_tax'],2);
		
		$gross_purchase_amt = ($taxable_sales_amt + $purchase_non_vat_amt);
		$gross_taxable_purch_amt = ($taxable_sales_amt + $output_tax_amt);
		...
		/* Original */
		$con[] = array('D','S','"'.$tin.'"','"'.strtoupper($supp_name).'"',"","",'"'.$address.'"',"",0,0,$taxable_sales_amt,$output_tax_amt,get_company_pref('gst_no'),date('m/d/Y', strtotime($_POST['to_date'])));
	...
	// while($row2 = db_fetch($res2)){
		...
		// }
		// $gross_purchase_amt2 = ($taxable_sales_amt2 + $exempt_purchase_amt2);
		
		// /* enddate | tin | regname | suppname | suppaddress | grosspurchamt | exemptpurchamt | zeroratedpurch | taxablepurch | purchsvc | purchcapgoods | purchgoodsthancapgoods | inputtax | grosstaxpurch */
		
		// /* Original */
		// /* $con[] = array('D','P','"'.$tin2.'"','"'.strtoupper(html_entity_decode($supp_name2)).'"',"","",'"'.strtoupper(html_entity_decode($address2)).'"',"",0,0,0,0,$gross_purchase_amt2,$output_tax_amt2,get_company_pref('gst_no'),date('m/d/Y', strtotime($_POST['to_date']))); */
		// /* Latest */
		// if($row2['tot_tax'] == 0){
		...
		// }else{
			// $con[] = array('D','P','"'.$tin2.'"','"'.strtoupper(html_entity_decode($supp_name2)).'"',"","",'"'.strtoupper(html_entity_decode($address2)).'"',"",0,0,0,0,$taxable_sales_amt2,$output_tax_amt2,get_company_pref('gst_no'),date('m/d/Y', strtotime($_POST['to_date'])));
		// }
	...
	$con[0] = array('H','S',"007492840","SRS RETAIL SYSTEMS INC.","","","",
							"SRS RETAIL SYSTEMS INC","DUMALAY ST. CORNER QUIRINO HIGHWAY ",
							"NOVALICHES QUEZON CITY 1123",
							'0.00','0.00',($tot_taxable_sales_amt),($tot_output_tax_amt),'028',
							date('m/d/Y', strtotime($_POST['to_date'])),12);
	...
	$filename = get_company_pref('gst_no')."S".date('m', strtotime($_POST['to_date']))."".date('Y', strtotime($_POST['to_date'])).".DAT";
	...
	div_start('content');
		...
		if(isset($_POST['display_btn'])){
			//-----FOR TRADE PURCHASES
				...
				// start_table($table_style2);
					...
					// while($row = db_fetch($res)){
						...
						// /* Input Tax Amount */
						// // $output_tax_amt = ($gross_purchase_amt * 0.12)+0;
						...
						// $gross_purchase_amt = ($taxable_sales_amt + $exempt_purchase_amt);
						// $gross_taxable_purch_amt = ($taxable_sales_amt + $output_tax_amt);
						...
						// alt_table_row_color($k);
							...
							// amount_cell($taxable_sales_amt); //With Value (Amount is equal to Gross Purchase Amount)
							...
							// amount_cell($output_tax_amt); //With Value (Gross Purchase Amount * 0.12 = Input Tax Amount)
					...
					// amount_cell($tot_taxable_sales_amt, true);
					...
					// amount_cell($tot_output_tax_amt, true);
			...
			$counter2 = 0;
				...
				display_heading('RENTALS AND OTHER INCOME');
				start_table($table_style2);
					// $th = array ('#', 'TIN', 'REGISTERED NAME', 'SUPPLIER\'S ADDRESS', 'GROSS PURCHASE AMT', 'EXEMPT PURCHASE AMT', 'ZERO-RATED AMT', 'TAXABLE PURCHASE AMT', 'PURCHASE OF SERVICES AMT', 'PURCHASE OF CAPITAL GOODS AMT', 'PURCHASE OF GOODS OTHER THAN CAPITAL GOODS AMT', 'INPUT TAX AMT', 'GROSS TAXABLE PURCHASE AMT'); //original
					$th = array ('#', 'Taxable Month', 'TIN', 'Registered Name', 'Customer\'s Address', 'Amount of Gross Sales', 'Amount of Taxable Sales', 'Amount of Output Tax', 'Amount of Gross Taxable Sales');
					
					// $res2 = load_non_trade_purchases($_POST['from_date'], $_POST['to_date']);
					$res2 = load_other_income($_POST['from_date'], $_POST['to_date']);
					while($row2 = db_fetch($res2)){
						...
						$tin2 = str_replace("-", "", trim($row2['bd_tin']));
						$tin2 = substr($tin2, 0, 9);
						$tin2 = addslashes($tin2);
						
						// $tin2 = ''; //Temporarily no TIN for display
						...
						alt_table_row_color($k2);
							label_cell($counter2);
							label_cell(date2sql($_POST['to_date']));
							label_cell($tin2);
							label_cell(strtoupper($supp_name2));
							label_cell($address2);
							amount_cell($gross_purchase_amt2); //With Value
							amount_cell($taxable_sales_amt2);
							amount_cell($output_tax_amt2); //With Value (Gross Purchase Amount * 0.12 = Input Tax Amount)
							amount_cell($gross_taxable_sales_amt2); //With Value (Gross Purchase Amount + Input Tax Amount = Gross Taxable Purchase Amount)
						end_row();
					...
					alt_table_row_color($k);
					label_cell('     <b>TOTAL</b>', 'colspan = 5 align="right"');
					amount_cell($tot_gross_purchase2, true);
					amount_cell($tot_taxable_sales_amt2, true);
					amount_cell($tot_output_tax_amt2, true);
					amount_cell($tot_gross_taxable_sales_amt2, true);
					end_row();
					...
					// amount_cell(($tot_taxable_sales_amt + $tot_taxable_sales_amt2), true);
					...
					// amount_cell(($tot_output_tax_amt + $tot_output_tax_amt2), true);
					// amount_cell(($tot_gross_taxable_purch_amt + $tot_gross_taxable_sales_amt2), true);
					// end_row();
