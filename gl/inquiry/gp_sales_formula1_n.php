...
if (isset($_POST['export'])){
		...
		$th = array('','Alaminos','Antipolo1', 'Antipolo2', 'B Silang','Bagumbong','Cainta', 'Cainta 2', 'Camarin', 'Gagalangin', 'Imus', 'Comembo', 'Malabon','Molino','Muzon','Navotas','Novaliches','Pateros','Pedro','Pinas','Punturin','Tondo','Valenzuela');
	...
	$branch_arr = array('SRSMALAM','SRSMANT1GF', 'SRSMANT2EM','SRSMBAG','SRSMBSL','SRSMCAINTA', 'SRSMCAINTA2', 'SRSMCAMA', 'SRSMGAL', 'SRSMIMU', 'SRSMKUM', 'SRSMMALA','SRSMMOL','SRSMMUZ', 'SRSMNAVO','SRSMNOVA','SRSMPAT','SRSMPEDRO','SRSMPINAS','SRSMPUN','SRSMTON','SRSMVAL');
...
function insert_into_fsales($branch,$fsales){
	...
	if($resultc){
		$sql="UPDATE FSales
			  SET totalsales=".$tsales.",totalcsales=".$tcsales.",gp=".$gp.",gppercentage=".$gppercentage.",IsNet =".$IsNet.",sukipoints=".$sukipoints."
	...
	//die();
			...
			$sql = " INSERT INTO FSales(branch,ldate,totalsales,totalcsales,gp,gppercentage,IsNet,sukipoints) 
			VALUES('".$branch."','".$date."',".$tsales.",".$tcsales.",".$gp.",".$gppercentage.",".$IsNet.",".$sukipoints.")"; 
	...
	$ms_db = mssql_connect('192.168.0.133','markuser','tseug'); 
			...
			/*$sql = "select 
				...
				$sql = "
						...
						sum(totalcost) AS tcsales,
						sum(extended) as tsales,
						case when sum(totalcost) = 0 then 0 else (sum(extended) - (sum(totalcost)))/NULLIF(sum(extended),0)*100 end  as gppercentage,
						sum(extended) - (sum(totalcost)) as gp,
						(select sum(amount) as sukipoints  from FinishedPayments where tendercode ='004'and voided=0 and cast(LogDate as date) = cast(finishedsales.ldate as date)) as sukipoints
						From 
						(select 
						FinishedSales.LogDate as ldate ,
						...
						0 as TotalCost,0 as charges,0 as discount,totalqty as rettotalqty ,abs(round(extended,4)) as retextended,0 as totalqty, 0 as extendedn 
...
echo "<div style='overflow:auto; height:400px;'>";
		start_table($table_style2.'style=overflow-x:scroll; width=50%');
			...
			$th = array('','Alaminos','Antipolo1', 'Antipolo2','Bagumbong','B Silang', 'Cainta', 'Cainta 2', 'Camarin', 'Gagalangin', 'Imus', 'Comembo', 'Malabon','Molino','Muzon','Navotas','Novaliches','Pateros','Pedro','Pinas','Punturin','Tondo','Valenzuela');
			table_header($th,'colspan=6');
			
			alt_table_row_color($k);
			label_cell('Date','colspan=6');
			...
			//s	echo $count_r.'----------'.$count_s;
				if($count_r == $count_s)
				{
					label_cell($res['ldate'],'colspan=6');
					$branch_counter = 0;
				}
