...
// , bdc.id as bank_ids
	$sql = "
		select oip.*,bdc.*,bdc.remark as bank_remark  from ".$branches_res['aria_db'].".".TB_PREF."other_income_payment_header as oip
...
$count2  = 1;
// echo '<pre>';
// print_r($all_branch);die();
foreach ($all_branch as $key => $row) 
{

	$bd_trans_no = get_gl_view_str_per_branch($row['branch_db'],ST_BANKDEPOSIT, $row["bd_trans_no"], $row["bd_trans_no"]);
	alt_table_row_color($k);
	label_cell(sql2date($row['bd_trans_date']));
	label_cell($row['branch_name']);
	label_cell($bd_trans_no);
	...
	$submit='delete_selected'.$row['bd_id'].'-'.$row['branch_db'];
	 if (($row['bd_trans_date']==Today()) OR ($_SESSION['wa_current_user']->username == 'admin') OR ($_SESSION['wa_current_user']->username == 'jenT')){
	 ...
	 //jade 4/19/2018
	 if($row['id'] !=""){
		 echo '<td><input type="text" value="'.$row['bank_remark'].'" onkeyup="updateRemarks(event,'.$row['id'].','.$count1++.',this.value)" xbrcode="'.$row['branch_db'].'" id="xbrcode'.$count2++.'" name="remarks"></td>';
	 }else{
	 	echo '<td></td>';
	 }
	 // ref_cells('', 'remarks','', .$count++);
...
<script type="text/javascript">
	var url = "../../gl/includes/ajax_request.php"
	function updateRemarks(e,id,count,value){
		var br_code = $('#xbrcode'+count).attr('xbrcode');
		console.log(value)
