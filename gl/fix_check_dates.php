...
if (isset($_POST['Fix'])){
	...
	c.tran_date, g.tran_date as gl_tran_date, b.trans_date, cd.chk_date, c.cv_id,
	...
	AND b.type='22'
	AND c.ov_amount!=0 
	AND cd.chk_date>='2016-01-01' AND cd.chk_date <='2016-12-31'
	AND g.account='10102299'
	AND (cd.chk_date!=b.trans_date OR cd.chk_date!=g.tran_date)
	...
	{
		$sql2 = "UPDATE ".TB_PREF."bank_trans SET trans_date='".$row['chk_date']."' 
		WHERE id='".$row['b_id']."'";
		db_query($sql2,"Failed to update 0_bank_trans.");
		
		$sql4 = "UPDATE ".TB_PREF."gl_trans SET tran_date='".$row['chk_date']."' 
	...
	display_notification("Updating gl tran_date and bank_trans trans_date is successful!");
...
start_form();
start_row();
submit_center('Fix',_("Fix Dates"), true, '', false);
end_table();
end_form();
end_page();
?>