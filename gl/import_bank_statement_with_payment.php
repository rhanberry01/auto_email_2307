...
function get_bt_cheque_details($chk_number,$debit_amount)
{
	$sql = "SELECT * FROM ".TB_PREF."cheque_details
			WHERE chk_number = '$chk_number' AND chk_amount='$debit_amount'";
	$res = db_query($sql);
	$row = db_fetch($res);
...
{
$type=ST_BANKDEPOSIT;
	$sql = "UPDATE ".TB_PREF."bank_trans SET bank_act = '$bank_account', trans_date='$date_paid',reconciled='1'
		...
		for ($i = 2; $i <= $excel_row; $i++) 
		{
			...
			if ($credit_amount!=0 AND $credit_amount>0){
				...
				if ($check_count<=0){
					...
					VALUES ('".date2sql($date_deposited)."',".db_escape($bank_ref_num).",".db_escape($deposit_type).",'$debit_amount','$credit_amount','$balance')";		
					//display_error($sql);
...
function handle_new_metro_excel_item()
{
	...
	if ($bank_format_marker=='Branch')
	{
		for ($i = 5; $i <= $excel_row; $i++) 
		{
			...
			if ($credit_amount>0 AND ($deposit_type=='CASH/CHECK DEPOSIT' OR $deposit_type=='BATCH CREDITING')) {
				
				$check_count=check_exist_statement($db_,$date_deposited,$credit_amount,$balance);
				
				if ($check_count<=0){
				$sql = "INSERT INTO cash_deposit.".TB_PREF."bank_statement_metro(date_deposited,deposit_type,credit_amount,balance)				
...
if (isset($_POST['upload'])) 
{
	...
	if ($_POST['bank_account']=='10102299'){
		...
		while($row = db_fetch($result))
		{
				...
				$sql="select * from cash_deposit.".TB_PREF."bank_statement_aub where cleared='0' AND debit_amount!=0 order by date_deposited";
				$res = db_query($sql);
				...
				$row_bank_trans=get_bt_cheque_details($bank_ref_num,$debit_amount);
				$bank_trans_id=$row_bank_trans['bank_trans_id'];
				
				$sql_update_bank_trans = "UPDATE ".TB_PREF."bank_trans SET reconciled='1'
				WHERE id ='$bank_trans_id' AND type='$type'";
				db_query($sql_update_bank_trans);
				
				update_bank_statement($db_,$bank_statement_id,$type,$bank_trans_id);
				}
		}
	}
		...
		$sql1="select * from cash_deposit.".TB_PREF."bank_statement_metro where cleared='0' and date_deposited>='2015-09-03' order by date_deposited";
		$res1 = db_query($sql1);
		...
		while($row = db_fetch($result))
		{
				...
				WHERE sdc.dc_remittance_date>='2015-10-01' AND sdc.dc_remittance_date<='2015-11-30'
				AND (sdc.dc_card_desc='METROBANK' OR sdc.dc_card_desc='metrobank')
				AND sdc.processed='0' AND sdc.paid='0'
				...
				//'".date2sql($start_date_paid)."'
				//'".date2sql(Today())."'
				
				//WHERE sdc.dc_remittance_date>='2015-05-05' AND sdc.dc_remittance_date<='2015-05-05'
				//WHERE sdc.dc_remittance_date>='".date2sql($start_date_paid)."' AND sdc.dc_remittance_date<='".date2sql(Today())."'
				...
				//display_error($count1);
					...
					while($row_sales_dr_cr = db_fetch($res))
					{
						...
						$sql2="select * from cash_deposit.".TB_PREF."bank_statement_metro where  date_deposited>'".$row_sales_dr_cr['date']."' and date_deposited<='".date2sql($date_to_clear)."'
						...
						//'".$row_sales_dr_cr['date']."'  				
							...
							while($statement_row = db_fetch($res2))
							{
								...
								$bank_statement_id=$statement_row['id'];
											...
											update_bank_statement($db_,$bank_statement_id,$inserttype,$cd_id);
