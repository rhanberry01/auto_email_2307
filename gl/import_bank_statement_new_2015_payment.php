...
{
$sql = "SELECT * FROM cash_deposit.".TB_PREF."centralized_payment_aub WHERE br_code='$myBranchCode' AND chk_number='$chk_number' AND chk_amount='$debit_amount' AND reconciled='0' ORDER BY bank_trans_date";
//display_error($sql);
$res = db_query($sql);
$row = db_fetch($res);
		...
		for ($i = 5; $i <= $excel_row; $i++) 
		{
			...
			$credit_amount = (double)$credit_amount;
						...
						if ($debit_amount!=0 AND $debit_amount>0){
							...
							$sql = "INSERT INTO cash_deposit.".TB_PREF."bank_statement_metro_2015(date_deposited,bank_ref_num,deposit_type,debit_amount,credit_amount,balance)				
...
function handle_new_aub_excel_item()
{
	...
	$db_='bank_statement_aub_2015_new';
	...
	if ($bank_format_marker=='Date')
	{
		for ($i = 2; $i <= $excel_row; $i++) 
		{
			...
			if ($debit_amount!=0 AND $debit_amount>0){
					...
					if ($check_count<=0 AND $debit_amount!=0){
					$sql = "INSERT INTO cash_deposit.".TB_PREF."bank_statement_aub_2015_new(date_deposited,bank_ref_num,deposit_type,debit_amount,credit_amount,balance)				
					VALUES ('".date2sql($date_deposited)."',".db_escape($bank_ref_num).",".db_escape($deposit_type).",'$debit_amount',0,'$balance')";		
					//display_error($sql);
					db_query($sql,'unable to import bank deposit statement');
					}
...
if (isset($_POST['upload'])) 
{
	...
	if ($_POST['bank_account']=='10102299'){
		...
		// {
				...
				$sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_aub_2015_new where bank_ref_num!='' AND cleared='0' AND debit_amount!=0 order by date_deposited";
				...
				//display_error($bank_trans_id);
					...
					{
						$sql_update_bank_trans = "UPDATE cash_deposit.".TB_PREF."centralized_payment_aub 
						SET reconciled='1',
	...
	//===================================
		...
		// {
			...
			$sql_payment="select * from cash_deposit.".TB_PREF."bank_statement_metro_2015 where cleared='0' AND debit_amount!=0 order by date_deposited";
			$res_payment = db_query($sql_payment);
