...
$_SESSION['page_title'] = _($help_context = "Journal Non-trade Replenishment");
...
start_table();
start_row();
	get_journal_non_trade_user_list_cells('Created By:','created_by');
...
function insert_journal_non_trade_details($selected_add,$pc_id,$pcd_date,$pcd_payee,$pcd_purpose,$exp_type,$pcd_ref,$pcd_tin,$tax_type,$prepared_by_id,$prepared_by_name)
{
global $path_to_root;
$sql = "INSERT INTO ".TB_PREF."journal_non_trade_details (pc_id,pcd_date,pcd_payee,pcd_purpose,pcd_gl_type,
...
$sql = "UPDATE ".TB_PREF."journal_non_trade_details 
...
function clear_journal_non_trade_details($selected_id,$pcd_gl_type,$pcd_payee,$pcd_purpose,$pcd_ref,$pcd_tin,$tax_type,$approved_by_id,$approved_by_name)	
{	
...
db_query($sql);
	 ...
	 $sql2 = "UPDATE ".TB_PREF."journal_non_trade_details 
...
{
$sql="SELECT pc_id FROM ".TB_PREF."journal_non_trade_details WHERE pcd_id=".db_escape($id)."";
$result=db_query($sql);
$row=db_fetch($result);
$pc_id=$row['pc_id'];

$sql="DELETE FROM ".TB_PREF."journal_non_trade_details WHERE pcd_id=".db_escape($id)." or  pcd_head_id=".db_escape($id)."";
...
display_notification_centered(_("Journal NT $trans_no has been processed."));
display_note(get_gl_view_str($trans_type, $trans_no, _("View the GL Postings for this Deposit")));
hyperlink_params("$path_to_root/gl/inquiry/journal_non_trade_inquiry.php", _("Process Another Petty Cash"));
display_footer_exit();
}
...
insert_journal_non_trade_details($selected_add,$pc_id,$pcd_date,$pcd_payee,$pcd_purpose,$exp_type,$pcd_ref,$pcd_tin,$tax_type+0,$approver_id,$approver_real_name);
...
clear_journal_non_trade_details($approve_add,$exp_type,$pcd_payee,$pcd_purpose,$pcd_ref,$pcd_tin,$tax_type,$approver_id,$approver_real_name);

$date_approved=Today();

add_gl_trans_temp(ST_JOURNAL_NT, $approve_add, $pcd_date, 2000010, 0, 0, $remarks,-$pcd_amount, null, 0, $pcd_payee);
if ($tax_amount!=0) {
add_gl_trans_temp(ST_JOURNAL_NT, $approve_add, $pcd_date, $tax_type, 0, 0, $remarks,$tax_amount, null, 0, $person_id);
}
add_gl_trans_temp(ST_JOURNAL_NT, $approve_add, $pcd_date, $exp_type, 0, 0, $remarks,$pcd_amount-$tax_amount, null, 0, $person_id);
$desc='Cleared';
add_audit_trail(ST_JOURNAL_NT, $approve_add, $date_approved,$desc);
...
table_header($th);			
										...
										$sql="select pcd.*,pch.pc_employee_id,pch.pc_employee_name from ".TB_PREF."journal_non_trade_details as pcd
										left join ".TB_PREF."journal_non_trade_header as pch
										...
										//display_error('m4');
										//$sql = "SELECT * FROM ".TB_PREF."journal_non_trade_details";
										$sql="select pcd.*,pch.pc_employee_id,pch.pc_employee_name from ".TB_PREF."journal_non_trade_details as pcd
										left join ".TB_PREF."journal_non_trade_header as pch
										on pcd.pc_id=pch.pc_id";
