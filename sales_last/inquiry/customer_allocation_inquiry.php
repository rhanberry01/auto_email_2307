...
{
	return get_trans_view_str($trans["type"], $trans["trans_no"], getPayRef($trans["trans_no"], $trans["type"]));
}
...
//------------------------------------------------------------------------------------------------
  ...
  $sql = "SELECT 
		...
		((trans.type = ".ST_SALESINVOICE.")
			AND trans.due_date < '" . date2sql(Today()) . "') AS OverDue, trans.reference
    	FROM "
			.TB_PREF."debtor_trans as trans, "
			.TB_PREF."debtors_master as debtor
    	WHERE debtor.debtor_no = trans.debtor_no
			AND ((trans.ov_gst + trans.ov_freight 
			+ trans.ov_freight_tax + trans.ov_discount + trans.ewt + trans.tracking + 
			(trans.ov_amount  ) ) != 0)
...
$cols = array(
	...
	_("Reference") => array('fun'=>'view_link', 'type'=>'nowrap'),
	/*_("Reference"), */
