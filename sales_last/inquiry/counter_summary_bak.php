...
function counter_summary_excel()
{
	...
	$sql = "SELECT DISTINCT
			...
			SUM(extended)
			FROM FinishedSales 
			WHERE LogDate >= '".date2sql($_POST['date_from'])." 00:00:00'
			AND LogDate <= '".date2sql($_POST['date_to'])." 00:00:00'
			GROUP BY TerminalNo
			";
	$res = ms_db_query($sql);
	...
	while($row = mssql_fetch_array($res))
	{
		...
		$r_sql .= ", SUM(
					CASE 
							WHEN pVatable = 1 AND TerminalNo = '".$row['TerminalNo']."'
								 THEN Extended
							ELSE 0 
					END)  as VatableSales_".$row['TerminalNo'];
		$r_sql .= ", SUM(
					CASE 
							WHEN pVatable = 0 AND TerminalNo = '".$row['TerminalNo']."'
								 THEN Extended
							ELSE 0 
					END)  as NonVatableSales_".$row['TerminalNo'];
		// $r_sql .= ",SUM(CASE 
							// WHEN TerminalNo = '".$row['TerminalNo']."'
								 // THEN Extended
							// ELSE 0 
					// END)  as Reading_".$row['TerminalNo'];
	}

	$r_sql .= " FROM FinishedSales 
				WHERE LogDate >= '".date2sql($_POST['date_from'])." 00:00:00'
				AND LogDate <= '".date2sql($_POST['date_to'])." 00:00:00'
				GROUP BY LogDate
				ORDER BY LogDate";
