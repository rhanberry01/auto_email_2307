...
function print_report()
{
	...
	$sql .= " account IN (2000)";
		...
		$sql .= " account IN (2000010)";
	}
	else {
		$sql .= " account IN (2000,2000010)";
	}	
				
	$sql .=" AND tran_date >= '".date2sql($from)."'
	...
	while($row = db_fetch($res))
	{
		if ($row['account'] != 2000 and $row['account'] != 2000010)
	...
	foreach ($c_header as $ind => $title)
		$rep->sheet->writeString($rep->y, $ind, ($ind < $subtractor ? $title : html_entity_decode(get_gl_account_name($title))), $format_bold_title);
