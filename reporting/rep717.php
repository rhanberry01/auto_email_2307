...
function print_unpaid_apv()
{
	...
	$rep = new FrontReport(_('Unpaid Sales Invoice'), "Unpaid Sales Invoice", user_pagesize(),9 ,'P');
	...
	$sql = "SELECT a.reference as apv_no, a.del_date as delivery_date, a.tran_date as apv_date, b.supp_name, 
					a.supp_reference as invoice_no, round(a.ov_amount+a.ov_gst,2) as invoice_amount, c.cv_no
			FROM (0_supp_trans a, 0_suppliers b)
			LEFT JOIN 0_cv_header c ON a.cv_id = c.id
			WHERE a.type = '20' AND a.ov_amount > '0' AND a.alloc = '0' 
