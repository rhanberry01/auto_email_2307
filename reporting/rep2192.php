...
function print_pdcc_for_payments()
{
	...
	$headers = array('APV REF', 'DM#', 'Date', 'Invoice Ref','Amount   ', 'Remarks'); //'Supplier',
...
{
	$sql = "SELECT supp_reference FROM ".TB_PREF."supp_trans
			WHERE type=20 and cv_id=$t_no";
	$res = db_query($sql);

	while($row = db_fetch($res)){
		$ref=$row['supp_reference'];
	}
...
SELECT st.*,s.supp_name,cv.bank_trans_id, bt.trans_date
	...
	else
	{
		...
		while($myrow = db_fetch($res))
		{
			...
			$supp_rows[] = array(
				...
				($myrow['cv_id'] != 0? 'SI# '.get_si_ref($myrow['cv_id']):''),
				$myrow['reference'],
