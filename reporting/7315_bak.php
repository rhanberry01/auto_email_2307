...
function print_GL_transactions()
{
	...
	// $sql = "SELECT DISTINCT c.type, c.trans_no, b.gst_no, b.supp_name, b.supplier_id,
		...
		// LEFT JOIN 0_supp_trans as st
		// on (a.type_no=st.trans_no and a.type=st.type) 
		// LEFT JOIN 0_cv_header as cv on st.cv_id =cv.id
		// WHERE a.tran_date >= '".date2sql($from)."'
		// AND a.tran_date <= '".date2sql($to)."'
		// AND a.amount!='0'
		// AND a.amount<0
		// AND a.type IN (20,53)
		// AND cv.bank_trans_id=0
		// ORDER BY c.supp_name, a.type, a.tran_date, a.amount";
		...
		while ($row = db_fetch($res))
		{
			...
			if($bank_trans_id!=0){
				$bank_row = get_bank_trans_of_cv($bank_trans_id,date2sql($to));
			}
			else{
				$bank_row['id']==0;
			}
			...
			if ($bank_row['id']=='' and $bank_row['id']==0){
				...
				// header
				if ($last_supp_id != $row['supplier_id'] and $row['type']==20)
				{
