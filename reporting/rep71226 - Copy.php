...
function print_report()
{
	...
	$sql = "SELECT type,type_no FROM `0_gl_trans` 
				WHERE account IN('5400','5450','1410010','1440')
	...
	$c_header = array('DATE', 'NAME OF SUPPLIER', 'ADDRESS', 'INV. NO ', 'TIN', 'VAT PURCHASES (DR)', 'NON-VAT PURCHASES (DR)', 'INPUT TAX (DR)', 'TOTAL INVOICE AMOUNT', 'ACCOUNTS PAYABLE (CR)','ADVANCES TO SUPPLIER');
	$c_totals = array();
...
// echo $sql;die;
	...
	while($row = db_fetch($res))
	{
		...
		if ($last_type != $row['type'] OR $last_type_no != $row['type_no'])
		{
			...
			$rep->sheet->writeNumber($rep->y, $index =  array_search($row['account'], $c_header_), abs($row['total']), $rep->formatRight);
			$rep->sheet->writeNumber($rep->y, $index1 =  array_search($row['account'], $c_header_1), abs($row['total']), $rep->formatRight);
			
			
			
			$c_totals[$index] += abs($row['total']);
			$c_totals1[$index1] += abs($row['total']);
		// }
	}
