...
// SELECT a.*, c.supp_name, st.supp_reference, st.special_reference FROM 0_gl_trans a 
// JOIN 0_chart_master b 
...
// ON a.type=st.type AND a.type_no=st.trans_no
// WHERE a.tran_date >= '2016-01-01' 
// AND a.tran_date <= '2016-06-30' AND a.account = '1440' 
// ORDER BY a.tran_date, a.counter
	

	$sql = "SELECT a.*, c.supp_name, st.supp_reference, st.special_reference FROM 0_gl_trans a 
	...
	while ($account=db_fetch($accounts))
	{
		...
		if ($rows > 0)
		{
			while ($myrow=db_fetch($trans))
			{
				...
				if($myrow["special_reference"]=='' OR $myrow["special_reference"]=='0'){
					$rep->TextCol(4, 6,	$voided_str. $myrow["supp_reference"]);
				}
				else{
					
					$rep->TextCol(4, 6,	$voided_str. $myrow["special_reference"]);
				}
