...
function get_gl_transactions_704($from_date, $to_date, $account, $order_by_person=false)
{
	...
	$sql = "SELECT a.*, c.supp_name,st.*,a.tran_date as t_date FROM 0_gl_trans a
  ...
  //$cols = array(0, 80, 100, 150, 210, 280, 340, 400, 450, 510, 570);
	...
	// else
		$headers = array(_('Type'),	'', _('#'),	_('Date'), "Particulars", "", 'TIN', _('Person/Item'),
			_('Debit'),	_('Credit'), _('Gross Amount'));
	...
	while ($account=db_fetch($accounts))
	{
		...
		if ($rows > 0)
		{
			while ($myrow=db_fetch($trans))
			{
				...
				$rep->DateCol(3, 4,	$myrow["t_date"], true);
				// if ($dim >= 1)
				...
				if ($myrow['amount'] > 0.0)
				{
				    ...
				    $rep->TextCol(10, 11, number_format2(($myrow['amount'] / .12), $dec));
				...
				else
				{
					...
					//$total_alloc += abs(($myrow['amount'] / .12));

				//$rep->TextCol(10, 11, number_format2($total_alloc, $dec));
		...
		if (abs($total_alloc) != 0)
		$rep->AmountCol(10, 11, ($total_alloc), $dec);
