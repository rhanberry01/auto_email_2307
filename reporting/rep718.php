...
function print_paid_unpaid_apv()
{
	...
	$sql = "SELECT b.supp_name, a.trans_no, a.type, a.supp_reference,a.tran_date, 
				IF(type = 20,round(ov_amount+ov_gst-ov_discount,2),round(ov_amount+ov_gst-ov_discount+a.ewt,2)) as amount, 
				c.cv_no, a.cv_id
			FROM (0_supp_trans a, 0_suppliers b)
			LEFT OUTER JOIN 0_cv_header c
			ON a.cv_id = c.id
			WHERE tran_date >= '".date2sql($from)."'
			AND tran_date <= '".date2sql($to)."'
			AND a.supplier_id = b.supplier_id
			AND IF(type = 20,round(ov_amount+ov_gst-ov_discount,2),round(ov_amount+ov_gst-ov_discount+a.ewt,2)) != 0";
	...
	{
		$last_cv = $supp_name = '';
		$cv_total = $supp_credit = $supp_debit = $total_credit = $total_debit = 0;
		$cv_trans = array();
		while($row = db_fetch($res))
		{	
			if (($last_cv != $row['cv_id'] AND round($cv_total,2) != 0) OR $supp_name != $row['supp_name'] OR
				$last_cv == 0) // display prev CV details
			{
				
				foreach($cv_trans[$last_cv] as $val)
				{
					$rep->TextCol(0, 1,	$val[0],0,0,0,0,null,1,0,'left');
					$rep->TextCol(1, 2,	$val[1],0,0,0,0,null,1,0,'left');
					$rep->TextCol(2, 3,	$val[2],0,0,0,0,null,1,0,'center');
					$rep->TextCol(3, 4,	$val[3],0,0,0,0,null,1,0,'right');
					$rep->TextCol(4, 5,	$val[4],0,0,0,0,null,1,0,'right');
					$rep->TextCol(5, 6,	$val[5]);
			...
			if ($supp_name != $row['supp_name']) //display total
			{
				if ($supp_name != '')
				{
					$bal = round(abs($supp_debit),2) - round(abs($supp_credit),2);
			...
			if ($row['amount'] <= 0) // debit //reversed
			{
				...
				$cv_trans[$row['cv_id']][] = array($systypes_array[$row['type']],
						'   '.$row['supp_reference'],
						sql2date($row['tran_date']),
						number_format2(abs($row['amount']),2),
						'',
						$row['cv_no']);
				
				$cv_total += round(abs($row['amount']),2);
			}
			else // credit
			{
				// $rep->TextCol(4, 5,	number_format2(abs($row['amount']),2),0,0,0,0,null,1,0,'right');
