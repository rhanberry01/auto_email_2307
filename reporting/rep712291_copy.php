...
/*	$sql="
	...
	$sql="
		...
		LEFT JOIN  srs_aria_alaminos.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_alaminos.0_gl_trans as gls ON
		...
		LEFT JOIN  srs_aria_graceville.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_graceville.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00') 
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_antipolo_manalo.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_antipolo_manalo.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_antipolo_quezon.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_antipolo_quezon.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_b_silang.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_b_silang.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_bagumbong.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_bagumbong.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_blum.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_blum.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_cainta.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_cainta.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_cainta_san_juan.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_cainta_san_juan.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_camarin.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_camarin.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_comembo.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_comembo.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_gala.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_gala.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_hero.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_hero.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_imus.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_imus.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_malabon.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_malabon.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_malabon_rest.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_malabon_rest.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_molino.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_molino.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_navotas.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_navotas.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_nova.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_nova.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_pateros.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_pateros.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_punturin_val.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_punturin_val.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_retail.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_retail.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_san_pedro.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_san_pedro.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_talon_uno.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_talon_uno.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_tondo.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_tondo.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."'
		...
		LEFT JOIN  srs_aria_valenzuela.0_cheque_details as cds
		on bts.type = cds.type and  bts.id = cds.bank_trans_id
		LEFT JOIN srs_aria_valenzuela.0_gl_trans as gls ON
		gls.type = bts.type  and gls.type_no =  bts.trans_no WHERE gls.account ='".$bank_account_code."'
		and gls.type =22 and bts.type = 22 and gls.amount < 0 and !(ISNULL(bts.reconciled) OR bts.reconciled='' OR  bts.reconciled='0000-00-00')
		and gls.tran_date >='".date2sql($from)."' and gls.tran_date <='".date2sql($to)."'
		and reconciled <> '' and bts.reconciled >='".date2sql($from)."' and bts.reconciled <='".date2sql($to)."')  as m
