...
// function get_gl_trans_amount2($type_type_no, $account)
// {
	// $type_type_no[0] = (type = $type AND type_no = $type_no);
	// $type_type_no[1] = array(20,101);
	...
	// /* $sql .= "			
					// (type = $type AND type_no = $type_no) OR
					// (type = $type AND type_no = $type_no) OR
					// (type = $type AND type_no = $type_no) OR
				// "; */
...
function print_GL_transactions()
{
	...
	$rep = new FrontReport(_('Consolidated Purchase Report (Summarized)'), "SummarizedConsolidatedPurchaseReport", 'LONG',8 ,'P');
	$dec = user_price_dec();
	...
	for($xyz=0;$xyz<=2;$xyz++)
	{
		...
		foreach($db_connections as $key=>$db_con)
		{
			...
			$sql = "(SELECT DISTINCT c.type, c.trans_no, b.gst_no, b.supp_name, b.supplier_id, b.supp_ref, b.address,
