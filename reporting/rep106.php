...
function GetSalesmanTrans($from, $to)
{
	...
	$sql = "SELECT DISTINCT ".TB_PREF."debtor_trans.*,
		(".TB_PREF."debtor_trans.ov_gst +  ".TB_PREF."debtor_trans.ov_freight + ".TB_PREF."debtor_trans.ov_freight_tax + ".TB_PREF."debtor_trans.ov_discount + ".TB_PREF."debtor_trans.ewt + ".TB_PREF."debtor_trans.tracking +
				(".TB_PREF."debtor_trans.ov_amount  * (1 - ".TB_PREF."debtor_trans.discount1) * (1 - ".TB_PREF."debtor_trans.discount2) * (1 - ".TB_PREF."debtor_trans.discount3)
						* (1 - ".TB_PREF."debtor_trans.discount4) * (1 - ".TB_PREF."debtor_trans.discount5) 
				)
			) AS InvoiceTotal,
...
function print_salesman_list()
{
	...
	$cols = array(0, 60, 150, 250, 325,	385, 450, 515);
	...
	$rep = new FrontReport(_('Sales Report per Salesman'), "SalesReportPerSalesman", user_pagesize());
	// $rep->Font();
	...
	// {
		...
		{
			$headers = array(_('Customer'), _(''), _(''), _('INV #'),
			_('Inv Date'),	_('Total'),	_('Provision'));	
			$headers2 = array(_('Salesman'), " ",	_('Email'), _('Phone'),	_('Provision'),
			_('Break Pt.'), _('Provision')." 2");
	...
	while ($myrow=db_fetch($result))
	{
		...
		if ($salesman != $myrow['salesman_code'])
		{
			...
			$rep->TextCol(1, 2,	" ");
			$rep->TextCol(2, 3, "Email");
			$rep->TextCol(3, 4,	"Phone");
			$rep->TextCol(4, 5,	"Provision");
			$rep->TextCol(5, 6,	"Break Pt.");	
			$rep->TextCol(6, 7, "Provision 2");
			$rep->Font('');			
			$rep->Line($rep->row - 4);
			$rep->NewLine(2);
			// header	
			...
			$rep->fontSize += 1;
			$rep->TextCol(0, 2,	strtoupper($myrow['salesman_code']." ".$myrow['salesman_name']));
			$rep->TextCol(2, 3,	$myrow['salesman_email']);
			$rep->TextCol(3, 4,	$myrow['salesman_phone']);
			...
			// header	
			$rep->Font('bold');	
			$rep->TextCol(0, 1,	"Customer");
			$rep->TextCol(1, 2,	" ");
			$rep->TextCol(2, 3, "");
			$rep->TextCol(3, 4,	"INV #");
			$rep->TextCol(4, 5,	"Inv Date");
			$rep->TextCol(5, 6,	"Total");	
			$rep->TextCol(6, 7, "Provision");
			$rep->Font('');			
			$rep->Line($rep->row - 4);
			$rep->NewLine(2);
			// header		
		...
		if (!$summary)
		{
			$rep->TextCol(0, 3,	$myrow['DebtorName']);
