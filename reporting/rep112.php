...
function get_receipt($type, $trans_no)
{
    $sql = "SELECT ".TB_PREF."debtor_trans.*,
				(".TB_PREF."debtor_trans.ov_gst +  ".TB_PREF."debtor_trans.ov_freight + ".TB_PREF."debtor_trans.ov_freight_tax + ".TB_PREF."debtor_trans.ov_discount + ".TB_PREF."debtor_trans.tracking +
...
function print_receipts()
{
	...
	$cols = array(4, 85, 150, 225, 275, 360, 450, 540);
	...
	$rep = new FrontReport(_('RECEIPT'), "ReceiptBulk", user_pagesize(),9,"P",null,null,true);
	$rep->currency = $cur;
	$rep->Font();
	...
	for ($i = $fno[0]; $i <= $tno[0]; $i++)
	{
		...
		/*
			...
			$rep->AmountCol(6, 7, $myrow['Total'] - $total_allocated + $myrow['ewt'], $dec, -2);
			$rep->NewLine();
			$rep->Font('bold');
			...
			$rep->Font();
			$rep->NewLine();
			$rep->TextCol(5, 6, $doc_Received." : ", - 2);
			// $rep->NewLine();
			...
			$rep->TextCol(6, 7, "_______________________________", - 2);
