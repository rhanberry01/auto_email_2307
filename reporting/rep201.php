...
function get_open_balance($supplier_id, $to, $convert)
{
    ...
    $sql .= ", 0)) AS charges,
    	...
    	".TB_PREF."supp_trans.ov_discount+ewt)";
    if ($convert)
    	$sql .= "* rate";
    $sql .= ", 0)) AS credits,
	...
	if ($convert)
		$sql .= " * rate";
	$sql .= ") AS Allocated,
		SUM((".TB_PREF."supp_trans.ov_amount + ".TB_PREF."supp_trans.ov_gst + ewt +
    	".TB_PREF."supp_trans.ov_discount)";
    if ($convert)
    	$sql .= " * rate";
    $sql .= ") AS OutStanding
...
function getTransactions($supplier_id, $from, $to,$sort_order=0)
{
	$from = date2sql($from);
	$to = date2sql($to);
	$order_date="IF (".TB_PREF."supp_trans.type=".ST_SUPPINVOICE." OR ".TB_PREF."supp_trans.type=".ST_SUPPCREDIT.",".TB_PREF."supp_trans.due_date,IF(".TB_PREF."supp_trans.type = ".ST_SUPPAYMENT.", ".TB_PREF."supp_trans.tran_date,".TB_PREF."supp_trans.tran_date))";
    $sql = "SELECT ".TB_PREF."supp_trans.*,
				(".TB_PREF."supp_trans.ov_amount + ".TB_PREF."supp_trans.ov_gst + ".TB_PREF."supp_trans.ov_discount+ewt)
				AS TotalAmount, ".TB_PREF."supp_trans.alloc AS Allocated,
				((".TB_PREF."supp_trans.type = ".ST_SUPPINVOICE.")
					AND ".TB_PREF."supp_trans.due_date < '$to') AS OverDue,$order_date
    			...
    			if($sort_order)
    				...
    				$sql.="ORDER BY ".TB_PREF."supp_trans.tran_date,id";
    	//			display_error($sql	);
...
function print_supplier_balances()
{
	...
	// $headers = array(_('Transaction'), _('#'), _('Date'), _(''), _('Charges'),
		// _('Credits'), _(''), _('Outstanding'));
	// $headers = array();
    ...
    $rep = new FrontReport(_('Statement of Account, Supplier(s)'), "StatementofAccount", user_pagesize());
	...
	$sql = "SELECT supplier_id, supp_name AS name, curr_code,terms FROM ".TB_PREF."suppliers a, ".TB_PREF."payment_terms b WHERE a.payment_terms=b.terms_indicator";
	if ($fromsupp != ALL_NUMERIC)
	...
	$res = getTransactions($myrow['supplier_id'], $from, $to,$sort_order);
	...
	$bal = get_open_balance($myrow['supplier_id'], $from, $convert);
		...
		if($bal['OutStanding'] != 0)
		{
			...
			$rep->NewLine(1, 2);
			$rep->TextCol(0, 2, "Forwarded Balance");
			$init[0] = $init[1] = 0.0;
		...
		while ($trans=db_fetch($res))
		{
			...
			if($sort_order&&($trans['type']==ST_SUPPINVOICE||$trans['type']==ST_SUPPCREDIT))
