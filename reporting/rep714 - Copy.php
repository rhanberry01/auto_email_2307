...
function print_GL_transactions()
{
	...
	$rep = new FrontReport(_('Purchase Report'), "PurchaseReport", user_pagesize(),9 ,'P');
	$dec = user_price_dec();
	...
	$cols = array(0, 75, 150, 225, 300, 375, 450, 425, 555);
	$aligns = array('left', 'left', 'center', 'right', 'right', 'right', 'right', 'right');
	$headers = array('APV', 'Del. Date', 'Inv. #', 'Purch. NON-VAT', 'Purch. VAT', '12% VAT', 'Sum of AMOUNT');
	//'Discount', 
	...
	while ($row = db_fetch($res))
	{
		...
		$details[$counter][3] = $p_nv != 0 ? number_format2($p_nv,2) : '-';
		$details[$counter][4] = $p_v != 0 ? number_format2($p_v,2) : '-';
		$details[$counter][5] = $vat != 0 ? number_format2($vat,2) : '-';
		$details[$counter][6] = number_format2($row['total'],2);
