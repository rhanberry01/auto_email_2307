...
function get_bank_balance_to_2($to, $account)
{
	$to = date2sql($to);
	$sql = "SELECT SUM(chk_amount) FROM ".TB_PREF."bank_trans 
			JOIN ".TB_PREF."cheque_details ON ".TB_PREF."cheque_details.bank_trans_id  = ".TB_PREF."bank_trans.trans_no 
			WHERE bank_act=".db_escape($account) . "
			AND deposited = 1
...
function get_bank_transactions_2($from, $to, $account)
{
	$from = date2sql($from);
	$to = date2sql($to);
	$sql = "SELECT ".TB_PREF."bank_trans.*, chk_date, chk_amount, chk_number, bank
		FROM ".TB_PREF."bank_trans
		JOIN ".TB_PREF."cheque_details ON ".TB_PREF."cheque_details.bank_trans_id  = ".TB_PREF."bank_trans.trans_no 
		WHERE ".TB_PREF."bank_trans.bank_act = ".db_escape($account) . "
		AND ".TB_PREF."cheque_details.deposited = 1
		AND chk_date >= '$from'
		AND chk_date <= '$to'
		AND ".TB_PREF."cheque_details.type = ".TB_PREF."bank_trans.type
		ORDER BY chk_date,".TB_PREF."bank_trans.id";
...
function print_bank_transactions()
{
	...
	if (get_bank_trans_type($acc) == 1)
	{
		$prev_balance = get_bank_balance_to_2($from, $account["id"]);

		$trans = get_bank_transactions_2($from, $to, $account['id']);
	}
	else
	{
	...
	if ($prev_balance != 0.0 || $rows != 0)
	{
		...
		if ($rows > 0)
		{
			...
			while ($myrow=db_fetch($trans))
			{
				if (get_bank_trans_type($acc) == 1)
				{
					$total += $myrow['chk_amount'];

					$rep->TextCol(0, 2, $systypes_array[$myrow["type"]]);
					$rep->TextCol(1, 2,	$myrow['ref']);
					$rep->TextCol(2, 3,	$myrow["chk_number"]);
					$rep->DateCol(3, 4,	$myrow["chk_date"], true);
					$rep->TextCol(4, 5,	payment_person_name($myrow["person_type_id"],$myrow["person_id"], false));
					if ($myrow['chk_amount'] > 0.0)
					{
						$rep->AmountCol(5, 6, abs($myrow['chk_amount']), $dec);
						$total_debit += abs($myrow['chk_amount']);
					}
					else
					{
						$rep->AmountCol(6, 7, abs($myrow['chk_amount']), $dec);
						$total_credit += abs($myrow['chk_amount']);
					}
