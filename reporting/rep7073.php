...
function print_profit_and_loss_statement()
{
	...
	// // while ($accounttype=db_fetch($typeresult))
		...
		$include = array('FDFB','WH2BU','IGNBO','IGNSA','SW','PS','IGBO','IGSA' );
		$rep->Font('i');
		while($ms_row = mssql_fetch_array($ms_res))
		{
			$add = array('PS','IGBO','IGSA');
			$less = array('FDFB','WH2BU','IGNBO','IGNSA','SW');

			if (in_array($ms_row['movementcode'],$include)){
				
				$rep->TextCol(0, 2, "   ".$ms_row['Description']);
				
				if (in_array($ms_row['movementcode'],$add)){
					$addsales += $ms_row['total'];
					$rep->AmountCol(2, 3, $ms_row['total'], $dec);
					// $rep->TextCol(3, 4, compute_percent($beg_inv,$a));
					$rep->NewLine();
				}
				if (in_array($ms_row['movementcode'],$less)){			
					$lesssales += $ms_row['total'];
					$rep->AmountCol(2, 3, '-'.$ms_row['total'], $dec);
					// $rep->TextCol(3, 4, compute_percent($beg_inv,$a));
					$rep->NewLine();
				}
	...
	// while ($accounttype=db_fetch($typeresult))
		...
		while($ms_row = mssql_fetch_array($ms_res))
		{
			$less = array('PS','IGBO','IGSA');
			$add = array('FDFB','WH2BU','IGNBO','IGNSA','SW');
