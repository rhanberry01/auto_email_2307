...
function print_remittances()
{
	...
	if ($email == 0)
	{
		$rep = new FrontReport("PAYMENT", "RemittanceBulk", user_pagesize(),  9, 'P', NULL, NULL,true);
		$rep->currency = $cur;
		$rep->Font();
	...
	for ($i = $fno[0]; $i <= $tno[0]; $i++)
	{
		...
		//	if($j==ST_SUPPCREDIT) $rep->title=_('RETURNS');
			$rep->Header2($myrow, null, $myrow, $baccount, $j);
...
*/
			...
			$bank_id = getWhatBank($j, $i);
			$btype = get_bank_trans_type($bank_id);
			$bank_trans = getBankTrans($j, $i);
			//display_error($j.' -- '.$i);
			// $bank_info = getBankInfo($bank_id, $btype, $bank_trans, $i);
			
			// if($bank_info['chdate'] == 'N/A'){
				// $bch_date = $bank_info['chdate'];
			// }else{
				// $bch_date = date('F d, Y', strtotime($bank_info['chdate']));
			// }
			
			// $rep->TextWrap(35, 210, 500, "Bank: ".$bank_info['name']);
			// $rep->TextWrap(35, 201, 500, "Branch: ".$bank_info['branch']);
			// $rep->TextWrap(35, 192, 500, "Cheque No.: ".$bank_info['chnum']);
			// $rep->TextWrap(35, 183, 500, "Cheque Date: ".$bch_date);
			...
			$myrow['Total'] *= -1;
			$rep->AmountCol(6, 7, abs($myrow['Total'] - $total_allocated - $myrow['ewt']), $dec, -2);
			$rep->NewLine();
