...
function getTransactions($category, $location)
{
	$sql = "SELECT ".TB_PREF."stock_master.category_id,
			...
			(".TB_PREF."stock_master.material_cost + ".TB_PREF."stock_master.labour_cost + ".TB_PREF."stock_master.overhead_cost)/1.12 AS UnitCost,
			SUM(".TB_PREF."stock_moves.qty) *(".TB_PREF."stock_master.material_cost + ".TB_PREF."stock_master.labour_cost + ".TB_PREF."stock_master.overhead_cost)/1.12 AS ItemTotal
...
function print_inventory_valuation_report()
{
	...
	if ($location == 'all')
		...
		$cols = array(0, 100, 275, 375, 400,470,540);

	$headers = array(_('Category'), '', _('Quantity'), "Unit", _('Unit Cost'), _('Value'));
	...
	while ($trans=db_fetch($res))
	{
		...
		if ($detail)
		{
			...
			$rep->AmountCol(4, 5, $trans['UnitCost'], $dec);
