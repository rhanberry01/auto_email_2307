...
function print_report()
{
    ...
    $params =   array( 	0 => '',
      ...
      $rep = new FrontReport(_('CASH_DEPOSIT_RECONCILED'), "BPI_BankRecon_DEPOSIT", "LETTER");
	...
	$rep->y ++;
	$rep->sheet->writeString($rep->y, 0, 'BPI Bank Recon (Cash Deposit)', $format_bold);
	$rep->y ++;
...
//===================================================================================
	...
	// get the header
	$c_header = array('BRANCH','REFERENCE', 'DATE RECONCILED','AMOUNT','STATUS');
...
// '2000105'
	...
	$rep->y ++;
	$rep->sheet2->writeString($rep->y, 0, 'BPI Bank Recon (Cash Deposit)', $format_bold);
	$rep->y ++;
...
//===================================================================================
	...
	// get the header
	$c_header = array('BRANCH','MEMO','TRANS DATE','AMOUNT','STATUS','GL DATE','CASH ON HAND');
	$c_header_last_index = count($c_header)-1;
//	print_r($c_header_last_index = count($c_header)-1);
...
where cd_cleared=0
and cd_bank_account_code='1010040'
";
...
//display_error($sql); die;
	...
	while($row = db_fetch($res))
	{
		...
		$sqlx = "SELECT tran_date,amount
		FROM 0_gl_trans WHERE type = 60
		and tran_date='".$row['cd_sales_date']."'
		AND account = '1010'
		AND amount!=0
		AND amount>0";
		//display_error($sqlx);
		...
		$resx = db_query($sqlx);
		$rowx = db_fetch($resx);
		$c_details[$count][5] = sql2date($rowx['tran_date']);
	...
	$rep->y ++;
	$rep->sheet3->writeString($rep->y, 0, 'BPI Bank Recon (Other Income)', $format_bold);
	$rep->y ++;
...
//display_error($sql); die;
	...
	$rep->y ++;
	$rep->sheet4->writeString($rep->y, 0, 'BPI Bank Recon (Other Income)', $format_bold);
	$rep->y ++;
...
//===================================================================================
	...
	// get the header
	$c_header = array('BRANCH','MEMO','TRANS DATE','AMOUNT','STATUS','GL DATE','ARIA CASH ON HAND');
	$c_header_last_index = count($c_header)-1;
//	print_r($c_header_last_index = count($c_header)-1);
...
and oi.bd_trans_date>='".date2sql($from)."' and oi.bd_trans_date<='".date2sql($to)."'
...
SELECT 'srsretail' as br_code,'retail' as branch,oi.*,gl.amount as amount 
...
//display_error($sql); die;
	...
	while($row = db_fetch($res))
	{
		...
		$c_details[$count][2] = sql2date($row['bd_trans_date']);
		...
		$sqlx = "SELECT tran_date,amount
		FROM 0_gl_trans WHERE type = 2
		and tran_date='".$row['bd_trans_date']."'
		and type_no='".$row['bd_trans_no']."'
		AND account = '1010'
		AND amount!=0
		AND amount>0";
		//display_error($sqlx);
	...
	$rep->y ++;
	$rep->sheet5->writeString($rep->y, 0, 'BPI Bank Recon (Other Income)', $format_bold);
	$rep->y ++;
...
and credit_amount!=0
and cleared=0
and deposit_type!='BATCH CREDITING'
";
...
//display_error($sql); die;
	...
	//}
		...
		$rep->y++;
			...
			$rep->sheet6 = $rep->addWorksheet('DR_CR_CARD_RECONCILED');
	...
	$rep->y ++;
	$rep->sheet6->writeString($rep->y, 0, 'BPI Bank Recon (Credit/Debit Card)', $format_bold);
	$rep->y ++;
...
//display_error($sql); die;
	...
	//}
			...
			$rep->sheet7 = $rep->addWorksheet('DR_CR_CARD_UNRECONCILED');
	...
	$rep->y ++;
	$rep->sheet7->writeString($rep->y, 0, 'BPI Bank Recon (Credit/Debit Card)', $format_bold);
	$rep->y ++;
...
//===================================================================================
	...
	// get the header
	$c_header = array('BRANCH','MEMO','TRANS DATE','AMOUNT','STATUS','GL DATE','ARIA RECEIVABLE FROM METRO');
	$c_header_last_index = count($c_header)-1;
//	print_r($c_header_last_index = count($c_header)-1);
...
//display_error($sql); die;
	...
	$rep->y ++;
	$rep->sheet7->writeString($rep->y, 2, 'TOTAL', $format_bold_right);
	$qwert = $rep->y;
	// foreach ($c_totals as $ind => $total)
	// {
		$rep->sheet7->writeNumber ($rep->y, 3, $total7, $format_bold_right);
	...
	//}
		...
		$rep->y++;
			...
			$rep->sheet8 = $rep->addWorksheet('JOURNAL');
	...
	$rep->y ++;
	$rep->sheet8->writeString($rep->y, 0, 'METRO Bank Recon (Payment)', $format_bold);
	$rep->y ++;
...
AND account ='1010040' AND amount>0 AND type=0 and memo_ like '%Adjustment for BPI,%'
