...
function print_price_listing()
{
	...
	$cols = array(0, 100, 385,450, 510, 550);
	...
	{
		$headers = array(_('Item Code'), _('Description'),	_('Selling Price'),	"Unit Cost", _('GP %'));
		$rep->Info($params, $cols, $headers, $aligns);
		$rep->Header__();
	}
	else
	...
	while ($myrow=db_fetch($result))
	{
		if ($catgor != $myrow['description'])
		{
			...
			$rep->NewLine();
			$rep->fontSize += 2;
			$rep->TextCol(0, 3, strtoupper($myrow['category_id'] . " - " . $myrow['description']));
			...
			// header	
			$rep->Font('bold');	
			$rep->TextCol(0, 1,	"Item Code");
			$rep->TextCol(1, 2,	"Description");
			$rep->TextCol(2, 3,	"Selling Price");
			$rep->TextCol(3, 4,	"Unit Price");
			$rep->TextCol(4, 5, "GP %");
			$rep->Font('');
			$rep->Line($rep->row  - 4);
			$rep->NewLine();
