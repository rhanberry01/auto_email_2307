...
function print_report()
{
    ...
    $params =   array( 	0 => '',
      ...
      $rep = new FrontReport(_('RECONCILED'), "BPI_BankRecon_CHECK", "LETTER");
	...
	$rep->y ++;
	$rep->sheet->writeString($rep->y, 0, 'BPI Bank Recon (Payment)', $format_bold);
	$rep->y ++;
...
//===================================================================================
	...
	// get the header
	$c_header = array('BRANCH','GL DATE','CV DATE','CV NO.', 'PAYEE','CHECK #', 'CHECK DATE','AMOUNT','DATE RECONCILED','STATUS');
...
//display_error($sql); die;
	...
	while($row = db_fetch($res))
	{
		...
		$c_details[$count][7] = $row['chk_amount'];
	...
	$rep->y++;
		...
		$rep->sheet2 = $rep->addWorksheet('OUTSTANDING_&_STALED');
	...
	$rep->y ++;
	$rep->sheet2->writeString($rep->y, 0, 'BPI Bank Recon (Payment)', $format_bold);
	$rep->y ++;
...
//display_error($sql); die;
	...
	while($row = db_fetch($res))
	{
		...
		if($mos>=6){
			...
			$c_details[$count][9] = 'OUTSTANDING CHECKS';
		}
	...
	$rep->y ++;
	$rep->sheet3->writeString($rep->y, 0, 'BPI Bank Recon (Payment)', $format_bold);
	$rep->y ++;
...
//display_error($sql); die;
	...
	$rep->y ++;
	$rep->sheet4->writeString($rep->y, 0, 'BPI Bank Recon (Payment)', $format_bold);
	$rep->y ++;
