...
function get_gl_transactions_704($from_date, $to_date, $account, $order_by_person=false)
{
	...
	$sql = "SELECT a.*, c.supp_name,s.supp_reference FROM 0_gl_trans a
...
function print_GL_transactions()
{
  ...
  //$cols = array(0, 80, 100, 150, 210, 280, 340, 400, 450, 510, 570);
	...
	while ($account=db_fetch($accounts))
	{
		...
		$trans = get_gl_transactions_704('', $to,  $account['account_code'], $group_by_person);
		...
		if ($rows > 0)
		{
					$bfw = get_gl_balance_from_to($from_2, $from ,$account['account_code'] , $_POST['Dimension'], $_POST['Dimension2']);
			...
			while ($myrow=db_fetch($trans))
			{
				...
				$rep->TextCol(4, 5,	$voided_str. $myrow["memo_"]);
