...
function print_report()
{
	...
	// get the header
	$c_header = array('CV DATE','CV NO.', 'PAYEE', 'INVOICE');
	if ($t_nt != 1)
				...
				while($last_check+1 != $row['chk_number'])
				{
					...
					if (is_array($m_check_row))
					{
						if($m_check_row['cv_id'] == 0) //voided
						{
							...
							$c_details[$count][1] = '';
							$c_details[$count][2] = html_entity_decode(get_supplier_name($m_check_row['supplier_id']));
							$c_details[$count][3] = '';
							$last_index = 3;
							if ($t_nt != 1)
							{
						...
						$c_details[$count][0] = sql2date($cv_header_['cv_date']);
						$c_details[$count][1] = ($cv_header_['cv_no']);
						$c_details[$count][2] = html_entity_decode(get_supplier_name($m_check_row['supplier_id']));
...
//die;
	...
	{
	//	$indx++;
	//	$rep->sheet->writeString($rep->y, $indx, get_gl_account_name($not_header[$ind1]), $format_bold_right);
	//	$indx++;
	//	$rep->sheet->writeString($rep->y, $indx, abs($amount), $format_bold_right);
	//	$indx++;
	//	$rep->sheet->writeString($rep->y, $indx, '', $format_bold_right);
	...
	/* die;
	$indx++;
	$rep->sheet->writeString($rep->y, $indx, 'dfdfdfd', $format_bold_right);
	$indx++;
	$rep->sheet->writeString($rep->y, $indx, '2000', $format_bold_right);
	$indx++;
	$rep->sheet->writeString($rep->y, $indx, '2000', $format_bold_right);
