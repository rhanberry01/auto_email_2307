...
function print_outstanding_GRN()
{
	...
	$cols = array(0, 30, 50, 190,	250, 320, 385, 450,	515);

	// $headers = array(_('Order #'), _(''), _('Item') . '/' . _('Description'), _('Qty Ord'), _('Qty Rcvd'), _('Balance'),
		// _('Unit Cost'), _('Value'));
    ...
    $rep = new FrontReport(_('Outstanding PO Report'), "OutstandingPO", user_pagesize());
	...
	While ($GRNs = db_fetch($res))
	{
		...
		if ($Supplier != $GRNs['supplier_id'])
		{
			...
			$rep->NewLine();
			$rep->fontSize += 2;
			$rep->TextCol(0, 6, strtoupper($supp_name));
		...
		$rep->TextCol(3, 4, "Qty Ord");
		$rep->TextCol(4, 5,	"Qty Rcvd");
