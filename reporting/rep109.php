...
function print_sales_orders()
{
	...
	if ($to == null)
		$to = 0;
		if(isset($_GET['order_no'])) $from = $to = $_GET['order_no'];
	$dec = user_price_dec();
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		//$rep->AddImage($path_to_root . "/printouts/so_form.jpg", 0, 288, 576, 504);
		//$rep->AddImage($path_to_root . "/printouts/scanned/so_form_cropped.jpg", 0, 288, 576, 504);
		
		$rep->fontSize = 7;
		$rep->TextWrap(5, 783, 300, 'Print Date : '.date('m-d-Y h:i a'));
		
		$rep->fontSize = 14;
		$rep->TextWrap(490, 738, 300, $myrow['reference']);
		$rep->fontSize = 9;
		$rep->TextWrap(438, 678, 80, sql2date($myrow['ord_date']));
		$rep->TextWrap(32, 660, 300, $myrow['name']);
		$rep->TextWrap(450, 660, 300, get_salesman_name($myrow['salesman']));
		$rep->TextWrap(30, 642, 300, $myrow['add_']);
		$rep->TextWrap(445, 624, 300, get_terms($myrow['payment_terms']));
		$rep->row = 575;
		...
		while ($myrow2=db_fetch($result))
		{
			...
			$oldrow = $rep->row;
			$rep->TextColLines(2, 3, $myrow2['stk_code']." - ".$myrow2['description']."  ".$myrow2['comment'], -2);
			$newrow = $rep->row;
			$rep->row = $oldrow;
		...
		$rep->TextWrap(75, 365, 300, $user_real_name);
		$rep->TextWrap(365, 342, 300, $ao['real_name']);
