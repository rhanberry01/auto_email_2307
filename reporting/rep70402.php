...
function print_GL_transactions()
{
	...
	$destination = $_POST['PARAM_5'];
		...
		$sql = "SELECT account_code,account_name FROM ".TB_PREF."chart_master WHERE account_type IN (40200,'4020')";
		$res=db_query($sql);
		while ($row=db_fetch($res))
		{
			$fromacc[]=$row['account_code'];
		}
	...
	$rep = new FrontReport(_('Promo Fund'), "PromoFund", user_pagesize(),9 ,'L');
	$dec = user_price_dec();
  ...
  //$cols = array(0, 80, 100, 150, 210, 280, 340, 400, 450, 510, 570);
	...
	$aligns = array('left', 'left', 'left',	'left',	'left', 'left',	'left',	'left',	'right', 'right', 'right');
		...
		$headers = array(_('Type'),	'', _('SAF #'),	_('Date'), "Particulars", "", 'TIN', _('Person/Item'),
	...
	while ($account=db_fetch($accounts))
	{
		...
		if ($rows > 0)
		{
			...
			{
				$reference = get_supp_trans($myrow["type_no"],$myrow["type"]);
				...
				$rep->TextCol(2, 3, $reference['supp_reference']);
