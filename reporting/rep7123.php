...
if ($t_nt)
	...
	// if ($t_nt)
			// $sql = "SELECT c.trans_no, c.type, c.supplier_id, c.tran_date, c.cv_id, a.id as a_id,
			// a.cv_no,b.trans_date, b.id as b_id, b.amount, b.reconciled, cd.chk_number, 
			// cd.chk_date, cd.deposit_date, cd.chk_amount
			// FROM 0_supp_trans as c
			// LEFT JOIN 0_cv_header as a 
			// ON c.cv_id=a.id 
			// LEFT JOIN 0_bank_trans as b 
			...
			// AND !ISNULL(cd.chk_number) order by b.amount desc";
	// else
		// $sql = "SELECT c.trans_no, c.type, c.supplier_id, c.tran_date, c.cv_id, a.id as a_id,
			// a.cv_no,b.trans_date, b.id as b_id, b.amount, b.reconciled, cd.chk_number, 
			// cd.chk_date, cd.deposit_date, cd.chk_amount
