...
function display_type ($type, $typename, $from, $to, $convert, &$dec, &$rep, $dimension, $dimension2, &$pg, $graphics, $add_spacing=0)
{
	$code_open_balance = 0;
	...
	while ($account=db_fetch($result))
	{
		...
		if (!$printtitle)
		{
			...
			$printtitle = 1;
			$rep->row -= 4;
			$rep->TextCol(0, 5, $typename, 0, 0, 0, 0, NULL, 1, 12+$add_spacing);
			$rep->row -= 4;
			$rep->Line($rep->row);
			$rep->NewLine();		
			$rep->Font('');
			...
			$rep->TextCol(1, 2,	$account['account_code'] . ' - ' . $account['account_name']);
	...
	while ($accounttype=db_fetch($result))
	{
		...
		if (!$printtitle AND get_gl_type_trans_from_to_amount($from, $to, $accounttype["id"], $dimension, $dimension2) != 0)
		{
			$printtitle = 1;
			$rep->row -= 4;
			$rep->TextCol(0, 5, $typename, 0, 0, 0, 0, NULL, 1, 12+$add_spacing);
			...
			$totals_arr = display_type($accounttype["id"], $accounttype["name"], $from, $to, $convert, $dec, 
				$rep, $dimension, $dimension2, $pg, $graphics, 12);
	...
	if (($code_open_balance != 0 OR $open_balance_total != 0 OR $code_period_balance != 0 OR $period_balance_total != 0))
	{
		$rep->row += 6;
		$rep->Line($rep->row);
		$rep->NewLine();
		$rep->Font('b');
		$rep->TextCol(0, 2,	_('Total') . " " . $typename, 0, 0, 0, 0, NULL, 1, 12+$add_spacing);
