...
function getTransactions($category, $location, $fromcust, $from, $to)
{
	...
	$sql = "SELECT ".TB_PREF."stock_master.category_id,
			...
			SUM(
				-".TB_PREF."stock_moves.qty*".TB_PREF."stock_moves.price
				*(1-".TB_PREF."stock_moves.discount_percent)
				*(1-".TB_PREF."stock_moves.discount_percent2)
				*(1-".TB_PREF."stock_moves.discount_percent3)
				*(1-".TB_PREF."stock_moves.discount_percent4)
				*(1-".TB_PREF."stock_moves.discount_percent5)
				*(1-".TB_PREF."stock_moves.discount_percent6)
			) / 1.12 AS amt,
			SUM(-".TB_PREF."stock_moves.qty *(".TB_PREF."stock_master.material_cost + ".TB_PREF."stock_master.labour_cost + ".TB_PREF."stock_master.overhead_cost))/1.12 AS cost
...
function print_inventory_sales()
{
	...
	$cols = array(0, 75, 175, 290, 345, 415, 485,	555);
	...
	while ($trans=db_fetch($res))
	{
		...
		if ($fromcust == ALL_NUMERIC)
		{
			...
			$rep->TextCol(2, 3, $trans['debtor_name'], 0, 0, 0, 0, NULL, 1, 10);
		}
		else
			$rep->TextCol(1, 3, $trans['description'].($trans['inactive']==1 ? " ("._("Inactive").")" : ""), -1);
		$rep->AmountCol(3, 4, $trans['qty'], get_qty_dec($trans['stock_id']), -10);
