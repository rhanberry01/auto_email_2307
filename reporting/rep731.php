...
function print_GL_transactions()
{
	...
	$from = $_POST['PARAM_0'];
	$to = $_POST['PARAM_1'];
	$trade_or_nontrade = $_POST['PARAM_2'];
	...
	$params =   array( 	0 => $comments,
    				    1 => array('text' => _('Period'), 'as of' => $to));
...
// st.supp_reference, st.cv_id, st.special_reference, st.reference, cv.bank_trans_id, cv.approved FROM 0_gl_trans a
...
// ORDER BY c.supp_name, a.type, a.tran_date, a.amount";
		...
		// $res = db_query($sql,'error.');
			...
			$sql = "SELECT a.supplier_id, a.reference as apv_no, a.del_date as delivery_date, a.tran_date as tran_date, 
			b.supp_name, a.supp_reference as supp_reference, 
			round(a.ov_amount+a.ov_gst,2) as amount, c.cv_no
			...
			";
				if($trade_or_nontrade <> '2'){
					$sql .= "and a.non_trade != ".$trade_or_nontrade." "; // added due to accunting request module trade payable ** rhan 11/10/17
				}
			$sql .= " ORDER BY supp_name, a.tran_date";
	 //echo $sql; die;
	...
	$res = db_query($sql);
		...
		while ($row = db_fetch($res))
		{
			...
			if ($row['tran_date']!='' and $row['tran_date']!='0000-00-00' ){
