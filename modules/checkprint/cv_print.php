...
{
	global $cv_header, $cheque, $db_connections, $path_to_root, $id, $is_belen_tan;
	...
	if ($cheque->pageNumber > 1)
	{
		...
		else if($cv_header['online_check_payment'] == 2 OR $cv_header['online_aca_payment'] == 2)
			$cheque->AddImage($path_to_root . "/reporting/images/cv_blank_mc.png", 0, $cheque->pageHeight, 612);
		else
		...
		$prepared_by_sql = "SELECT user, date(stamp), time(stamp) FROM ".TB_PREF."audit_trail 
										WHERE type = 99 
										AND trans_no = $id
		...
		if (db_num_rows($prepared_by_res) > 0)
		{
			...
			$cheque->TextWrap(58+70, 71-11-12-27+13, 100, date('m/d h:i A',strtotime($cv_header['creation_date'])), 'left');
		}

		$noted_by_sql = "SELECT user,date(stamp), time(stamp) FROM ".TB_PREF."audit_trail 
									WHERE type = 99 
									AND trans_no = $id
		...
		if (db_num_rows($noted_by_res) > 0)
		{
			...
			$cheque->TextWrap(58+111+18+111+18+63, 71-11-12-27+13, 90, $date__[1].'/'.$date__[0]. ' ' . date('h:i A',strtotime($noted_by_row[2])), 'left');
		}
	...
	if ($is_belen_tan)
		...
		$cheque->TextWrap(0,($cheque->pageHeight-$cheque->topMargin),$cheque->pageWidth,'TAN, BELEN T (San Roque Supermarket - Novaliches)', 'center');
...
$is_belen_tan = 1;
if (isset($_GET['is_belen_tan']))
	$is_belen_tan = $_GET['is_belen_tan'];
...
foreach($ids as $id)
{
	...
	//SAN ROQUE SUPERMARKET RETAIL SYSTEMS INC. - Novaliches
