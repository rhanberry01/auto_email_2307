...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error( _("The Chequing Number cannot be empty."));
	...
	if ($input_error != 1) 
	{
    	...
    	if ($selected_id != -1) 
    	{
    		...
    		update_check_banking_reference($selected_id, $_POST['next_reference'], 
				$_POST['booklet_start'], $_POST['booklet_end']);
    		commit_transaction();
...
$th = array(_("Account"), _("Bank Account Reference"), _("Next Cheque No"),
			_("Booklet Start"),_("Booklet End"), "", "");
			
if ($_SESSION['wa_current_user']->access == 2) // admin only
	array_splice($th,2,0,'User');
...
if ($Mode== 'Edit') {
	...
	if ($flag_count) unassigned_check_list_row(_("Unassigned Chequing Accounts:"), 'bank_ref', null, true);
...
end_table(1);
echo '<center>*** leave booklet start and end to disable checking of check number if within the series</center>';
