				...
				case 3: //for online only
					$where .= " AND x.amount > 0
							  ...
							  AND x.person_id IN (SELECT supplier_id 
								FROM ".TB_PREF."suppliers WHERE billing_institution_code != '' AND billing_institution_code != 'PBCOM')	";
					// CHECK FOR PRINTED CV
					// if ($print_status == 1) // printed CV only
					// {
						// $sql .= " AND cv_printed = 1 AND amount > 0";
	...
	if ($_POST['hide_dr'] == 0)
	 ...
	 while ($myrow = db_fetch($result))
	 {
		...
		if (!$cv_voided)
		{
			...
			else  // approved
			{
				if ($myrow['bank_trans_id'] != 0) // check issued
				{
					...
					if ($myrow['online_payment'] == 0 AND $myrow['online_check_payment'] == 0)
					{
						label_cell('<b>Paid '.
				...
				{
					if ($myrow['online_payment'] == 1 and $myrow['online_check_payment'] == 0 and check_cv_id($myrow['id']) == '')
					{
						$_POST['for_op'.$myrow['id']] = 1;
						label_cell('<b>for Online Payment</b>', 'nowrap');
					}
					else if ($myrow['online_payment'] == 1 and $myrow['online_check_payment'] == 0  and check_cv_id($myrow['id']) != ''){
						label_cell('<b>NOT YET CLEARED</b>','align=center');
					}
					
					else if ($myrow['online_check_payment'] == 1 and $myrow['online_payment'] == 0 and check_cv_id($myrow['id']) == '')
					{
		...
		if($_SESSION['wa_current_user']->access == 16 OR $_SESSION['wa_current_user']->access == 6)
			...
			if ($myrow['bank_trans_id'] != 0)
			{
				if(($myrow['bank_trans_id'] != 0 AND $myrow['online_payment'] != 2 AND $myrow['online_check_payment'] != 2) 
