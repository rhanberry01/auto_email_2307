...
function cv_print_header()
{
	...
	$cheque->fontSize += 3;
	// $cheque->font('b');
	$cheque->TextWrap(85+360+60, 325+18+14, 100,$cv_header['cv_no'], 'left');
	// $cheque->font('');
	$cheque->fontSize -= 5;
	$cheque->TextWrap(0,($cheque->pageHeight-$cheque->topMargin)-25,$cheque->pageWidth,$com['coy_name'], 'center');
	// $cheque->TextWrap(0,($cheque->pageHeight-$cheque->topMargin)-10,$cheque->pageWidth,$com['postal_address'], 'center');
	$cheque->fontSize += 2;
	//}
}
...
while ($cv_d_row = db_fetch($cv_details))
{
	...
	for ($ii = 0; $ii <= 0 ; $ii ++)
	{
		...
		if ($tran_det['supp_reference'] != '')
			$add_text = "  --  SI # " . $tran_det['supp_reference'];
		...
		$sobra = $cheque->TextWrap(110, $cheque->row, 250, $systypes_array[$cv_d_row['trans_type']]. ' # '.$tran_det['reference'] . $add_text , 'left',
			0, 0, NULL, 0, false);
		...
		// $cheque->TextWrapLines(110,250,$systypes_array[$cv_d_row['trans_type']]. ' # '.$tran_det['reference'] . $add_text , 'left');
		
		// $trans__[] = array($cv_d_row['trans_type'], $cv_d_row['trans_no']);
		// $total_allocated += $cv_d_row['amount'];
	}
}
