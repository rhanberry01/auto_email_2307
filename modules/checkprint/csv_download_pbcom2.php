...
$sql = "SELECT * FROM ".TB_PREF."csv_pbcom
		WHERE id = $id";
$res = db_query($sql);
$row = db_fetch($res);
...
$filename = strtoupper($db_connections[$_SESSION['wa_current_user']->company]['srs_branch']).'_PBCOM_batch_'.$id.'.csv';
...
if (!file_exists($target))
{
	...
	$sql = "SELECT * FROM ".TB_PREF."csv_details_pbcom
			WHERE csv_id = $id";
	// display_error($sql);
	$res = db_query($sql);
	$count = 0;
