...
function blur_alloc(i) {
		...
		if (i.name.substr(0,11) != 'ewt_percent')
			...
			var change = (get_amount(i.name) / 1.12) * (get_amount('tot_amount'+i.name.substr(11)) / 100);
		...
		if (i.name != 'amount' && i.name != 'charge' && i.name != 'discount' && i.name!= 'ewt')
		{
			...
			{
				ewt_amount = (get_amount(i.name) / 1.12) * (get_amount('tot_amount'+i.name.substr(11)) / 100) ;
				last_ewt_amount = (i.getAttribute('_last') / 1.12) * (get_amount('tot_amount'+i.name.substr(11)) / 100) ;
...
function allocate_none(doc) {
	...
	ewt_amount = (get_amount('ewt_percent'+doc) / 1.12) * (get_amount('tot_amount'+doc) / 100);
