...
(function($) {
	$.prompt = function(message, options,ty) {
		...
		$.each(message,function(statename,stateobj){
			...
			$state.children('.'+ options.prefix +'buttons').children('button').click(function(){
				...
				if(close === undefined || close) {
					removePrompt(true,clicked,msg,forminputs,ty);
				}
			});
		...
		var removePrompt = function(callCallback, clicked, msg, formvals,ty){
			// $jqi.remove();
			jQuery.prompt.close();
			...
			$jqif.fadeOut(options.overlayspeed,function(){
				...
				$jqif.remove();
				if(callCallback) {
					options.callback(clicked,msg,formvals,ty);
				}
	...
	$.prompt.defaults = {
		...
		buttons: {
	 	...
	 	loaded: function(){
			...
			if($.find('input[name="uname"]').length)
